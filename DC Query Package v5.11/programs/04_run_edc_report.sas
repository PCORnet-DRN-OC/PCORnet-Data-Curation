...
*    Study: PCORnet
*
*  Purpose: Run the Empirical Data Curation portion of PCORnet Data Curation Query Package v5.11
* 
*   Inputs: 
...
*          3) edc_report.sas - Print of compiled EDC report in PDF file format 
*              stored as (<DataMart Id>_<response date>_EDCRPT.pdf)
*
*  Requirements:  
...
*
*  Actions needed: 
*               1) User provides root network/server paths where the required
*                     sub-directory structure was created for the most recently approved DataMart refresh
*					  (Example: /ct/pcornet/queries/PROD_P02_DQA_FDPRO_DCQ_T1D3_r009_v01/)
*               2) User provides root network/server paths where the required
*                     sub-directory structure has been created (section below) 
*                     (Example: /ct/pcornet/queries/PROD_P02_DQA_FDPRO_DCQ_T1D3_r010_v01/) 
*******************************************************************************/
...
********************************************************************************;
 ...
 /*Enter network/server directory for the 
 query results from the 
 most recently approved DataMart refresh: */   %let ppath=;
 
 /*Enter network/server directory
 for the current query results: */           %let qpath=;

...
*  Outputs: 
*    1) Up to 11 SAS datasets stored in /dmlocal containing all records which violate one or more of the heuristics
...
data immunization (compress=yes);
	...
	where not missing(vx_code) and vx_code_type in ('ND','RX','CH');
	table='IMMUNIZATION';
	code_type=vx_code_type;
	code=vx_code;
	if code_type in ('CH') then code_clean=strip(upcase(compress((code),"-, "))); 
		else if code_type in ('ND','RX') then code_clean=strip(upcase(compress((code)," ")));

...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package V6.12 - all queries,
  ...
  %elapsed(begin);
    ...
    *- Macro to compare dates against birth and death -*;
    ...
    %if &_yprocedures=1 %then %do;
        proc sort data=procedures;
             by patid;
        run;
        %xmin(idsn=procedures,var=px_date,ord=3)
    %end;
    %if &_ydiagnosis=1 %then %do;
        proc sort data=diagnosis;
             by patid;
        run;
        %xmin(idsn=diagnosis,var=dx_date,ord=13)
    %end;
    %if &_yvital=1 %then %do;
    ...
    *- Macro to compare dates within a data table -*;
    ...
    %if &_yencounter=1 %then %do;
        ...
        %within(idsn=encounter,var=discharge_date,var2=admit_date,ord=28)
    %end;
    %if &_yprocedures=1 %then %do;
        %within(idsn=procedures,var=px_date,var2=admit_date,adjm=-5,ord=27)
    %end;
    %if &_ydiagnosis=1 %then %do;
        %within(idsn=diagnosis,var=dx_date,var2=admit_date,adjm=-5,ord=29)
    %end;
    %if &_yobs_clin=1 %then %do;
        %within(idsn=obs_clin,var=obsclin_stop_date,var2=obsclin_start_date,ord=31)
    %end;
    %if &_yobs_gen=1 %then %do;
        %within(idsn=obs_gen,var=obsgen_stop_date,var2=obsgen_start_date,ord=32)
    %end;

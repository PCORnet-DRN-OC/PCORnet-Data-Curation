...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package V6.12 - 
...
filename edcref "&qpath./infolder/edc_reference.cpt";
...
%macro minmax(idsn,var,var_tm,_n);
     ...
     proc means data=&idsn._dates nway noprint;
          var &var time&_n futureflag&_n preflag&_n;
...
********************************************************************************;
*- EDC reference data sets, but only interested in DC_TABLES, drop all others -*;
proc cimport library=work infile=edcref;
run;

proc datasets noprint;
     delete DC_SUMMARY FOOTERS FORMATS HEADERS LAB_VOLUME_REF PEDIATRIC_DATAMARTS 
            Q2_STAT_DLG_LOINC REQUIRED_STRUCTURE SPECIMEN_SOURCE_CATEGORY 
            TBL_IVI_REF TOC;
quit;
...
********************************************************************************;
*- Create macro variable containing all of the loinc values to exclude
********************************************************************************;
%global _excl_loinc;

data _null;
     length exclude $7000;
     set loinc_exclude end=eof;
    
     retain exclude;
     if _n_=1 then exclude= '"' ||strip(code)|| '"';
     else exclude=strip(exclude) ||' "' ||strip(code)|| '"';
    
     if eof then call symput('_excl_loinc',strip(exclude));
run;

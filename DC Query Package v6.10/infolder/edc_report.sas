...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet EDC report v6.10
* 
...
%expref(dsn=lab_volume_ref);
%expref(dsn=specimen_source_category);
%expref(dsn=q2_stat_dlg_loinc);
...
proc format;
     ...
     value met_desc
        ...
        3='Number of unique patients with at least 1 DIAGNOSIS face-to-face encounter and VITAL records within the past 5 years'
        4='Number of unique patients with at least 1 DIAGNOSIS face-to-face encounter, VITAL, and PRESCRIBING or MED_ADMIN records within the past 5 years'
        5='Number of unique patients with at least 1 DIAGNOSIS face-to-face encounter, VITAL, PRESCRIBING or MED_ADMIN, and LAB_RESULT_CM records within the past 5 years'
...
data tbl;
     ...
     if ord=0 then do; col4=" "; threshold=.; end;
         ...
         threshold=.;
         if resultn^=.t and denom_dem>0 then col4=strip(put((resultn/denom_dem)*100,comma16.))|| "%";
         else col4=" ";
     end;
     else do;
         if ord=3 and resultn^=.t and denom_dem345>0 and .<(resultn/denom_dem345)*100<80.0 then threshold=1;
         if resultn^=.t and denom_dem345>0 then col4=strip(put((resultn/denom_dem345)*100,comma16.))|| "%";
         else col4=" ";
     end;
run;
...
%macro print_ib;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ib split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define col4     /display flow "Percent" style(header)=[just=center cellwidth=10%] style(column)=[just=center];
         define col5     /display flow "Source table" style(header)=[just=left cellwidth=12%] style(column)=[just=left];
         compute col4;
            if threshold=1 then call define(_col_, "style", "style=[color=blue]");
         endcomp;
    run;    
...
%macro _plat;
    proc sort data=lab_volume_ref(keep=pedi dc_lab_group lower_bound median)
...
data print_ig;
     length col1 col2 col3 col4 col5c col6 $50;
     ...
     * create column variables and output *;
     ...
     if lower_bound^=. and median^=. then col4=strip(put(lower_bound,8.1))||" ("||strip(put(median,8.1))||")";
...
%macro print_ig;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ig split='|' style(header)=[backgroundcolor=CXCCCCCC];
         column pg col5c col5flag col1 col2 col3 col4 col5 col6;
         where pg=1;

         define pg       /order noprint;
         define col5c    /display noprint;
         define col5flag /display noprint;
         ...
         define col3     /display flow "Percentage of|patients in the ENCOUNTER table" style(header)=[just=center cellwidth=15%] 
                                    style(column)=[just=center];
         define col4     /display flow "Threshold (median) percentage of patients in the ENCOUNTER table" 
                                    style(header)=[just=center cellwidth=13%] style(column)=[just=center];
         define col5     /computed flow "Data Check 3.13 Exception"  style(header)=[just=center cellwidth=12%] style(column)=[just=center];
         define col6     /display flow "Source tables" style(header)=[just=left cellwidth=22%] style(column)=[just=left];
         compute col5 / char length=3;
            if col5flag=1 then do;
                col5=col5c;
...
data data;
     ...
     * flag BT records *;
     ...
     keep dc_name category cross_category2 statistic diag resultn bt_flag cross_category;
run;
...
proc sql noprint;
     ...
     select sum(resultn) into :denom_obsclin from dc_normal
         where datamartid=%upcase("&dmid") and dc_name in ("OBSCLIN_L3_N") and
            variable="OBSCLINID" and statistic in ("ALL_N" "NULL_N");
     select sum(resultn) into :denom_obsknown from dc_normal
         where datamartid=%upcase("&dmid") and dc_name in ("OBSCLIN_L3_RECORDC") and
            statistic="ALL_N" and variable in ("KNOWN_TEST_RESULT_NUM");
quit;
...
data r12;
     ...
     col0=strip(put(ord,_data_check.));
     col1=strip(put(ord,known_desc.));
     col3=strip(put(&distinct_obsclin,threshold.));
...
data print_ivi_oc;
     ...
     keep pg tbl_ord ord col:;
run;

data print_ivi;
     set print_ivi r12 print_ivi_oc;
run;
...
%macro print_ivi;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ivi split='|' style(header)=[backgroundcolor=CXCCCCCC];
         column pg col000 col00 col0 col1 ord col3 col4 col5p col5 col6;
         where pg=1;
...
%macro print_va;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_va split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define col3     /computed format=thresholdc. "Current|Refresh" style(header)=[just=center cellwidth=12%] style(column)=[just=center];
         ...
         define col6     /computed format=thresholdc. "Current|Refresh" style(header)=[just=center cellwidth=12%] style(column)=[just=center];
...
%macro print_vb;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_vb split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define col3     /computed format=thresholdc. "Current|Refresh" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         ...
         define col6     /computed format=thresholdc. "Current|Refresh" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
...
%macro print_vc;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_vc split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define col2     /display flow "Previous|Refresh" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col3     /computed format=thresholdc. "Current|Refresh" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col4     /computed format=threshold1c. "Percent|Change" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col5     /display flow "Previous|Refresh" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col6     /computed format=thresholdc. "Current|Refresh" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col7     /computed format=threshold1c. "Percent|Change" style(header)=[just=center cellwidth=11%] style(column)=[just=center];

...
data &qname(compress=yes);
     set pcordata.&qname(keep=patid encounterid medadmin_start_: medadmin_stop:
                              medadmin_providerid medadmin_code);
...
data &qname(compress=yes);
     ...
     * known test result range *;
     ...
     drop specimen_source lab_result_source priority result_qual 
...
*- Clear working directory -*;
%clean(savedsn=&savevit &savepres &savedisp &savelab &savemedadm nobs_all);
...
%if &_ydiagnosis=1 %then %do;
    ...
    *- Data with legitimate date -*;
    data xdiagnosis(compress=yes);
         set pcordata.diagnosis(keep=admit_date patid enc_type where=(admit_date^=. and 
                                enc_type in ("ED" "EI" "IP" "OS" "AV")));
...
*- Clear working directory -*;
%clean(savedsn=xdiagnosis xvital &savepres &savedisp &savelab &savemedadm nobs_all);
...
*- Data with legitimate date -*;
data xprescribing;
     set prescribing(keep=RXNORM_CUI rx_order_date patid where=(^missing(RXNORM_CUI) and rx_order_date^=.));
...
*- Clear working directory -*;
%clean(savedsn=xdiagnosis xvital xprescribing xmed_admin &savelab &savemedadm nobs_all);

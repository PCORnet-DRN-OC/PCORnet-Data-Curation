...
*    Study: PCORnet
*
*  Purpose: Run the query portion of Data Curation Query Package v6.10 
* 
*   Inputs: 
*	    1) PCORnet CDM v6.1 SAS data stores (datasets or views) 
*	    2) SAS programs: /infolder/data_curation_query_base.sas
*						 /infolder/data_curation_query_lab.sas
*						 /infolder/data_curation_query_obsclin.sas
*						 /infolder/data_curation_query_xtbl.sas
*						 /infolder/data_curation_query_main.sas
*						 /infolder/data_curation_print.sas
*       3) SAS datasets: /infolder/dc_reference.cpt
*
*  Outputs: 
*       From data_curation_query_base.sas
...
*                (<DataMart Id>_<response date>_data_curation_<_grp>.cpt)
*           4) SAS log files of query portion stored in /drnoc
*                (<DataMart Id>_<response date>_data_curation_query_base.log)
*                (<DataMart Id>_<response date>_data_curation_query_<grp>.log)
...
*				3) User provides the query group to process (section below)
...
*               6) User provides the low cell count value if requested to do so (section below)
*                     (Example: 11) (default is 0)
*               7) User provides the lookback years if requested to do so (section below)
...
********************************************************************************;
 ...
 /*Enter low cell count threshold value. DO NOT MODIFY
 UNLESS INSTRUCTED TO DO SO*/                			%let threshold=0;

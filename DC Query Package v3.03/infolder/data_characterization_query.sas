...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Characterization Query Package V3.03
* 
...
*           6) SAS transport file of DataMart meta-data stored in /drnoc
...
%macro minmax(idsn,var,var_tm);
     ...
     proc means data=&idsn.&var nway noprint;
          ...
          output out=m&idsn n=ndt ntm dum1 dum2  sum=dum3 dum4 futuresum presum
                 ...
                 mean=meandt meantm dum9 dum10  median=mediandt mediantm dum11 dum12 
                 nmiss=nmissdt nmisstm dum13 dum14;
     run;
     ...
     data &idsn._&var;
          ...
          * counts *;
          ...
          keep datamartid response_date query_package dataset tag tag_tm 
               min max mean median n nmiss future_dt_n pre2010_n min_tm 
               max_tm mean_tm median_tm n_tm nmiss_tm;
     run;
%mend minmax;
...
data _null_;
     ...
     retain ulength 0;
     if name in ("PATID" "ENCOUNTERID" "ADMIT_DATE" "PROVIDERID") then ulength=ulength+length;
    
     if eof then call symput("_ulength",strip(put(ulength,8.)));
run;

*- Derive categorical variable -*;
data data;
     length col1 $4 _unique_elig _unique_visit $&_ulength;
     ...
     * create UNIQUE_VISIT and determine the max length needed *;
     retain maxlength_uv maxlength_elig 0;
     if cmiss(patid,encounterid,admit_date,enc_type)=0 then do;
        _unique_elig=compress(patid)|| "/" || compress(encounterid) || "/" || 
                    put(admit_date,date9.);
        maxlength_elig=max(maxlength_elig,length(_unique_elig));

        if cmiss(patid,encounterid,admit_date,providerid,enc_type)=0 then do;
            _unique_visit=compress(patid)|| "/" || compress(encounterid) || "/" || 
     ...
     * create macro variable to re-size UNIQUE_VISIT *;
     if eof then do;
            call symput("_maxlen_elig",strip(put(maxlength_elig,8.)));
            call symput("_maxlen_uv",strip(put(maxlength_uv,8.)));
     end;

     keep col1 _unique_visit _unique_elig patid;
run;

*- Re-establish length of UNIQUE_VISIT to make it as small as possible -*;
data data;
     length unique_visit $&_maxlen_uv unique_elig $&_maxlen_elig;
     set data;
     unique_visit=_unique_visit;
     unique_elig=_unique_elig;
     drop _unique_visit _unique_elig;
run;
...
proc sort data=data(drop=patid unique_elig) out=visit nodupkey;
...
proc sort data=data(drop=unique_visit unique_elig) out=pid nodupkey;
     by col1 patid;
     where patid^=' ';
run;
...
proc sort data=data(drop=patid unique_visit) out=elig nodupkey;
     by col1 unique_elig;
     where unique_elig^=' ';
run;
...
data query;
     ...
     merge stats(where=(_type_=1)) 
           ...
           elig_unique(rename=(_freq_=distinct_elig))
     ;
     by col1;
     ...
     * counts *;
     ...
     elig_record_n=strip(put(distinct_elig,threshold.));
     if _freq_>&threshold then record_pct=put((_freq_/denom)*100,6.1);
...
data data;
     ...
     * set missing to impossible high value for categorization *;
     ...
     else if dispense_sup=. then col1=99999;
     keep col1;
run;
...
data data;
     ...
     if rxnorm_cui^=. then col1=strip(put(rxnorm_cui,16.));
...
data query;
     ...
     * counts *;
     ...
     keep datamartid response_date query_package rxnorm_cui record_n record_pct
          distinct_patid_n;
run;
...
proc sql;
     create table dmlocal.&qname as select
         datamartid, response_date, query_package, rxnorm_cui, record_n, record_pct,
             distinct_patid_n
     from query;
quit;
...
proc means data=rxnorm_cui nway completetypes noprint missing;
     class col1/preloadfmt;
     output out=rxnormcui_unique n=n;
     format col1 $null.;
run;
...
proc means data=data nway completetypes noprint missing;
     class col1/preloadfmt;
     var rx_days_supply;
     output out=stats n=rxn mean=rxmean max=rxmax sum=rxsum min=rxmin nmiss=rxnmiss;
     format col1 $null.;
run;
...
data data;
     ...
     else if lab_name=" " then col1="ZZZA";
     else col1="ZZZB";
...
data data;
     ...
     else if lab_name=" " then labname="ZZZA";
     else labname="ZZZB";
    
     if lab_loinc^=" " then col1=lab_loinc;
     else if lab_loinc=" " then col1="ZZZD";
...
data query;
     length record_n distinct_patid_n $20;
     if _n_=1 then set stats(where=(_type_=2) rename=(_freq_=denom));
     ...
     * apply threshold *;
     %threshold(nullval=col1^="ZZZA")
     %threshold(_tvar=record_loinc,nullval=col1^="ZZZA")
     %threshold(_tvar=distinct_patid,nullval=col1^="ZZZA")
...
data data;
     ...
     if lab_px_type in ("09" "10" "11" "C2" "C3" "C4" "H3" "HC" "LC" "ND" "RE") 
...
%macro orph_enc;
    %do oe = 1 %to &workdata_count;
        ...
        %if &_orph=DIAGNOSIS or &_orph=PROCEDURES or &_orph=VITAL or 
            &_orph=LAB_RESULT_CM or &_orph=PRESCRIBING %then %do;

            proc sql noprint;
                 create table orph_&_orph as select unique encounterid
                %if &_orph=DIAGNOSIS or &_orph=PROCEDURES %then %do;
                    , enc_type, admit_date, providerid  %end;
                    from pcordata.&_orph
                    where encounterid^=" "
                    order by encounterid;
            quit;

            data orph_&_orph(keep=memname ord recordn mis:);
                 length encounterid $ &maxlength memname $32;
                 merge orph_&_orph(in=o) 
                       orphan_encid(in=oe rename=(enc_type=etype admit_date=adate
                                                  providerid=provid)) end=eof;
                 by encounterid;
                 retain recordn 0;
                 if o and not oe then recordn=recordn+1;
                 retain mis_etype_n mis_adate_n mis_provid_n 0;
                 %if &_orph=DIAGNOSIS or &_orph=PROCEDURES %then %do;
                     if o and oe then do;
                         if enc_type^=etype then mis_etype_n=mis_etype_n+1;
                         if admit_date^=adate then mis_adate_n=mis_adate_n+1;
                         if providerid^=provid then mis_provid_n=mis_provid_n+1;
                     end;
                 %end;
                 if eof then do;
                     memname="&_orph";
...
%macro orph_pat;
    %do op = 1 %to &workdata_count;
        ...
        %if &_orphp=ENROLLMENT or &_orphp=ENCOUNTER or &_orphp=DIAGNOSIS or 
            ...
            &_orphp=PRESCRIBING or &_orphp=DISPENSING or &_orphp=DEATH %then %do;
...
data query;
     ...
     set orph_encounterid(in=oe)
         orph_patid(in=op)
         orph_encounterid(in=mde drop=recordn where=(memname="DIAGNOSIS") rename=(mis_etype_n=recordn))
         orph_encounterid(in=mda drop=recordn where=(memname="DIAGNOSIS") rename=(mis_adate_n=recordn))
         orph_encounterid(in=mdp drop=recordn where=(memname="DIAGNOSIS") rename=(mis_provid_n=recordn))
         orph_encounterid(in=mpe drop=recordn where=(memname="PROCEDURES") rename=(mis_etype_n=recordn))
         orph_encounterid(in=mpa drop=recordn where=(memname="PROCEDURES") rename=(mis_adate_n=recordn))
         orph_encounterid(in=mpp drop=recordn where=(memname="PROCEDURES") rename=(mis_provid_n=recordn))
     ;
...
proc sql;
     create table dmlocal.&qname as select
         datamartid, response_date, query_package, dataset, tag, min, median, 
         max, n, nmiss, future_dt_n, pre2010_n
     from query;
quit;
...
data min&var(keep=compvar b_cnt d_cnt);
     length compvar $20;
     merge min&idsn(in=m) 
           demographic(keep=patid birth_date) 
           death(keep=patid death_date) 
           end=eof;
     by patid;
...
data xprescribing;
     length year 4.;
     set prescribing(keep=rxnorm_cui rx_start_date patid);
     if rxnorm_cui^=.;

     if rx_start_date^=. then year=year(rx_start_date);
...
*- Clear working directory -*;
%clean(savedsn=xdiagnosis xvital &savelab);
...
*- Merge diagnosis, vital, and lab data -*;
data data;
     merge xdiagnosis(in=e) xvital(in=v) xlab_result_cm(in=l);
     by patid year;
     if e and v and l;
run;
...
title "Data Characterization query run times";
footnote;
...
%if &_yencounter=1 %then %do;
    ...
    %prnt(pdsn=enc_l3_enctype_admrsc,_obs=100,_svar=enc_type admitting_source,_suppvar=record_n record_pct);
    ...
    %prnt(pdsn=enc_l3_drg,_obs=100,_svar=drg,_suppvar=record_n record_pct);
...
%if &_ydeath=1 %then %do;
    ...
    %prnt(pdsn=death_l3_date_ym,_obs=100,_svar=death_date,_suppvar=record_n);
...
%if &_ydispensing=1 %then %do;
    %prnt(pdsn=disp_l3_n);
    %prnt(pdsn=disp_l3_ndc,_obs=100,_svar=ndc,_suppvar=record_n record_pct);
...
%if &_yprescribing=1 %then %do;
    %prnt(pdsn=pres_l3_n);
    %prnt(pdsn=pres_l3_rxcui,_obs=100,_svar=rxnorm_cui,_suppvar=record_n record_pct distinct_patid_n);
    %prnt(pdsn=pres_l3_supdist2);
    %prnt(pdsn=pres_l3_rxcui_rxsup,_obs=100,_svar=rxnorm_cui,_suppvar=min mean max sum n nmiss,_recordn=n);

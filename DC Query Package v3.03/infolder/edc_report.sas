...
options nodate nonumber nobyline orientation=landscape validvarname=upcase 
        formchar="|___|||___+=|_/\<>" missing=' ' mprint mlogic symbolgen;
...
data print_ig;
     ...
     * create column variables and output *;
     ...
     col5=strip(put((dist_patid/denom)*100,5.1));
...
data print_iib(drop=col3);
     ...
     * do not want to highlight these records *;
     ...
     if col1^ in ("LAB_RESULT_CM" "HARVEST") then pg=1;
     else pg=2;
run;
...
data orphan;
     ...
     if resultn=0 or (row=9 and scan(table,3,' ')^ in ("DIAGNOSIS" "PROCEDURES" "VITAL" "LAB_RESULT_CM")) then delete;
...
data print_iie;
     ...
     if row^=9 then do;
        ...
        if col4 not in (" " "None" "n/a") then c4=2;
        if col5 not in (" " "None" "n/a") then c5=2;
     end;
...
%macro print_iie;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_iie split='~' style(header)=[backgroundcolor=CXCCCCCC] missing;
         column pg row col1 col2 col3 c4 c5 col4 col5 col6 col7 col8;
         where pg=1;
...
data tbl(keep=ord bound statistic result col:);
     ...
     %stat(dcn=DEM_L3_AGEYRSDIST2,ord=1,bound=H,cat=>89 yrs,stat=RECORD_PCT)
     ...
     %stat(dcn=PRES_L3_SUPDIST2,ord=6,bound=L,cat=<1 day,stat=RECORD_PCT)
     %stat(dcn=PRES_L3_SUPDIST2,ord=6,bound=H,cat=>90 days,stat=RECORD_N)
     %stat(dcn=PRES_L3_SUPDIST2,ord=6,bound=H,cat=>90 days,stat=RECORD_PCT)

     %stat(dcn=DISP_L3_SUPDIST2,ord=7,bound=L,cat=<1 day,stat=RECORD_N)
     %stat(dcn=DISP_L3_SUPDIST2,ord=7,bound=L,cat=<1 day,stat=RECORD_PCT)
     %stat(dcn=DISP_L3_SUPDIST2,ord=7,bound=H,cat=>90 days,stat=RECORD_N)
     %stat(dcn=DISP_L3_SUPDIST2,ord=7,bound=H,cat=>90 days,stat=RECORD_PCT)
run;
...
data print_iiib(keep=ord col: med:);
     ...
     merge tbl(where=(statistic="RECORD_N" and bound="L") rename=(result=col7))
           tbl(where=(statistic="RECORD_PCT" and bound="L") rename=(result=col8))
           tbl(where=(statistic="RECORD_N" and bound="H") rename=(result=col9))
           tbl(where=(statistic="RECORD_PCT" and bound="H") rename=(result=col10))
           xdenom
           median
     ;
     by ord;
...
data data;
     ...
     set dc_normal(where=(datamartid=%upcase("&dmid") and statistic="RECORD_N" and
         ...
         (dc_name="DIA_L3_DXTYPE_ENCTYPE" and 
            category^ in ("NI" "UN" "OT" "NULL or missing" "Values outside of CDM specifications")))) 
     ;
...
data data;
     ...
     set dc_normal(where=(datamartid=%upcase("&dmid") and statistic="RECORD_N" and
         ...
         (dc_name="PRO_L3_PXTYPE_ENCTYPE" and 
            category^ in ("NI" "UN" "OT" "NULL or missing" "Values outside of CDM specifications"))))
     ;
...
data numerators;
     ...
     if (dc_name in ("VIT_L3_VITAL_SOURCE" "DEATH_L3_IMPUTE" "DEATH_L3_MATCH") and
        ...
        (dc_name in ("LAB_L3_NAME" "LAB_L3_SOURCE" "LAB_L3_NAME_LOINC" "LAB_L3_PRIORITY" 
...
data print_ive/*(keep=col: perc: thresholdflag:)*/;
     ...
     if pat_np>0 and pat_p>0 then do;

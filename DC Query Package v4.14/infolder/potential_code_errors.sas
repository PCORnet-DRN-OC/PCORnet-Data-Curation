...
*  Purpose: This program was developed by the Coordinating Center to help partners identify ETL errors
	    in populating ICD9 and ICD10 diagnosis codes; ICD & CPT/HCPCS procedure codes; RXCUI codes; LOINC codes; 
	    and NDC codes. The program identifes codes that violate one or more heuristics described in the 
	    Work Plan. Heuristics will not identify all erroneous codes.   

*  Outputs: 
*    1) Up to 9 SAS datasets stored in /dmlocal containing all records which violate one or more of the heuristics
...
*	 2) CSV versions of the 10 datasets stored in /dmlocal
...
*BADSUM: summarize bad records by code type and code and calculate % of total;
%macro badsum (ds=,tbl=);
proc sql;
...
*TBLSUM: summarize records by code type;
%macro tblsum (tbl=);
proc sql;
...
*********************************************************************************************;
* PROCEDURES table (ICD and CPT/HCPCS codes)
*********************************************************************************************;
...
data lab_result_cm (compress=yes);
	format lab_result_cm_id code_type code code_clean code_clean_r code_length anyalpha anydigit;
	length code_type $2 table $15;
...
data obs_clin (compress=yes);
	...
	set dpath.obs_clin (keep=obsclinid obsclin_code obsclin_type);
...
*apply heuristics to identify bad records (same as LAB_RESULT_CM);	
...
*********************************************************************************************;
* OBS_GEN table (LOINC or NDC.  Does not include ICD9 because codes could be either DX or PX)
*********************************************************************************************;
...
data obs_gen (compress=yes);
	length code_type $2 table $15;
	format obsgenid code_type code code_clean code_length anyalpha anydigit;
	set dpath.obs_gen (keep=obsgenid obsgen_code obsgen_type);
	where not missing(obsgen_code) and obsgen_type in ('LC','ND');
	table='OBS_GEN';
...
data dmlocal.code_summary;
	format DATAMARTID QUERY_DATE TABLE CODE_TYPE BAD_RECORDS RECORDS PCT;
	format _all_;
	set diagnosis_summary 
		...
		;
	label code_type='Code Type' table='Table' records='Records' bad_records='Potential Bad Records' pct='Percent';
	if pct=. then pct=0.0;
	DATAMARTID="&dmid.";
...
title2 'Code Summary';
proc print width=min data=dmlocal.code_summary label;
	run;

...
*    Study: PCORnet
*
*  Purpose: Print PCORnet Data Curation Query Package V4.14
* 
...
proc contents data=dmlocal._all_ noprint 
     %if %upcase(&_grp)=MAIN %then %do;
          out=dmlocal(where=((scan(upcase(memname),1,'_') in ("DEATH" "DEATHC" "DEM" 
              "ENC" "DIA" "PRO" "ENR" "VIT" "DISP" "PRES" "MEDADM" "OBSCLIN" "OBSGEN" 
              "COND" "PROCM" "PROV" "TRIAL") and scan(upcase(memname),2,'_')="L3") or

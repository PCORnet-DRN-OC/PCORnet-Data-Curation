...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package V6.0 - OBS CLIN queries,
* 
...
%let saveobsclin=obs_clin obsclin_codes;
...
*- Derive categorical variable -*;
data data;
     length obsclincode col1 $50;
     set obs_clin(keep=obsclin_code obsclin_result_unit);
     if obsclin_code^=" ";
    
     if obsclin_code^=" " then obsclincode=obsclin_code;
     else if obsclin_code=" " then obsclincode="ZZZA";
    
     if obsclin_result_unit^=" " then col1=obsclin_result_unit;
     else if obsclin_result_unit=" " then col1="ZZZA";
    
     keep obsclincode col1;
run;

*- Derive statistics -*;
proc means data=data nway noprint missing;
     class obsclincode col1;
     output out=stats;
     format obsclincode col1 $nullout.;
run;

*- Derive appropriate counts and variables -*;
data query;
     length record_n $20 obsclin_result_unit obsclin_code $50;
     set stats;
     ...
     * table values *;
     obsclin_result_unit=put(col1,$nullout.);
     obsclin_code=put(obsclincode,$nullout.);
    
     * apply threshold *;
     %threshold(nullval=col1^="ZZZD" and obsclin_code^="ZZZD")

     * counts *;
     record_n=strip(put(_freq_,threshold.));

     keep datamartid response_date query_package obsclin_code 

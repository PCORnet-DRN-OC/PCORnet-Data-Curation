...
data tbl(keep=ord: col:);
     ...
     %stat(dcn=COND_L3_STATUS,ord=12,ord2=47,var=CONDITION_STATUS)
     %stat(dcn=COND_L3_TYPE,ord=12,ord2=48,var=CONDITION_TYPE)
     %stat(dcn=COND_L3_SOURCE,ord=12,ord2=49,var=CONDITION_SOURCE)
     %stat(dcn=DEATHC_L3_CODE,ord=13,ord2=50,var=DEATH_CAUSE_CODE)
     %stat(dcn=DEATHC_L3_SOURCE,ord=13,ord2=51,var=DEATH_CAUSE_SOURCE)
     %stat(dcn=DEATHC_L3_TYPE,ord=13,ord2=52,var=DEATH_CAUSE_TYPE)
     %stat(dcn=DEATHC_L3_CONF,ord=13,ord2=53,var=DEATH_CAUSE_CONFIDENCE)
     %stat(dcn=PROCM_L3_ITEM,ord=14,ord2=54,var=PRO_ITEM)
     %stat(dcn=PROCM_L3_METHOD,ord=14,ord2=55,var=PRO_METHOD)
     %stat(dcn=PROCM_L3_MODE,ord=14,ord2=56,var=PRO_MODE)
     %stat(dcn=PROCM_L3_CAT,ord=14,ord2=57,var=PRO_CAT)
run;
...
data numerators;
     ...
     if (dc_name in ("VIT_L3_VITAL_SOURCE" "DEATH_L3_IMPUTE" "DEATH_L3_MATCH" 
                     ...
                     "LAB_L3_NAME_RUNIT" "LAB_L3_LOW" "LAB_L3_HIGH" "LAB_L3_ABN" 
     ...
     if dc_name in ("LAB_L3_NAME_RUNIT" "LAB_L3_PX_PXTYPE") then var=cross_variable;
...
data print_ive(keep=col: perc: thresholdflag: exceptionflag);
     ...
     if (compress(col2) in ("0" "BT")) and resultn>0 then exceptionflag=1;
     if resultn^=.t then col3=strip(put(resultn,comma16.));
     else col3=strip(put(resultn,threshold.));

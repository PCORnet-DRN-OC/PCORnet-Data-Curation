...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package V4.12 - 
...
*                (<DataMart Id>_<response date>_data_curation_query_xtbl.pdf)
...
%clean(savedsn=&savedeath &savedemog &saveenc &savediag &saveproc &saveenr &savevit 
               ...
               med_admin_medadmin_stop_date obs_clin_obsclin_date);
...
data dmlocal.xtbl_l3_metadata;
     set dmlocal.xtbl_l3_metadata
         dmlocal.elapsed_xtbl(in=e where=(query="DC PROGRAM - XTBL"))
         %if %upcase(&_grp)=ALL %then %do;
             dmlocal.elapsed_main(in=e where=(query="DC PROGRAM - MAIN"))
             dmlocal.elapsed_lab(in=e where=(query="DC PROGRAM - LAB"))
         %end;
     ;
     if e then do;
...
proc sql noprint;
     select unique memname into :workdata separated by ' '  from dsn
         where substr(upcase(memname),1,8)="XTBL_L3_" or
...
title "Data Curation query run times - XTBL";
footnote;
proc print width=min data=dmlocal.elapsed_xtbl label;
...
* Close PDF
*******************************************************************************;
%if %upcase(&_grp)=XTBL %then %do; ods pdf close; %end;
%else %if %upcase(&_grp)=ALL %then %do; ods pdf close; %end;

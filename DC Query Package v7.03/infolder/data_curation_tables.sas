...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package v7.03 - all queries
* 
...
proc sql noprint;
	...
	select max(length) into :maxlength_pat from sashelp.vcolumn 
		where upcase(libname)="PCORDATA" and upcase(name)="PATID";
...
%if &_yharvest=1 and %upcase(&_part1)=YES %then %do;
    ...
    * Identify session encoding;
    %let encoding = %sysfunc(getOption(ENCODING));
	...
	* Bring everything together;
	data data;    
		length networkid network_name datamartid datamart_name query_package tag 
			...
			vx_record_date_mgmt vx_admin_date_mgmt vx_exp_date_mgmt session_encoding $50 
		...
		* Table variables;
		...
		response_date=put("&sysdate9"d,yymmdd10.);
		...
		array v networkid network_name datamartid datamart_name datamart_platform cdm_version
	...
	* Normalize data;
	proc transpose data=data out=query(rename=(_name_=name col1=value));
		var networkid network_name datamartid datamart_name datamart_platform
			...
			sas_odbc sas_oracle sas_postgres sas_sql sas_teradata datastore session_encoding;
	run;
	...
	* Delete intermediary datasets;
	...
	%elapsed(end);
		...
		*******************************************************************************;
			...
			* Delete intermediary datasets;
			proc delete data=hash_l3_n_temp;
			quit;
            ...
            * Drop RECORD_N, add DATASET, and exclude NULL or missing rows;
            data hash_l3_token_availability_temp;
			...
			data dmlocal.hash_l3_token_availability;
				set hash_l3_token_availability_temp;
			run;	

			* Delete intermediary dataset and HASH_TOKEN dataset;
		    proc delete data=hash_l3_token_availability_temp;
		    run; 
...
%if &_yldsadrs=1 %then %do;
    ...
    data
        ...
        * Flag valid values;
        ...
        if notdigit(strip(county_fips))=0 and length(county_fips)=5 and substr(strip(county_fips),1,2)=strip(state_fips) then valid_county_fips='Y';
        if notdigit(strip(state_fips))=0 and length(state_fips)=2 then valid_state_fips='Y';
        if notdigit(strip(ruca_zip))=0 and 1<=length(ruca_zip)<=2 and not missing(ruca_zip)then valid_ruca_zip='Y';
...
%if &_ypro_cm=1 %then %do;
	...
	data pro_cm (compress=yes);
        set pcordata.pro_cm (keep=patid pro_date pro_code pro_type
            %if %upcase(&_part1)=YES %then %do;
                encounterid pro_time pro_cat pro_method pro_mode pro_cm_id pro_source
            %end;
            %if %upcase(&_part2)=YES %then %do;
                pro_fullname pro_name
            %end;
            ) end=eof;
...
%if &_yvital=1 %then %do;
	...
	data vital (compress=yes);
        set pcordata.vital (keep=patid measure_date encounterid
            %if %upcase(&_part1)=YES %then %do;
                measure_time bp_position vitalid smoking tobacco tobacco_type vital_source
            %end;
            %if %upcase(&_part2)=YES %then %do;
...
%if &_yencounter=1 %then %do;
	...
	data
        ...
        %if %upcase(&_part2)=YES %then %do;                
            encounterid (compress=yes keep=patid encounterid enc_type) /*To be sorted and used for queries combining other CDM dataset to ENCOUNTER by ENCOUNTERID*/
        %end;
        %if %upcase(&_part1)=YES %then %do;    
...
%if &_ycondition=1 %then %do;
	...
	data condition (compress=yes);
        set pcordata.condition (keep=patid report_date condition condition_type
            %if %upcase(&_part1)=YES %then %do;
                encounterid onset_date resolve_date conditionid condition_source condition_status
            %end;
            ) end=eof;
...
%if &_ydiagnosis=1 %then %do;
	...
	data diagnosis (compress=yes);
        set pcordata.diagnosis (keep=patid admit_date dx_date encounterid dx dx_type enc_type pdx dx_source
            %if %upcase(&_part1)=YES %then %do;
                dx_poa diagnosisid providerid
            %end;
            %if %upcase(&_part2)=YES %then %do;
                dx_origin
            %end;
		...
		* Restrict data to within lookback period;
        ...
        %if %upcase(&_part2)=YES %then %do;
            ...
            if enc_type in ('IP' 'EI' 'IS' 'OS') and dx_source ne 'DI' then inpatient_di=1;
            else inpatient_di=0;
        %end;        
    run; 
...
%if &_yobs_clin=1 %then %do;
	...
	data obs_clin (compress=yes);
        set pcordata.obs_clin (keep=patid obsclin_start_date obsclin_result_unit obsclin_type obsclin_result_modifier obsclin_result_qual
            ...
            %end;
            %if %upcase(&_part2)=YES %then %do;
                obsclin_code obsclin_result_num obsclin_result_text obsclin_result_snomed
            %end;
            ) end=eof;
...
%if %upcase(&_part1)=YES %then %do;
    ...
    *- Macro to compare dates against birth and death -*;
    ...
    %end;
    %if &_ymed_admin=1 %then %do;
        proc sort data=med_admin (keep=patid medadmin_start_date medadmin_stop_date) out=med_admin_dl;
             by patid; 
        run;
    ...
    *- Macro to compare dates within a data table -*;
    %macro within(idsn,var,var2,adjp,adjm,ord);
        ...
        %else %if &idsn=external_meds_dl %then %do;
            data extmeds&var;
        %end;
        %else %if &idsn=med_admin_dl %then %do;
            data medadm&var;
        %end;
        %else %do;
            data &idsn&var;
    ...
    %end;
    %if &_ymed_admin=1 %then %do;
        %within(idsn=med_admin_dl,var=medadmin_stop_date,var2=medadmin_start_date,ord=35)
    %end;
    ...
    *- Macro to compare dates across data tables -*;
    ...
    data query;
        ...
        set data (in=b rename=(b_cnt=distinct_patid))
            ...
            summedadmmedadmin_stop_date (in=md rename=(sum_within=distinct_patid))
            ;
         ...
         * Create comparision variable;
         ...
         else if md then date_comparison="MEDADMIN_START_DATE > MEDADMIN_STOP_DATE";
	...
	* Delete intermediary datasets;
	proc delete data=data sumproceduresdischarge_date sumprocedures_dlpx_date sumencounter_dldischarge_date sumdiagnosis_dldx_date sumdiagnosisdischarge_date
            sumobs_clin_dlobsclin_stop_date sumobs_gen_dlobsgen_stop_date sumpatrelrelationship_end sumextmedsext_pat_end_date summedadmmedadmin_stop_date query;
	quit;
...
%if %upcase(&_part1)=YES %then %do;
    ...
    * Orphan ENCOUNTERID records from each appropriate table ;
    proc sql noprint;
         ...
         create table orphan_encid_patid as select unique patid, encounterid, enc_type, admit_date
            from pcordata.encounter
            where encounterid^=" " and patid^=" "
    ...
    proc datasets library=work nolist;
        modify orphan_provid;
        index create providerid;
    quit; 
    ...
    %macro orph_enc;
            ...
            %if &_orph=DIAGNOSIS or &_orph=PROCEDURES or &_orph=VITAL or 
                ...
                * Mismatch rows - ENCOUNTER;
                %if &_orph=DIAGNOSIS or &_orph=PROCEDURES %then %do;
                    ...
                    proc sort data=&_orph (keep=patid encounterid enc_type admit_date) out=mismatch_&_orph;
                         by patid encounterid;
                         where encounterid^=" " and patid^=" ";
                    run;
                    ...
                    data mismatch_&_orph(keep=memname ord mis:);
                         ...
                         merge
                             ...
                             orphan_encid_patid (in=oe rename=(enc_type=etype admit_date=adate)) end=eof
                             ;
                         by patid encounterid;
        ...
        * Delete intermediary datasets;
        proc delete data=orphan_encid orphan_encid_patid orphan_provid;
        quit;
        ...
        %do op=1 %to &workdata_count;
            ...
            %if &_orphp=ENROLLMENT or &_orphp=ENCOUNTER or &_orphp=DIAGNOSIS or 
                ...
                data orph_&_orphp(keep=memname ord recordn patidnum);
                     length patid $ &maxlength_pat memname $32;
                     merge
                         orph_&_orphp(in=o) 
                ...
                * Compile into one dataset;
                %if &_orphp^=DEMOGRAPHIC or &nobs_hash_token>0 %then %do;
...
%if %upcase(&_part1)=YES %then %do;
    ...
    %macro _dates(dsn,_vn,dtvlist,tmvlist1,tmvlist);
      ...
      %else %do;
            data &dsn._dates;
      %end;
            length futureflag1-futureflag&_vn 3.;
            ...
            keep &dtvlist time: futureflag: invdtflag: invtmflag:;
        run;
    ...
    * Order variables;
    proc sql;
         create table dmlocal.&qname as select
             ...
             median, p95, max, n, nmiss, future_dt_n, invalid_n
         from query;
    quit;
		...
		%if %upcase(&_part2)=YES %then %do;
			...
			* Start timer outside of macro in order to capture derivation processing time;
            ...
            data obs_clin_loinc (compress=yes);
                set obs_clin (keep=patid obsclin_code obsclin_type obsclin_result_num where=(obsclin_type='LC')) end=eof;
            ...
            * Delete intermediary dataset;
            proc delete data=obsclin_l3_loinc_length;
            quit;
            ...
            %t_cont(dset=OBS_CLIN_LOINC, var=OBSCLIN_RESULT_NUM, qnam=obsclin_l3_loinc_result_num, byvar=OBSCLIN_CODE, stats=min p1 p5 p25 median p75 p95 p99 max n nmiss);
			...
			%n_pct_noref(dset=OBS_CLIN, var=OBSCLIN_RESULT_SNOMED, qnam=obsclin_l3_result_snomed, distinct=y);
            ...
            * Delete intermediary dataset;
            proc delete data=obs_clin_loinc;
            quit;

		%end; * End of Part 2 OBS_CLIN queries;

	* Delete intermediary and OBS_CLIN datasets;
...
%if &_ycondition=1 %then %do;
		...
		*******************************************************************************;
		%if %upcase(&_part2)=YES %then %do;
			...
			%n_pct_multilev(dset=CONDITION, var1=CONDITION, var2=CONDITION_TYPE, ref2=valuesets, qnam=cond_l3_cond_condtype, distinct=y, nopct=n, observedonly=y);
		...
		*******************************************************************************;
		%if %upcase(&_part2)=YES %then %do;
			...
			* Start timer outside of macro in order to capture derivation processing time;
            ...
            %let nobs_pro_cm_loinc = 0;

            data pro_cm_loinc (compress=yes);
                set pro_cm (keep=patid pro_code pro_type where=(pro_type='LC')) end=eof;

                * Need macro value for # of observations in this input dataset for use in macro;
                if eof then call symputx('nobs_pro_cm_loinc',_n_);
                
                drop pro_type;              
            run;         

            %n_pct_noref(dset=PRO_CM_LOINC, var=PRO_CODE, qnam=procm_l3_loinc, distinct=y, timer=n);
            ...
            * Delete intermediary dataset;
            proc delete data=pro_cm_loinc procm_l3_loinc procm_l3_loinc_length;
            quit;
...
%if &_yldsadrs=1 %then %do;
		...
		*******************************************************************************;
		%if %upcase(&_part1)=YES %then %do;
			...
			%tag(dset=LDS_ADDRESS_HISTORY,  varlist=PATID ADDRESSID ADDRESS_ZIP5 ADDRESS_ZIP9 ADDRESS_COUNTY RUCA_ZIP COUNTY_FIPS STATE_FIPS, qnam=ldsadrs_l3_n, valid1=3, valid2=4, valid3=6, valid4=7, valid5=8);
			%n_pct(dset=LDS_ADDRESS_HISTORY, var=CURRENT_ADDRESS_FLAG,      qnam=ldsadrs_l3_current, distinct=y); 
...
%if &_yexternal_meds=1 %then %do;
		...
		*******************************************************************************;
		%if %upcase(&_part2)=YES %then %do;
			...
			%n_pct_noref(dset=EXTERNAL_MEDS, var=EXT_RECORD_DATE_Y,   qnam=extmed_l3_rdate_y, varlbl=EXT_RECORD_DATE, distinct=y);proc print data=dmlocal.extmed_l3_rdate_y;
			%n_pct_noref(dset=EXTERNAL_MEDS, var=EXT_RECORD_DATE_YM,  qnam=extmed_l3_rdate_ym, varlbl=EXT_RECORD_DATE, nopct=y, distinct=y);proc print data=dmlocal.extmed_l3_rdate_ym;
		...
		%if %upcase(&_part1)=YES %then %do;
            ...
            * Delete RECORD_N and RECORD_PCT columns and NULL or missing row from this query;
			...
			data dmlocal.enc_l3_dash2;
				set enc_l3_dash2_temp;
			run;
         
            * Delete intermediary datasets;
			proc delete data=dash_ref enc_l3_dash2_temp
            ...
            * Reorder columns;       
            ...
            retain datamartid response_date query_package facility_type_grp facility_type record_n record_pct distinct_patid_n;
                set enc_l3_facilitytype_temp;
            run;
         
            * Delete intermediary datasets;
			proc delete data=facility_type facility_type_grp enc_l3_facilitytype_temp;
            quit;
            ...
            * Reorder columns;       
            ...
            retain datamartid response_date query_package payer_type_primary_grp payer_type_primary record_n record_pct distinct_patid_n;
                set enc_l3_payertype1_temp;
            run;
            
            * Delete intermediary datasets;
			proc delete data=payer_type payer_type_primary_grp enc_l3_payertype1_temp;
            quit;
            ...
            * Reorder columns;       
            ...
            retain datamartid response_date query_package payer_type_secondary_grp payer_type_secondary record_n record_pct distinct_patid_n;
                set enc_l3_payertype2_temp;
            run;
            
            * Delete intermediary datasets;
			proc delete data=enc_l3_payertype2_temp;
            quit;
		...
		%if %upcase(&_part2)=YES %then %do;
            ...
            * Drop macro-created NULL or missing row since it does not apply to this variable;
			...
			data dmlocal.enc_l3_los_dist;
				set enc_l3_los_dist_temp;
			run;
           
            * Delete intermediary datasets;
			proc delete data=encounter_los los_enc_type_ref los_group_ref enc_l3_los_dist_temp;
            quit;
            ...
            %if &_yldsadrs=1 %then %do;  
                ...
                * Subset to only subjects within 1yr dash period;
                data zipc_1y (compress=yes);
                    merge
                        lds_address_history_f2f_current  (in=inl keep=patid valid_address_zip5 address_zip5
                            where=(valid_address_zip5='Y' or missing(address_zip5)))
                        encounter_period (in=ine where=(period='1 yr'))
                        ;
                    by patid;
                    if inl and ine;
                ...
                * Subset to only subjects within 5yr dash period;
                data countyfips_5y (compress=yes);
                    merge
                        lds_address_history_f2f_current  (in=inl keep=patid valid_county_fips county_fips 
                            where=(valid_county_fips='Y' or missing(county_fips)))
                        encounter_period (in=ine where=(period='5 yrs'))
                        ;
                    by patid;
                    if inl and ine;
                ...
                * Subset to only subjects within 5yr dash period;
                data statefips_5y (compress=yes);
                    merge
                        lds_address_history_f2f_current  (in=inl keep=patid valid_state_fips state_fips
                            where=(valid_state_fips='Y' or missing(state_fips)))
                        encounter_period (in=ine where=(period='5 yrs'))
                        ;
                    by patid;
                    if inl and ine;
                ...
                * Subset to only subjects within 5yr dash period;
                data rucac_5y (compress=yes);
                    merge
                        lds_address_history_f2f_current  (in=inl keep=patid valid_ruca_zip ruca_zip 
                            where=(valid_ruca_zip='Y' or missing(ruca_zip)))
                        encounter_period (in=ine where=(period='5 yrs'))
                        ;
                    by patid;
                    if inl and ine;
                ...
                * Start timer outside of macro in order to capture derivation processing time;
                %let qname=xtbl_l3_cfipszip_5y;
                %elapsed(begin);
                
                %let nobs_statefips_5y = 0;

                * Subset to only subjects within 5yr dash period;
                data cfipszip_5y (compress=yes);
                    merge
                        lds_address_history_f2f_current  (in=inl keep=patid valid_address_zip5 valid_county_fips
                            where=(valid_address_zip5='Y' or valid_county_fips='Y'))
                        encounter_period (in=ine where=(period='5 yrs'))
                        ;
                    by patid;
                    if inl and ine;
                    ...
                    * Create retained version of flags for reduction of one record per PATID;
                    retain address_zip5 county_fips both;
                    ...
                    if valid_address_zip5='Y' then address_zip5=1;
                    if valid_county_fips='Y' then county_fips=1;
                    if valid_address_zip5='Y' and valid_county_fips='Y' then both=1;

                    * Reduce dataset down to one record per per PATID;
                    if last.patid;

                    keep patients address_zip5 county_fips both;
                run;

                * Need macro value for # of observations in this input dataset for use in macro;
                data _null_;
                    set cfipszip_5y end=eof;
                    
                    if eof then call symputx('nobs_cfipszip_5y',_n_);
                run; 

                %tag(dset=CFIPSZIP_5Y, varlist=PATIENTS ADDRESS_ZIP5 COUNTY_FIPS BOTH, qnam=xtbl_l3_cfipszip_5y, timer=n);

                * Post-processing to rename row/column and keep only desired columns;
                data dmlocal.xtbl_l3_cfipszip_5y;
                    set dmlocal.xtbl_l3_cfipszip_5y (drop=distinct_n null_n);

                    if tag='BOTH' then tag='ADDRESS_ZIP5 and COUNTY_FIPS';

                    rename all_n=distinct_patid_n;
                run;                    

                * Delete intermediary datasets;
                proc delete data=cfipszip_5y lds_address_history_f2f_current encounter_period;
                quit;
                
                %elapsed(end);

                /***End of XTBL_L3_CFIPSZIP_5Y***/ 

            %end;
			/*End of ENCOUNTER queries requiring LDS_ADDRESS_HISTORY*/
...
%if &_yvital=1 %then %do;
		...
		*******************************************************************************;
		%if %upcase(&_part2)=YES %then %do;  
			...
			*******************************************************************************;
			* VITAL and ENCOUNTER combined queries
			*******************************************************************************; 
			%if &_yencounter=1 %then %do;				                  
				...
				* Start timer outside of macro in order to capture derivation processing time;
				%let qname=xtbl_l3_vit_enctype;
				%elapsed(begin);
				
				proc sort data=vital (keep=patid encounterid) out=vit_enctype;
					by patid encounterid;
				run;

				data vit_enctype (compress=yes);				
					length encounterid $&maxlength_enc;
					merge
						vit_enctype (in=inp)
						encounterid (in=ine keep=patid encounterid enc_type)
						;
					by patid encounterid;
					if inp and ine;
				run;
				...
				* Create reference file for PRES_ENCTYPE based on same versions used for ENCOUNTER summary;
				data vit_enctype_valuesets;
					set valuesets (where=(table_name='ENCOUNTER'));
					
					table_name='VIT_ENCTYPE';
				run;

				%n_pct(dset=VIT_ENCTYPE, var=ENC_TYPE, qnam=xtbl_l3_vit_enctype, ref=vit_enctype_valuesets, distinct=y, timer=n);

				* Delete intermediary and ENCOUNTER datasets;
				proc delete data=vit_enctype vit_enctype_valuesets encounter;
                quit;

                %elapsed(end);

            %end; 
            /*End of VITAL queries requiring ENCOUNTER*/     
            ...
            * Delete RECORD_N and RECORD_PCT columns and NULL or missing row from this query;
            ...
            data dmlocal.vit_l3_dash1;
                set vit_l3_dash1_temp;
            run;
                    
            * Delete intermediary datasets;
            proc delete data=dash_ref vit_l3_dash1_temp;
            quit;
			...
			%if &_yvital=1 %then %do;	
                ...
                * Delete RECORD_N and RECORD_PCT columns and NULL or missing row from this query;
				...
				data dmlocal.xtbl_l3_dash1;
					set xtbl_l3_dash1_temp;
				run;
               
				* Delete intermediary datasets;
				proc delete data=diagnosis_period vital_period xtbl_l3_dash1_temp;
                quit;
		...
		*******************************************************************************;
		%if %upcase(&_part2)=YES %then %do;
            ...
            %n_pct_multilev(dset=DIAGNOSIS, var1=PDX, var2=ENC_TYPE, var3=DX_ORIGIN, ref1=valuesets, ref2=valuesets, ref3=valuesets, qnam=dia_l3_pdx_enctype, distinct=y, distinctenc=y, 
                subvar=inpatient_di, subset=%str(inpatient_di=0));			       
            ...
            * Derive PDXGRP - One-per-subject flag identifying if the ENCOUNTERID ever had a records with PDX=P;
            data pdxgrp (compress=yes);
                set diagnosis (keep=encounterid pdx inpatient_di where=(inpatient_di=0));
                by encounterid;
            ...
            * Drop RECORD_N and PDXGRP=NULL or missing rows that come out of macro for this special case;
			...
			data dmlocal.dia_l3_pdxgrp_enctype;
				set dia_l3_pdxgrp_enctype_temp;
			run;

            * Delete intermediary and DIAGNOSIS datasets;
            proc delete data=pdxgrp diagnosis pdxgrp_valueset dia_l3_pdxgrp_enctype_temp;
            quit;
			...
			%if &_yvital=1 and &_ydiagnosis=1 and &_yprescribing and &_ymed_admin=1 %then %do;	
                ...
                * Delete RECORD_N and RECORD_PCT columns and NULL or missing row from this query;
				...
				data dmlocal.xtbl_l3_dash2;
					set xtbl_l3_dash2_temp;
				run;
             
				* Delete intermediary datasets;
				proc delete data=dash1 med_admin_period prescribing_period xtbl_l3_dash2_temp;
                quit;
			...
			%if &_yvital=1 and &_ydiagnosis=1 and &_yprescribing and &_ymed_admin=1 and &_ylab_result_cm %then %do;	
                ...
                * Delete RECORD_N and RECORD_PCT columns and NULL or missing row from this query;
				...
				data dmlocal.xtbl_l3_dash3;
					set xtbl_l3_dash3_temp;
				run;
             
				* Delete intermediary datasets;
				proc delete data=dash2 dash3 lab_result_cm_period dash_ref xtbl_l3_dash3_temp;
                quit;
		...
		*******************************************************************************;
		%if %upcase(&_part2)=YES %then %do;
            ...
            * Reorder columns;       
            ...
            retain datamartid response_date query_package rxnorm_cui rxnorm_cui_tty record_n record_pct distinct_patid_n;
                set pres_l3_rxcui_temp;
            run;

			* Delete intermediary datasets;
			proc delete data=pres_l3_rxcui_temp;
            quit;

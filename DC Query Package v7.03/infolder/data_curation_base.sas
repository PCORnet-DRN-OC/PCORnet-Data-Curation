...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package v7.03 - 
...
data _null_;
     ...
     call symput("tday",left(put("&sysdate9"d,yymmddn8.)));
...
%macro tag(dset=,varlist=,qnam=,timer=,subset=,validall=,valid1=,valid2=,valid3=,valid4=,valid5=,distinct=y,distvalid=,null=y);
    ...
    %if &&nobs_&dset>0 %then %do;
        ...
        %do i=1 %to &numvars;
            ...
            proc sort data=&dset (keep=&var
                ...
                %if &valid1=&i or &valid2=&i or &valid3=&i or &valid4=&i or &valid5=&i or %upcase(&validall)=Y %then %do; 
                    valid_&var
                %end;
                ) out=invar_sorted;
                by &var               
...
%macro n_pct(dset=,var=,qnam=,subset=,ref=valuesets,cdmref=y,timer=,alphasort=y,dec=1,distinct=,distinctvis=,distinct1=,distinct2=,elig=);
    ...
    * Determine length of VALUESET_ITEM;            
    proc contents data=&ref (where=(table_name="&dset" and field_name="&var")) noprint out=valueset_item_length (keep=name length where=(name='VALUESET_ITEM'));
    run;

    data _null_;
        set valueset_item_length;

        call symputx('valueset_item_length',length);

        * Add 10 extra characters to the length for the label to account for the addition of the ordering, or $42 to account for "Values outside of CDM", whichever is larger;
        call symputx('valueset_item_lab_length',max(length+10,42));
    run;

    * Delete intermediary dataset;
    proc delete data=valueset_item_length;
    run; 
   
    * Bring in CDM value set for variable;
    data &var._vals;
        length valueset_item $&valueset_item_length label $&valueset_item_lab_length;
    ...
    * Only do the proc means if input dataset is not empty;
    %if &subset_obs>0 %then %do;
        ...
        * If any extra counts requested, cut down on any extra length on summary variable;
        %if %upcase(&distinct)=Y or %upcase(&distinct1)=Y or %upcase(&distinct2)=Y or %upcase(&elig)=Y %then %do;
            ...
            data &dset._subset (compress=yes);
                set &dset (keep=&var
                    %if %upcase(&distinct)=Y %then %do;
                        patid
                    %end;
                    %if %upcase(&distinct1)=Y %then %do;
                        patid_1
                    %end;
                    %if %upcase(&distinct2)=Y %then %do;
                        patid_2
                    %end;
        ...
        * Count distinct PATID_1s within group;
        %if %upcase(&distinct1)=Y %then %do;
            
            * Reduce input data down to one record per summary variable type per PATID_1;
            proc sort data=&dset._subset (keep=patid_1 &var) out=&qnam._oneper1 (drop=patid_1) nodupkey;
                by &var patid_1;
            run;
    
            * Get counts by summary variable;
            proc means data=&qnam._oneper1 noprint missing;
                class &var;
                output out=stats_oneper1;
            run;

            data stats;
                merge
                    stats
                    stats_oneper1 (keep=&var _type_ _freq_ where=(_type_patid_1=1) rename=(_type_=_type_patid_1 _freq_=_freq_patid_1))
                    ;
                by &var;
        ...
        * Count distinct PATID_2s within group;
        %if %upcase(&distinct2)=Y %then %do;
            
            * Reduce input data down to one record per summary variable type per PATID_2;
            proc sort data=&dset._subset (keep=patid_2 &var) out=&qnam._oneper2 (drop=patid_2) nodupkey;
                by &var patid_2;
            run;
    
            * Get counts by summary variable;
            proc means data=&qnam._oneper2 noprint missing;
                class &var;
                output out=stats_oneper2;
            run;

            data stats;
                merge
                    stats
                    stats_oneper2 (keep=&var _type_ _freq_ where=(_type_patid_2=1) rename=(_type_=_type_patid_2 _freq_=_freq_patid_2))
                    ;
                by &var;
        ...
        * Count eligible records within group;
        %if %upcase(&elig)=Y %then %do;
            ...
            * Reduce input data down to one record per summary variable type per identifiers that deem a record eligible;
        ...
        * Create final variables and sort order;
        data &qnam;        
            ...
            * Sort alphabetically by default, use custom sort (set in custom reference file) if otherwise specified with empty alphasort parameter;
            ...
            %if %upcase(&distinct1)=Y %then %do;
                * Fill in zero for missing category if needed;
                if _freq_patid_1=. then _freq_patid_1=0;
                
                length distinct_patid_1_n $20;
                
                distinct_patid_1_n=strip(put(_freq_patid_1,12.));
            ...
            %if %upcase(&distinct2)=Y %then %do;
                * Fill in zero for missing category if needed;
                if _freq_patid_2=. then _freq_patid_2=0;
                
                length distinct_patid_2_n $20;
                
                distinct_patid_2_n=strip(put(_freq_patid_2,12.));
        ...
        * Delete intermediary datasets;
        proc delete data=stats
                ...
                %end;
                %if %upcase(&distinct1)=Y %then %do;
                    &qnam._oneper1 stats_oneper1
                %end;
                %if %upcase(&distinct2)=Y %then %do;
                    &qnam._oneper2 stats_oneper2
                %end;
                %if %upcase(&distinctvis)=Y %then %do;
        ...
        data &qnam;
            ...
            * Sort alphabetically by default, use custom sort (set in custom reference file) if otherwise specified with empty alphasort parameter;
            ...
            %if %upcase(&distinct1)=Y %then %do;
                ...
                distinct_patid_1_n='0';
            %end;

            %if %upcase(&distinct2)=Y %then %do;
                length distinct_patid_2_n $20;
                
                distinct_patid_2_n='0';
            %end;
    ...
    %do i=1 %to &numvars;
        ...
        * Process value set for variable from reference file, if applicable;
        %if &&ref&i ne %then %do;     
            ...
            * Determine length of VALUESET_ITEM;            
            proc contents data=&&ref&i (where=(table_name="&dset" and field_name="&&var&i")) noprint out=valueset_item_length (keep=name length where=(name='VALUESET_ITEM'));
            run;
...
%macro n_pct_noref(dset=,var=,qnam=,timer=,dec=1,nopct=,varlbl=,distinct=,distinct1=,distinct2=,distinctenc=);
    ...
    %if &&nobs_&dset>0 %then %do;
        ...
        * Merge missing template onto counts;
        ...
        %if %upcase(&distinct)=Y %then %do;
            ...
            * Reduce input data down to one record per summary variable type per patid;
            proc sort data=&dset (keep=patid &var) out=&qnam._oneper (drop=patid) nodupkey;
                by &var patid;
            run;
        ...
        %if %upcase(&distinct1)=Y %then %do;
            ...
            * Reduce input data down to one record per summary variable type per patid;
            proc sort data=&dset (keep=patid_1 &var) out=&qnam._oneper1 (drop=patid_1) nodupkey;
                by &var patid_1;
            run;
            ...
            * Get counts by summary variable;
            proc means data=&qnam._oneper1 noprint missing;
                class &var;
                output out=&qnam._oneper1;
            run;

            data &qnam;
                merge
                    &qnam
                    &qnam._oneper1 (keep=&var _type_ _freq_ where=(_type_patid_1=1) rename=(_type_=_type_patid_1 _freq_=_freq_patid_1))
                    ;
                by &var;
        ...
        %if %upcase(&distinct2)=Y %then %do;
            ...
            * Reduce input data down to one record per summary variable type per patid;
            proc sort data=&dset (keep=patid_2 &var) out=&qnam._oneper2 (drop=patid_2) nodupkey;
                by &var patid_2;
            run;
            ...
            * Get counts by summary variable;
            proc means data=&qnam._oneper2 noprint missing;
                class &var;
                output out=&qnam._oneper2;
            run;

            data &qnam;
                merge
                    &qnam
                    &qnam._oneper2 (keep=&var _type_ _freq_ where=(_type_patid_2=1) rename=(_type_=_type_patid_2 _freq_=_freq_patid_2))
                    ;
                by &var;
        ...
        %if %upcase(&distinctenc)=Y %then %do;
            ...
            * Reduce input data down to one record per summary variable type per encounter id;
            proc sort data=&dset (keep=encounterid &var) out=&qnam._oneperenc (drop=encounterid) nodupkey;
                by &var encounterid;
            run;
        ...
        * If it is a year/month summary, super-populate all year/months between the first and last available year/months;
        %if %index(%upcase(&qnam),_YM)>0 %then %do;
            ...
            data &qnam;
                ...
                %if %upcase(&distinct1)=Y %then %do;
                    if distinct_patid_1_n='' then distinct_patid_1_n='0';
                %end;

                %if %upcase(&distinct2)=Y %then %do;
                    if distinct_patid_2_n='' then distinct_patid_2_n='0';
                %end;
...
%macro minmax(idsn,var,var_tm,_n);
     ...
     proc means data=&idsn._dates nway noprint;
          var &var time&_n futureflag&_n invdtflag&_n invtmflag&_n;
          output out=m&idsn 
                 n=ndt ntm dum1 dum2 dum3
                 sum=dum6 dum7 futuresum invdtsum invtmsum
                 min=mindt mintm dum8 dum9 dum10
                 max=maxdt maxtm dum13 dum14 dum15
                 p5=p5dt p5tm dum18 dum19 dum20
                 p95=p95dt p95tm dum23 dum24 dum25
                 mean=meandt meantm dum28 dum29 dum30
                 median=mediandt mediantm dum33 dum34 dum35
                 nmiss=nmissdt nmisstm dum38 dum39 dum40;
     run;
     ...
     data &idsn._&var;
          ...
          * counts *;
          ...
          keep datamartid response_date query_package dataset tag tag_tm 
               min max p5 p95 mean median n nmiss future_dt_n 
               invalid_n min_tm max_tm p5_tm p95_tm mean_tm median_tm
...
%macro prnt(pdsn=,_obs=max,dropvar=datamartid response_date query_package,_svar=,_suppvar=,_recordn=record_n,orient=portrait,origsort=n);
     ...
     * if not printing entire dataset, sort so that top frequencies are printed *;
     %if &_obs^=max %then %do;
        ...
        %if &origsort^=n %then %do;
            ...
            proc sort data=dmlocal.&pdsn out=&pdsn._all;
                by &_svar;
            run;
     ...
     %if &_obs^=max %then %do;
         %if &origsort^=n %then %do;
            ...
            proc print width=min data=&pdsn._print (drop=&dropvar obs=&_obs);
            run;

            proc delete data=&pdsn &pdsn._all &pdsn._print;
            run;
...
%global _ycondition _ydeath _ydeathc _ydemographic _ydiagnosis _ydispensing 
        ...
        _yprescribing _yprocedures _ypro_cm _yprovider _yvital _yptrial _ypat_relationship _yexternal_meds
...
proc sql noprint;
     ...
     select count(*) into :_ypat_relationship from pcordata
            where memname="PAT_RELATIONSHIP";
     select count(*) into :_yexternal_meds from pcordata
            where memname="EXTERNAL_MEDS";
quit;
...
%if %upcase(&_part2)=YES %then %do;
    ...
    * Determine length of LOINC code field in LOINC reference code for use in multiple queries;            
    proc contents data=loinc noprint out=loinc_num_length (keep=name length where=(name='LOINC_NUM') rename=(length=loinc_num_length));
    run;
...
data elapsed_order;
    ...
    array txt (332) $100 _temporary_ ('DC PROGRAM' 
	
		'DEATH' 'DEATH_L3_N' 'DEATH_L3_IMPUTE' 'DEATH_L3_SOURCE' 'DEATH_L3_MATCH' 'DEATH_L3_DATE_Y' 
        ...
        'ENC_L3_ENCTYPE_DISSTAT' 'ENC_L3_ENCTYPE_DRG' 'ENC_L3_FACILITYLOC' 'ENC_L3_FACILITYTYPE_FACILITYLOC' 
		
		'DEMOGRAPHIC' 'DEM_L3_N' 'DEM_L3_AGEYRSDIST1' 'DEM_OBS_L3_AGEYRSDIST1' 'DEM_L3_AGEYRSDIST2' 'DEM_OBS_L3_AGEYRSDIST2' 'DEM_L3_HISPDIST' 
		'DEM_OBS_L3_HISPDIST' 'DEM_L3_RACEDIST' 'DEM_OBS_L3_RACEDIST' 
        ...
        'DEM_OBS_L3_ORIENTDIST' 'DEM_L3_BIOBANK' 'DEM_L3_RACEETHMISS' 'DEM_L3_RACEETHAIAN' 'DEM_L3_RACEETHASIAN' 'DEM_L3_RACEETHBLACK' 'DEM_L3_RACEETHHISPANIC'
		'DEM_L3_RACEETHMENA' 'DEM_L3_RACEETHNHPI' 'DEM_L3_RACEETHWHITE'
		
		'DIAGNOSIS' 'DIA_L3_N' 'DIA_L3_DX' 'DIA_L3_DXPOA' 'DIA_L3_DXSOURCE' 'DIA_L3_DXTYPE_DXSOURCE' 'DIA_L3_PDX' 'DIA_L3_DXTYPE_ENCTYPE' 
        ...
        'DIA_L3_ORIGIN' 'DIA_L3_DXTYPE' 'DIA_L3_DXTYPE_ADATE_Y' 'DIA_L3_ENCTYPE_ADATE_YM' 
		
		'PROCEDURES' 'PRO_L3_N' 'PRO_L3_PX' 'PRO_L3_PXSOURCE' 'PRO_L3_PPX' 'PRO_L3_ENCTYPE' 'PRO_L3_PXTYPE_ENCTYPE' 'PRO_L3_ADATE_Y' 'PRO_L3_ADATE_YM' 
		'PRO_L3_PXDATE_Y' 'PRO_L3_PXTYPE' 'PRO_L3_PXTYPE_ADATE_Y' 'PRO_L3_ENCTYPE_ADATE_YM' 'PRO_L3_PX_PXTYPE' 
		
		'ENROLLMENT' 'ENR_L3_N' 'ENR_L3_BASEDIST' 'ENR_L3_CHART' 
		
		'VITAL' 'VIT_L3_N' 'VIT_L3_VITAL_SOURCE' 'VIT_L3_BP_POSITION_TYPE' 'VIT_L3_SMOKING' 'VIT_L3_TOBACCO' 'VIT_L3_TOBACCO_TYPE' 'VIT_L3_MDATE_Y' 
		'VIT_L3_MDATE_YM' 'VIT_L3_HT' 'VIT_L3_HT_DIST' 'VIT_L3_WT' 'VIT_L3_WT_DIST' 'VIT_L3_DIASTOLIC' 'VIT_L3_SYSTOLIC' 'VIT_L3_BMI' 'VIT_L3_DASH1' 
		
		'DISPENSING' 'DISP_L3_N' 'DISP_L3_DOSEUNIT' 'DISP_L3_ROUTE' 'DISP_L3_SOURCE' 'DISP_L3_NDC' 'DISP_L3_DDATE_Y' 'DISP_L3_DDATE_YM' 'DISP_L3_SUPDIST2' 
		'DISP_L3_DISPAMT_DIST' 'DISP_L3_DOSE_DIST' 
        ...
        'PRES_L3_RXQTY_DIST' 'PRES_L3_RXREFILL_DIST' 'PRES_L3_RXDOSEODR_DIST' 
		
		'MED_ADMIN' 'MEDADM_L3_N' 'MEDADM_L3_DOSEADMUNIT' 'MEDADM_L3_ROUTE' 'MEDADM_L3_SOURCE' 'MEDADM_L3_TYPE' 'MEDADM_L3_DOSEADM' 'MEDADM_L3_CODE_TYPE' 
		'MEDADM_L3_RXCUI_TIER' 'MEDADM_L3_SDATE_Y' 'MEDADM_L3_SDATE_YM' 
        ...
        'OBSGEN_L3_RECORDC' 'OBSGEN_L3_CODE_TYPE' 'OBSGEN_L3_SDATE_Y' 'OBSGEN_L3_SDATE_YM' 'OBSGEN_L3_CODE_UNIT' 
		
		'CONDITION' 'COND_L3_N' 'COND_L3_STATUS' 'COND_L3_TYPE' 'COND_L3_SOURCE' 'COND_L3_CONDITION' 'COND_L3_COND_CONDTYPE' 'COND_L3_RDATE_Y' 'COND_L3_RDATE_YM' 
		
		'PRO_CM' 'PROCM_L3_N' 'PROCM_L3_METHOD' 'PROCM_L3_MODE' 'PROCM_L3_CAT' 'PROCM_L3_TYPE' 'PROCM_L3_SOURCE' 'PROCM_L3_PDATE_Y' 'PROCM_L3_PDATE_YM' 
		'PROCM_L3_LOINC' 'PROCM_L3_FULLNAME' 'PROCM_L3_NAME' 'PROCM_L3_CODE_TYPE'
		
		'PROVIDER' 'PROV_L3_N' 'PROV_L3_NPIFLAG' 'PROV_L3_SPECIALTY' 'PROV_L3_SPECIALTY_GROUP' 'PROV_L3_SPECIALTY_ENCTYPE' 'PROV_L3_SEX' 
		
		'LDS_ADDRESS_HISTORY' 'LDSADRS_L3_N' 'LDSADRS_L3_ADRSUSE' 'LDSADRS_L3_ADRSTYPE' 'LDSADRS_L3_ADRSPREF' 'LDSADRS_L3_ADRSSTATE' 'LDSADRS_L3_ADRSCITY' 
		'LDSADRS_L3_ADRSZIP5' 'LDSADRS_L3_ADRSZIP9' 'LDSADRS_L3_CURRENT' 'LDSADRS_L3_STATEFIPS' 'LDSADRS_L3_COUNTYFIPS' 'LDSADRS_L3_RUCAZIP'
		
		'IMMUNIZATION' 'IMMUNE_L3_N' 'IMMUNE_L3_CODE_CODETYPE' 'IMMUNE_L3_STATUS' 'IMMUNE_L3_STATUSREASON' 'IMMUNE_L3_SOURCE' 'IMMUNE_L3_DOSEUNIT' 
		'IMMUNE_L3_ROUTE' 'IMMUNE_L3_BODYSITE' 'IMMUNE_L3_MANUFACTURER' 'IMMUNE_L3_RDATE_Y' 'IMMUNE_L3_RDATE_YM' 'IMMUNE_L3_ADATE_Y' 'IMMUNE_L3_ADATE_YM' 
		'IMMUNE_L3_CODETYPE' 'IMMUNE_L3_DOSE_DIST' 'IMMUNE_L3_LOTNUM' 
		
		'OBS_CLIN' 'OBSCLIN_L3_N' 'OBSCLIN_L3_MOD' 'OBSCLIN_L3_ABN' 'OBSCLIN_L3_QUAL' 'OBSCLIN_L3_RUNIT' 'OBSCLIN_L3_TYPE' 'OBSCLIN_L3_SOURCE' 
        'OBSCLIN_L3_RECORDC' 'OBSCLIN_L3_CODE_TYPE' 'OBSCLIN_L3_RESULT_SNOMED' 'OBSCLIN_L3_SDATE_Y' 'OBSCLIN_L3_SDATE_YM' 'OBSCLIN_L3_CODE_UNIT' 'OBSCLIN_L3_LOINC' 'OBSCLIN_L3_LOINC_RESULT_NUM'
		
		'LAB_RESULT_CM' 'LAB_L3_N' 'LAB_L3_SOURCE' 'LAB_L3_PRIORITY' 'LAB_L3_QUAL' 'LAB_L3_MOD' 'LAB_L3_LOW' 'LAB_L3_HIGH' 'LAB_L3_PX_TYPE' 'LAB_L3_ABN' 'LAB_L3_RSOURCE' 
        ...
        'LAB_L3_PX_PXTYPE' 'LAB_L3_LOINC_RESULT_NUM' 
		
		'LAB_HISTORY' 'LABHIST_L3_N' 'LABHIST_L3_LOINC' 'LABHIST_L3_SEXDIST' 'LABHIST_L3_RACEDIST' 'LABHIST_L3_UNIT' 
        'LABHIST_L3_LOW' 'LABHIST_L3_HIGH' 'LABHIST_L3_MIN_WKS' 'LABHIST_L3_MAX_WKS' 'LABHIST_L3_PDSTART_Y' 'LABHIST_L3_PDEND_Y' 
		
		'XTBL_L3_NON_UNIQUE' 'XTBL_L3_VIT_ENCTYPE' 'XTBL_L3_DATES' 'XTBL_L3_DATE_LOGIC' 'XTBL_L3_TIMES' 'XTBL_L3_MISMATCH' 'XTBL_L3_RACE_ENC' 'XTBL_L3_DASH1' 'XTBL_L3_DASH2' 'XTBL_L3_DASH3' 
        ...
        'XTBL_L3_ZIP5C_5Y' 'XTBL_L3_COUNTYFIPSC_5Y' 'XTBL_L3_STATEFIPSC_5Y' 'XTBL_L3_CFIPSZIP_5Y' 'XTBL_L3_RUCAC_5Y' 'XTBL_L3_ZIP5C_1Y' 
        
		'HASH_TOKEN' 'HASH_L3_N' 'HASH_L3_TOKEN_AVAILABILITY' 

        'PAT_RELATIONSHIP' 'PATREL_L3_N' 'PATREL_L3_TYPE' 'PATREL_L3_SDATE_Y' 'PATREL_L3_SDATE_YM'

        'EXTERNAL_MEDS' 'EXTMED_L3_N' 'EXTMED_L3_RDATE_Y' 'EXTMED_L3_RDATE_YM' 'EXTMED_L3_RXDOSE_DIST' 'EXTMED_L3_RXDOSEFORM' 'EXTMED_L3_RXDOSE_UNIT' 
        'EXTMED_L3_ROUTE' 'EXTMED_L3_RXCUI' 'EXTMED_L3_RXCUI_TIER' 'EXTMED_L3_BASIS' 'EXTMED_L3_SOURCE'
		
		'PCORNET_TRIAL' 'TRIAL_L3_N');

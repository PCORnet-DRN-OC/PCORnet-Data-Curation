...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet EDC report V7.03
* 
...
options nodate nonumber nobyline orientation=landscape validvarname=upcase 
        formchar="|___|||___+=|_/\<>" missing=' ' nomprint nomlogic nosymbolgen;
...
%macro ttl_ftn;
    data _null_;
         ...
         * Used to prevent log messages for empty footnotes;
         if footnote ne '' then call symputx("hasfn",1);
         else call symputx("hasfn",0);
    run;
...
********************************************************************************;
* GLOBALIZE MACRO VARIABLES
********************************************************************************;
%global ets_inst dm_ehr _anydispensingdata _anyhashdata _anylabdata _anymedadmindata 
        _anyobsclindata _anyobsgendata _anyprescribingdata _anyprocmdata 
        _anyvitaldata;

********************************************************************************;
* CREATE MACRO VARIABLES THAT CHECK ON CDM TABLE COUNTS
********************************************************************************;
proc sql noprint;
     select count(unique dc_name) into :_anydispensingdata from dc_normal
        where datamartid=%upcase("&dmid") and scan(dc_name,1,'_')="DISP";
     select count(unique dc_name) into :_anyhashdata from dc_normal
        where datamartid=%upcase("&dmid") and scan(dc_name,1,'_')="HASH";
     select count(unique dc_name) into :_anymedadmindata from dc_normal
        where datamartid=%upcase("&dmid") and scan(dc_name,1,'_')="MEDADM";
     select count(unique dc_name) into :_anyobsgendata from dc_normal
        where datamartid=%upcase("&dmid") and scan(dc_name,1,'_')="OBSGEN";
     select count(unique dc_name) into :_anyprescribingdata from dc_normal
        where datamartid=%upcase("&dmid") and scan(dc_name,1,'_')="PRES";
     select count(unique dc_name) into :_anyprocmdata from dc_normal
        where datamartid=%upcase("&dmid") and scan(dc_name,1,'_')="PROCM";
     select count(unique dc_name) into :_anyvitaldata from dc_normal
        where datamartid=%upcase("&dmid") and scan(dc_name,1,'_')="VIT";
quit;
...
data _null_;
     ...
     if dc_name="XTBL_L3_METADATA" and 
        variable in ("QUERY_PACKAGE" "REFRESH_MAX" "RESPONSE_DATE" "LOOKBACK_DATE"
                     "CDM_VERSION" "SAS_ETS" "SAS_ETS_INSTALLED" "DATAMART_EHR");
     ...
     else if variable="DATAMART_EHR" then call symput("dm_ehr",strip(resultc));
run;
...
data newcat;
     ...
     if dc_name="XTBL_L3_RACE_ENC" then delete; 
     else if index(dc_name,'RACEETH')>0 and statistic='RECORD_PCT' then delete;

     if dc_name="DEM_L3_AGEYRSDIST2" then cat=strip(put(category,$agegroup.));
     else if category in ("R" "NI" "UN" "OT") and dc_name="DEM_L3_RACEETHMISS" then cat="Missing, NI, UN or OT";
     else if category="NULL or missing" and dc_name="DEM_L3_RACEETHMISS" then cat="NULL or missing";
          ...
          (dc_name="DEM_L3_RACEDIST" and category="07")
          then cat="Missing, NI, UN or OT";     
...
data tbl(keep=ord ord2 col1 resultc dc_name statistic);
     ...
     %cat(dcn=DEM_L3_RACEETHAIAN,cat='Y',ord=6,ord2=1,col1=%str(American Indian or Alaska Native))    
     %cat(dcn=DEM_L3_RACEETHASIAN,cat='Y',ord=6,ord2=2,col1=%str(Asian))
     %cat(dcn=DEM_L3_RACEETHBLACK,cat='Y',ord=6,ord2=3,col1=%str(Black))
     %cat(dcn=DEM_L3_RACEETHHISPANIC,cat='Y',ord=6,ord2=4,col1=%str(Hispanic))
     %cat(dcn=DEM_L3_RACEETHMENA,cat='Y',ord=6,ord2=5,col1=%str(Middle Eastern or North African))
     %cat(dcn=DEM_L3_RACEETHNHPI,cat='Y',ord=6,ord2=6,col1=%str(Native Hawaiian or Pacific Islander))
     %cat(dcn=DEM_L3_RACEETHWHITE,cat='Y',ord=6,ord2=7,col1=%str(White))
     %cat(dcn=DEM_L3_RACEETHMISS,cat=%str('Missing, NI, UN or OT'),ord=6,ord2=8,col1=%str(Missing or Refused))
     ...
     %cat(dcn=DEM_L3_ORIENTDIST,cat='QU',ord=98,ord2=4,col1=Queer)
...
*- Calculate percentages for RACEETH categories -*;
data print_ia;
     set print_ia; 
     if _n_=1 then set newcat_sum (keep=dc_name category resultn where=(dc_name_='DEM_L3_RACEETHMISS' and category='Missing, NI, UN or OT') rename=(dc_name=dc_name_ resultn=raceethmiss_miss));
     if _n_=1 then set newcat_sum (keep=dc_name category resultn where=(dc_name_='DEM_L3_RACEETHMISS' and category='NULL or missing') rename=(dc_name=dc_name_ resultn=raceethmiss_null));

     if ord=6 then do;
          if ord2=8 and sum(raceethmiss_miss,raceethmiss_null)>0 then col3=strip(put(input(compress(col2,','),best20.)/sum(raceethmiss_miss,raceethmiss_null)*100,5.1));
          else if ord2>0 and raceethmiss_null>0 then col3=strip(put(input(compress(col2,','),best20.)/raceethmiss_null*100,5.1));

          if ord2=0 then col4='';
          else if ord2=1 then col4='DEM_L3_RACEETHAIAN';
          else if ord2=2 then col4='DEM_L3_RACEETHASIAN';
          else if ord2=3 then col4='DEM_L3_RACEETHBLACK';
          else if ord2=4 then col4='DEM_L3_RACEETHHISPANIC';
          else if ord2=5 then col4='DEM_L3_RACEETHMENA';
          else if ord2=6 then col4='DEM_L3_RACEETHNHPI';
          else if ord2=7 then col4='DEM_L3_RACEETHWHITE';
          else if ord2=8 then col4='DEM_L3_RACEETHMISS';
     end;

     drop dc_name_ category raceethmiss_miss raceethmiss_null;
run;     
...
proc format;
     value metric
        ...
        8='Patients with address ZIP5 and county FIPS code data'
        ;
     ...
     value met_desc
        ...
        8='Percentage of patients with a face-to-face visit in the past 5 years that have ADDRESS_ZIP5 and COUNTY_FIPS address data for their current physical address'
        ;
run;
...
data prows;
     ...
     if _n_=1 then set dc_normal(keep=dc_name variable category statistic resultn rename=(resultn=denom_fipszip)
                                 where=(dc_name="ENC_L3_DASH2" and variable="PERIOD" and statistic="DISTINCT_PATID_N" and category="5 yrs"));
     set dc_normal(in=d where=(dc_name="DIA_L3_N" and variable="PATID" and statistic="DISTINCT_N"))
         ...
         dc_normal(in=f where=(dc_name="XTBL_L3_CFIPSZIP_5Y" and variable="ADDRESS_ZIP5 and COUNTY_FIPS" and statistic="DISTINCT_PATID_N"))
     ;
   
     if d then ord=6;
     else if p then ord=7;
     else if f then ord=8;

     if ord in (6 7 ) then do;
          if .<(resultn/denom_enc)*100<50.0 then threshold=1;
          col3=strip(put(resultn,comma16.));
          col4=strip(put((resultn/denom_enc)*100,comma16.1));
          col5="ENC_L3_N; " || strip(dc_name);
     end;
     else do;
          if denom_fipszip>0 and (.<(resultn/denom_fipszip)*100<80.0 or resultn=0) then threshold=1;
          col3=strip(put(resultn,comma16.));
          col4=strip(put((resultn/denom_fipszip)*100,comma16.1));
          col5="XTBL_L3_CFIPSZIP_5Y";
     end;
...
%macro print_ib;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ib split='|' style(header)=[backgroundcolor=CXCCCCCC];
         column ord col1 col2 threshold col3 col4 col5;
         ...
         compute col4;
               if threshold=1 then do;
                    if ord in (3 8) then call define(_col_, "style", "style=[color=blue fontweight=bold]");
                    else if ord in (6 7) then call define(_col_, "style", "style=[color=red fontweight=bold]");
               end;
         endcomp;
    run;    
...
%if &_c1data^= and &_c1data^=0 and &_c1std^= %then %do;
    ...
    *- Produce output -*;
    ...
    proc gplot data=chart_1a;
         where deviations ne . and start ne .;
...
data data;
     set dc_normal(where=(datamartid=%upcase("&dmid") and scan(dc_name,3,'_')="N" and variable in 
                  ("PATID" "PATID_1" "ENCOUNTERID" "ENROLLID" "DIAGNOSISID" "PROCEDURESID" "VITALID" "LAB_RESULT_CM_ID" "PRESCRIBINGID" "DISPENSINGID" 
                   "CONDITIONID" "DEATHCID" "PRO_CM_ID" "PROVIDERID" "MEDADMNID" "OBSCLINID" "OBSGENID" "IMMUNIZATIONID"
                   "LABHISTORYID" "PATRELID" "EXTMEDID")));
     ...
     if variable='PATID_1' then variable='PATID';

     if variable not in ("PATID" "PATID_1" "ENCOUNTERID") then variable="ID";
...
proc sort data=dc_normal(keep=datamartid dc_name table statistic result: variable)
     ...
     where datamartid=%upcase("&dmid") and dc_name="XTBL_L3_DATES" and 
        variable not in ("DISCHARGE_DATE" "ENR_END_DATE" "DX_DATE" "PX_DATE" 
           ...
           "OBSGEN_STOP_DATE" "RELATIONSHIP_END" "EXT_PAT_START_DATE" "EXT_PAT_END_DATE" "EXT_END_DATE");
run;
...
data print_id;
     ...
     if col2='' then do;
          if table='LAB_HISTORY' then col2='---';
          else col2='N/A';
     end;
     if upcase(col2)="N/A" and table not in ("EXTERNAL_MEDS" "PAT_RELATIONSHIP") then threshold=1;
     ...
     %if &nobs_external_meds=0 %then %do;
          if col0='EXTERNAL_MEDS' then delete;
     %end;

     %if &nobs_pat_relationship=0 %then %do;
          if col0='PAT_RELATIONSHIP' then delete;
     %end;
...
data data;
     ...
     set dc_normal(where=(datamartid=%upcase("&dmid") and statistic="RECORD_N"
         and dc_name in ("DIA_L3_ENCTYPE" "PRO_L3_ENCTYPE" "ENC_L3_ENCTYPE" "XTBL_L3_LAB_ENCTYPE" "XTBL_L3_VIT_ENCTYPE")
...
data print_ie(keep=ord col: enc_fl);
     ...
     merge 
           ...
           stats(where=(scan(dc_name,1,'_')="DIA") rename=(sum=diagnosis denom=dia_den))
           stats(where=(scan(dc_name,3,'_')="LAB") rename=(sum=lab_result_cm denom=lab_den))
           stats(where=(scan(dc_name,1,'_')="PRO") rename=(sum=procedure denom=pro_den))
           stats(where=(scan(dc_name,3,'_')="VIT") rename=(sum=vital denom=vit_den))
     ;
     by cat;
     ...
     col6=strip(put(lab_result_cm,comma16.));
     if cat^="TOTAL" and lab_result_cm>0 and lab_den>0 then col7=strip(put((lab_result_cm/lab_den)*100,comma16.1));

     col8=strip(put(procedure,comma16.));
     if cat^="TOTAL" and procedure>0 and pro_den>0 then col9=strip(put((procedure/pro_den)*100,comma16.1));

     col10=strip(put(vital,comma16.));
     if cat^="TOTAL" and vital>0 and vit_den>0 then col11=strip(put((vital/vit_den)*100,comma16.1));
...
%macro print_ie;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ie split='|' style(header)=[backgroundcolor=CXCCCCCC];
         column enc_fl col1 
                ("ENCOUNTER|(ENC_L3_ENCTYPE)|___________________" col2 col3) 
                ("DIAGNOSIS|(DIA_L3_ENCTYPE)|___________________" col4 col5)
                ("LAB_RESULT_CM|(XTBL_L3_LAB_ENCTYPE)|___________________" col6 col7)
                ("PROCEDURES|(PRO_L3_ENCTYPE)|___________________" col8 col9)
                ("VITAL|(XTBL_L3_VIT_ENCTYPE)|___________________" col10 col11);

         define enc_fl   /display noprint;
         define col1     /display flow "Encounter Type" style(header)=[just=left cellwidth=23%] style(column)=[just=left];
         define col2     /display flow "N" style(header)=[just=center cellwidth=10%] style(column)=[just=center];
         define col3     /display flow "%" style(header)=[just=center cellwidth=5%] style(column)=[just=center];
         define col4     /display flow "N" style(header)=[just=center cellwidth=10%] style(column)=[just=center];
         define col5     /display flow "%" style(header)=[just=center cellwidth=5%] style(column)=[just=center];
         ...
         define col10    /display flow "N" style(header)=[just=center cellwidth=10%] style(column)=[just=center];
         define col11    /display flow "%" style(header)=[just=center cellwidth=5%] style(column)=[just=center];
    run;    
...
%if &_c2data^= and &_c2data^=0 and &_c2std^= %then %do;
    ...
    *- Produce output -*;
    ...
    proc gplot data=chart_ib;
         where deviations ne . and count ne .;
...
data _null_;
     set standard end=eof;
     retain totalsum 0;
     if not missing(sum) then totalsum=totalsum+sum;
     if eof then do;
...
proc format;
     value pkeys
      ...
      24='EXTMEDID is unique'
      25='PATRELID (concatentation of PATID_1 + PATID_2 + RELATIONSHIP_TYPE) is unique'
        ;
run;
...
%tbl(dcn=EXTMED_L3_N,ord=24,var=EXTMEDID)
%tbl(dcn=PATREL_L3_N,ord=25,var=PATRELID)
...
data tbl(keep=ord: col:);
     ...
     %stat(dcn=DEM_L3_BIOBANK,ord=1,ord2=7,var=BIOBANK_FLAG)     
     %stat(dcn=DEM_L3_RACEETHMISS,ord=1,ord2=8,var=RACE_ETH_MISSING)
     %stat(dcn=DEM_L3_RACEETHAIAN,ord=1,ord2=9,var=RACE_ETH_AI_AN)
     %stat(dcn=DEM_L3_RACEETHASIAN,ord=1,ord2=10,var=RACE_ETH_ASIAN)
     %stat(dcn=DEM_L3_RACEETHBLACK,ord=1,ord2=11,var=RACE_ETH_BLACK)
     %stat(dcn=DEM_L3_RACEETHHISPANIC,ord=1,ord2=12,var=RACE_ETH_HISPANIC)
     %stat(dcn=DEM_L3_RACEETHMENA,ord=1,ord2=13,var=RACE_ETH_ME_NA)
     %stat(dcn=DEM_L3_RACEETHNHPI,ord=1,ord2=14,var=RACE_ETH_NH_PI)
     %stat(dcn=DEM_L3_RACEETHWHITE,ord=1,ord2=15,var=RACE_ETH_WHITE)
     ...
     %stat(dcn=DIA_L3_DXTYPE,ord=5,ord2=2,var=DX_TYPE)
     ...
     %stat(dcn=DIA_L3_ORIGIN,ord=5,ord2=6,var=DX_ORIGIN)
     %stat(dcn=PRO_L3_ENCTYPE,ord=6,ord2=1,var=ENC_TYPE)
     %stat(dcn=PRO_L3_PXTYPE,ord=6,ord2=2,var=PX_TYPE)
     ...
     %stat(dcn=PROCM_L3_METHOD,ord=14,ord2=2,var=PRO_METHOD)
     %stat(dcn=PROCM_L3_MODE,ord=14,ord2=3,var=PRO_MODE)
     %stat(dcn=PROCM_L3_CAT,ord=14,ord2=4,var=PRO_CAT)
     %stat(dcn=PROCM_L3_SOURCE,ord=14,ord2=5,var=PRO_SOURCE)
     ...
     %stat(dcn=LDSADRS_L3_CURRENT,ord=21,ord2=5,var=CURRENT_ADDRESS_FLAG)
     ...
     %stat(dcn=EXTMED_L3_RXDOSEFORM,ord=24,ord2=1,var=EXT_DOSE_FORM)
     %stat(dcn=EXTMED_L3_RXDOSE_UNIT,ord=24,ord2=2,var=EXT_DOSE_ORDERED_UNIT)
     %stat(dcn=EXTMED_L3_ROUTE,ord=24,ord2=3,var=EXT_ROUTE)
     %stat(dcn=EXTMED_L3_BASIS,ord=24,ord2=4,var=EXT_BASIS)
     %stat(dcn=EXTMED_L3_SOURCE,ord=24,ord2=5,var=EXTMED_SOURCE)
     %stat(dcn=PATREL_L3_TYPE,ord=25,ord2=1,var=RELATIONSHIP_TYPE)
run;
...
* Sort by Table and Field;
proc sort data=print_iib;
     by col1 col2;
run; 
...
data print_iib;
     ...
     if _n_<=30 then pg=1;
     else if _n_<=60 then pg=2;
     else if _n_<=90 then pg=3;
     else if _n_<=120 then pg=4;
     else if _n_<=150 then pg=5;
     else pg=6;
run;     
...
data tbl(keep=ord: col: _obs3);
	...
	%stat(dcn=HASH_L3_N,ord=20,ord2=2,var=TOKEN_ENCRYPTION_KEY,stat=NULL_N)
     ...
     %stat(dcn=EXTMED_L3_N,ord=24,ord2=1,var=EXTMEDID,stat=NULL_N)
     %stat(dcn=EXTMED_L3_N,ord=24,ord2=2,var=PATID,stat=NULL_N)
     %stat(dcn=PATREL_L3_N,ord=25,ord2=1,var=PATID_1,stat=NULL_N)
     %stat(dcn=PATREL_L3_N,ord=25,ord2=2,var=PATID_2,stat=NULL_N)
     %stat(dcn=PATREL_L3_TYPE,ord=25,ord2=3,cat=NULL or missing,var=RELATIONSHIP_TYPE,stat=RECORD_N)
run;
...
proc format;
     ...
     value $dcdesc
        ...
        '1.21'='Geo codes in the LDS_ADDRESS_HISTORY table do not conform to expected values'
        '1.22'='Patients have multiple current physical addresses in the LDS_ADDRESS_HISTORY table'
        ;

    value except
        1='Required table is not present.  All tables must be present in an instantiation of the CDM'
        2='Table required to be populated is not populated. DEMOGRAPHIC, ENROLLMENT, ENCOUNTER, DIAGNOSIS, PROCEDURES, and HARVEST must be populated. If EHR data is included in the DataMart, LAB_RESULT_CM, PRESCRIBING and VITAL must also be populated. '
        ...
        8='Field lengths are not harmonized for one or more of the following fields: PATID, PATID_1, PATID_2, ENCOUNTERID, PRESCRIBINGID, PROCEDURESID, PROVIDERID, MEDADMIN_PROVIDERID, OBSGEN_PROVIDERID, OBSCLIN_PROVIDERID, RX_PROVIDERID, and VX_PROVIDERID.'
        ...
        10='LDS_ADDRESS_HISTORY.STATE_FIPS, LDS_ADDRESS_HISTORY.COUNTY_FIPS, or LDS_ADDRESS_HISTORY.RUCA_ZIP contains alphabetical characters or does not have the expected number of digits, or the first 2 digits of COUNTY_FIPS does not match STATE_FIPS.'
        11="A PATID has multiple ADDRESSIDs where CURRENT_ADDRESS_FLAG='Y'"
        ;
run;
...
data compare_tbl(keep=miss_table not_pop)
     ...
     * keep if required *;
     ...
     if .<=nobsn<=0 and ((upcase(current_version)="YES") or (upcase(current_version)="MAYBE" and strip("&dm_ehr")="02")) then do;
...
data var_si;
     ...
     else if name in ('PATID_1' 'PATID_2') then name='PATID';
...
data row8;
     length memname $32 name $45;
     merge 
          patid_length(in=pt)
          patid_1_length(in=pt1)
          patid_2_length(in=pt2)
     ...
     retain name;
     array l pt pt1 pt2 enc pres proc prov;
     array lt $15 ptt pt1t pt2t enct prest proct provt ('PATID' 'PATID_1' 'PATID_2' 'ENCOUNTERID' 'PRESCRIBINGID' 'PROCEDURESID' 'PROVIDERID');
        do i = 1 to dim(l);
...
data row9;
     ...
     if flr=1 then do;
        if (z5r=1 and z9r=1) then do;
            ...
            name="FACILITY_LOCATION, ADDRESS_ZIP5, ADDRESS_ZIP9";
        end;
        else if z5r=1 then do;
            memname="ENCOUNTER, LDS_ADDRESS_HISTORY";
            name="FACILITY_LOCATION, ADDRESS_ZIP5";
        end;
        else if z9r=1 then do;
            memname="ENCOUNTER, LDS_ADDRESS_HISTORY";
            name="FACILITY_LOCATION, ADDRESS_ZIP9";
        end;
        else do;
            memname="ENCOUNTER";
...
data row10;
     length memname $32 name $45;
     merge dc_normal(in=z5 keep=datamartid statistic dc_name variable resultn table 
                   where=(datamartid=%upcase("&dmid") and statistic="ALL_N" 
                   and dc_name='LDSADRS_L3_N' and variable='STATE_FIPS')
                   rename=(resultn=state_all))
           dc_normal(in=z5 keep=datamartid statistic dc_name variable resultn table 
                   where=(datamartid=%upcase("&dmid") and statistic="VALID_N" 
                   and dc_name='LDSADRS_L3_N' and variable='STATE_FIPS')
                   rename=(resultn=state_valid))
           dc_normal(in=z9 keep=datamartid statistic dc_name variable resultn table 
                   where=(datamartid=%upcase("&dmid") and statistic="ALL_N" 
                   and dc_name='LDSADRS_L3_N' and variable='COUNTY_FIPS')
                   rename=(resultn=county_all))
           dc_normal(in=z9 keep=datamartid statistic dc_name variable resultn table 
                   where=(datamartid=%upcase("&dmid") and statistic="VALID_N" 
                   and dc_name='LDSADRS_L3_N' and variable='COUNTY_FIPS')
                   rename=(resultn=county_valid))
           dc_normal(in=z9 keep=datamartid statistic dc_name variable resultn table 
                   where=(datamartid=%upcase("&dmid") and statistic="ALL_N" 
                   and dc_name='LDSADRS_L3_N' and variable='RUCA_ZIP')
                   rename=(resultn=ruca_all))
           dc_normal(in=z9 keep=datamartid statistic dc_name variable resultn table 
                   where=(datamartid=%upcase("&dmid") and statistic="VALID_N" 
                   and dc_name='LDSADRS_L3_N' and variable='RUCA_ZIP')
                   rename=(resultn=ruca_valid))
     ;
     by datamartid;
     ...
     if state_valid<state_all then str=1;
     else str=0;
     if county_valid<county_all then ctr=1;
     else ctr=0;
     if ruca_valid<ruca_all then rcr=1;
     else rcr=0;

     if str=1 then do;
        if (ctr=1 and rcr=1) then do;
            memname="LDS_ADDRESS_HISTORY";
            name="STATE_FIPS, COUNTY_FIPS, RUCA_ZIP";
        end;
        else if ctr=1 then do;
            memname="LDS_ADDRESS_HISTORY";
            name="STATE_FIPS, COUNTY_FIPS";
        end;
        else if rcr=1 then do;
            memname="LDS_ADDRESS_HISTORY";
            name="STATE_FIPS, RUCA_ZIP";
        end;
     end;
     else if ctr=1 then do;
        memname="LDS_ADDRESS_HISTORY";
        if rcr=1 then name="COUNTY_FIPS, RUCA_ZIP";
        else name="COUNTY_FIPS";
     end;
     else if rcr=1 then do;
        memname="LDS_ADDRESS_HISTORY";
        name="RUCA_ZIP";
...
data row11;
     length memname $32 name $45;
     merge dc_normal(in=z10 keep=datamartid statistic dc_name category resultn table 
                   where=(datamartid=%upcase("&dmid") and statistic="DISTINCT_PATID_N" 
                   and dc_name='LDSADRS_L3_CURRENT' and category='Y')
                   rename=(resultn=z10_distinct))
           dc_normal(in=z10 keep=datamartid statistic dc_name category resultn table 
                   where=(datamartid=%upcase("&dmid") and statistic="RECORD_N" 
                   and dc_name='LDSADRS_L3_CURRENT' and category='Y')
                   rename=(resultn=z10_record))
     ;
     by datamartid;
     ...
     if z10_distinct<z10_record then do;
          memname="LDS_ADDRESS_HISTORY";
          name="CURRENT_ADDRESS_FLAG";
     end;
     else do;
          memname=" ";
          name="None";
...
data final;
     ...
     if row not in (1,2) then do;
        ...
        if name="FACILITY_LOCATION, ADDRESS_ZIP5, " then 
           col5="FACILITY_LOCATION, ADDRESS_ZIP5, ADDRESS_ZIP9";
     end;
     else if row in (1,2) then col5="n/a";

     if row<9 then col6="DATAMART_ALL";
     else if row=9 then col6="ENC_L3_N; LDSADRS_L3_N";
     else if row=10 then col6="LDSADRS_L3_N";
     else if row=11 then col6="LDSADRS_L3_CURRENT";
run;
...
proc format;
     ...
     value dcdesc
        ...
        14.02='DEMOGRAPHIC table is'
        14.03='not included in the'
        14.04='HASH_TOKEN table'
    ...
    value except
         ...
         8.06='LAB_RESULT_CM,'
         8.07='LDS_ADDRESS_HISTORY,EXTERNAL_MEDS,'
         8.08='MED_ADMIN, OBS_CLIN, OBS_GEN,'
         8.09='PCORNET_TRIAL, PAT_RELATIONSHIP,'
         8.10='PRESCRIBING,PROCEDURES, PRO_CM,'
         8.11='or VITAL table'
...
data denom;
     merge dc_normal(where=(datamartid=%upcase("&dmid") and dc_name="DIA_L3_N" and variable="ENCOUNTERID" and statistic="DISTINCT_N") 
           ...
           dc_normal(where=(datamartid=%upcase("&dmid") and dc_name="EXTMED_L3_N" and variable="PATID" and statistic="DISTINCT_N") 
                        rename=(resultn=pat_extmed))
           dc_normal(where=(datamartid=%upcase("&dmid") and dc_name="PATREL_L3_N" and variable="PATID_1" and statistic="DISTINCT_N") 
                        rename=(resultn=pat_patrel1))
           dc_normal(where=(datamartid=%upcase("&dmid") and dc_name="PATREL_L3_N" and variable="PATID_2" and statistic="DISTINCT_N") 
                        rename=(resultn=pat_patrel2))
...
data orphan;
     ...
     else if scan(variable,1,' ') in ("PATID" "PATID_1" "PATID_2") then row=8;
...
data final;
     ...
     if row=8 and resultn>=0 then do;
        ...
        else if col4="EXTERNAL_MEDS" and pat_extmed>0 then col7=strip(put((resultn/pat_extmed)*100,16.1));
        else if col4="PAT_RELATIONSHIP" then do;
          if scan(variable,1,' ')="PATID_1" and pat_patrel1>0 then col7=strip(put((resultn/pat_patrel1)*100,16.1));
          if scan(variable,1,' ')="PATID_2" and pat_patrel2>0 then col7=strip(put((resultn/pat_patrel2)*100,16.1));
        end;
     end;
...
%macro print_iif;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_iif split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define codes         /display format=comma16. flow "Distinct Codes" style(header)=[just=center cellwidth=12%] style(column)=[just=center];
...
%end;
********************************************************************************;
* TABLE IIG TABLE IIG (LOINC PANELS)
********************************************************************************;
%let tname=table iig;
%if %index(&_part_mstring,%upcase(&tname))>0 %then %do;
...
proc means data=dc_normal nway noprint;
     class dc_name table variable;
     var resultn;
     output out=denominator sum=denom;
     where datamartid=%upcase("&dmid") and statistic in ("RECORD_N") and 
           ((dc_name="LAB_L3_LOINC" and table="LAB_RESULT_CM" and variable="LAB_LOINC") or 
            (dc_name="PROCM_L3_LOINC" and table="PRO_CM" and variable="PRO_CODE") or 
            (dc_name="OBSCLIN_L3_LOINC" and table="OBS_CLIN" and variable="OBSCLIN_CODE"));
run;
...
proc means data=dc_normal nway noprint;
     class dc_name table variable;
     var resultn;
     output out=numerator sum=num;
     where datamartid=%upcase("&dmid") and statistic in ("RECORD_N") and cross_category="Panel" and
           ((dc_name="LAB_L3_LOINC" and table="LAB_RESULT_CM" and variable="LAB_LOINC") or 
            (dc_name="PROCM_L3_LOINC" and table="PRO_CM" and variable="PRO_CODE") or 
            (dc_name="OBSCLIN_L3_LOINC" and table="OBS_CLIN" and variable="OBSCLIN_CODE"));
run;

*- Dummy data, just in case -*;
data dummy;
     length dc_name $100 table variable $50;
     num=0;
     denom=0;
     ...
     ord=2;
     dc_name="OBSCLIN_L3_LOINC";
     table="OBS_CLIN";
     variable="OBSCLIN_CODE";
     output;

     ord=3;
     dc_name="PROCM_L3_LOINC";
     table="PRO_CM";
     variable="PRO_CODE";
     output;
run;
...
data print_iig;
     length col1-col6 $50;
     merge dummy numerator denominator;
     by dc_name table variable;

     col1=strip(table);
     col2=strip(variable);
     col3=strip(put(num,comma16.));
     col4=strip(put(denom,comma16.));
     if denom>0 and num>=0 then col5=strip(put((num/denom)*100,6.2));
     col6=strip(dc_name);
...
data dc_summary_iig;
     length edc_table_s_ $10;
     set print_iig(keep=col5);
     if input(col5,5.2)>5.00;
     row=1.20;
     exception=1;
     edc_table_s_="Table IIG";
     keep edc_table_s_ row exception;
run;

proc sort data=dc_summary_iig nodupkey;
     by edc_table_s_ row;
run;
    
*- Append exceptions to master dataset -*;
data dc_summary;
     merge dc_summary dc_summary_iig;
     by edc_table_s_ row;
run;
...
%macro print_iig;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_iig split='|' style(header)=[backgroundcolor=CXCCCCCC];
         column pg ord col1 col2 col3 col4 col5 col6;
         where pg=1;
         ...
         define col3     /display "LOINC records with a Panel_Type of 'Panel'" style(header)=[just=center cellwidth=12%] style(column)=[just=center];
         define col4     /display flow "LOINC records" style(header)=[just=center cellwidth=12%] style(column)=[just=center];
         define col5     /display flow "% of LOINC records that are panels" style(header)=[just=center cellwidth=8%] style(column)=[just=center];
...
data tbl(keep=ord: col: dc_name resultn);
     ...
     %stat(dcn=XTBL_L3_DATES,ord=24,ord2=40,tbl=EXTERNAL_MEDS,var=EXT_RECORD_DATE,stat=FUTURE_DT_N)
     %stat(dcn=XTBL_L3_DATES,ord=24,ord2=41,tbl=EXTERNAL_MEDS,var=EXT_PAT_START_DATE,stat=FUTURE_DT_N)
     %stat(dcn=XTBL_L3_DATES,ord=24,ord2=42,tbl=EXTERNAL_MEDS,var=EXT_PAT_END_DATE,stat=FUTURE_DT_N)
     %stat(dcn=XTBL_L3_DATES,ord=24,ord2=43,tbl=EXTERNAL_MEDS,var=EXT_END_DATE,stat=FUTURE_DT_N)
     %stat(dcn=XTBL_L3_DATES,ord=25,ord2=44,tbl=PAT_RELATIONSHIP,var=RELATIONSHIP_START,stat=FUTURE_DT_N)
     %stat(dcn=XTBL_L3_DATES,ord=25,ord2=45,tbl=PAT_RELATIONSHIP,var=RELATIONSHIP_END,stat=FUTURE_DT_N)
run;
...
data den(keep=ord: denom);
     ...
     %denom(dcn=XTBL_L3_DATES,ord=24,ord2=40,tbl=EXTERNAL_MEDS,var=EXT_RECORD_DATE)
     %denom(dcn=XTBL_L3_DATES,ord=24,ord2=41,tbl=EXTERNAL_MEDS,var=EXT_PAT_START_DATE)
     %denom(dcn=XTBL_L3_DATES,ord=24,ord2=42,tbl=EXTERNAL_MEDS,var=EXT_PAT_END_DATE)
     %denom(dcn=XTBL_L3_DATES,ord=24,ord2=43,tbl=EXTERNAL_MEDS,var=EXT_END_DATE)
     %denom(dcn=XTBL_L3_DATES,ord=25,ord2=44,tbl=PAT_RELATIONSHIP,var=RELATIONSHIP_START)
     %denom(dcn=XTBL_L3_DATES,ord=25,ord2=45,tbl=PAT_RELATIONSHIP,var=RELATIONSHIP_END)
run;
...
data print_iiia(drop=col3);
     ...
     if col1^ in ("OBS_CLIN" "OBS_GEN" "LDS_ADDRESS_HISTORY" "IMMUNIZATION" "EXTERNAL_MEDS" "PAT_RELATIONSHIP") 
        then pg=1;
     else pg=2;
...
%macro stat(dcn=,ord=,bound=,cat=,stat=);
     if dc_name="&dcn" and category in (&cat) and statistic="&stat" then do;
        ord=&ord;
        bound="&bound";
        col1=strip(table);        
        col12=strip(dc_name);
...
*- Non-cross table queries -*;
data tbl(keep=ord bound statistic result: col: category);
     length col1 col12 $50;
     set dc_normal(where=(datamartid=%upcase("&dmid")));

     %stat(dcn=DEM_L3_AGEYRSDIST2,ord=1,bound=L,cat=%str("<0 yrs"),stat=RECORD_N)
     %stat(dcn=DEM_L3_AGEYRSDIST2,ord=1,bound=H,cat=%str(">89 yrs"),stat=RECORD_N)

     %stat(dcn=VIT_L3_HT,ord=2,bound=L,cat=%str("<0" "0-10" "11-20"),stat=RECORD_N)
     %stat(dcn=VIT_L3_HT,ord=2,bound=H,cat=%str("77-82" "83-88" "89-94" ">=95"),stat=RECORD_N)
    
     %stat(dcn=VIT_L3_WT,ord=3,bound=L,cat=%str("<0"),stat=RECORD_N)
     %stat(dcn=VIT_L3_WT,ord=3,bound=H,cat=%str(">350"),stat=RECORD_N)
    
     %stat(dcn=VIT_L3_DIASTOLIC,ord=4,bound=L,cat=%str("<40"),stat=RECORD_N)
     %stat(dcn=VIT_L3_DIASTOLIC,ord=4,bound=H,cat=%str(">120"),stat=RECORD_N)
    
     %stat(dcn=VIT_L3_SYSTOLIC,ord=5,bound=L,cat=%str("<40"),stat=RECORD_N)
     %stat(dcn=VIT_L3_SYSTOLIC,ord=5,bound=H,cat=%str(">210"),stat=RECORD_N)

     %stat(dcn=DISP_L3_SUPDIST2,ord=6,bound=L,cat=%str("<1 day"),stat=RECORD_N)
     %stat(dcn=DISP_L3_SUPDIST2,ord=6,bound=H,cat=%str(">90 days"),stat=RECORD_N)
run;

proc means data=tbl nway missing noprint;
     class ord col1 col12 bound;
     var resultn;
     output out=xtbl sum=xresultn;
run;
...
data xtbl;
     ...
     length result $50;
     result=strip(put(xresultn,comma16.));
run;     
...
data print_iiib(keep=ord col: );
     ...
     merge xtbl(where=(bound="L") rename=(result=col7 xresultn=col7n))
           xtbl(where=(bound="H") rename=(result=col9 xresultn=col9n))
           xdenom
           median
     ;
     by ord;
...
proc format;
     invalue rowfmt
        ...
        'RELATIONSHIP_START > RELATIONSHIP_END'=33
        'EXT_PAT_START_DATE > EXT_PAT_END_DATE'=34
        'MEDADMIN_START_DATE > MEDADMIN_STOP_DATE'=35
    ;
...
%if %index(&_part_mstring,%upcase(&tname))>0 %then %do;
%global nback nsd exp_encounter exp_diagnosis exp_procedures exp_vital exp_prescribing exp_lab_result_cm exp_med_admin;
...
data p5values;
     ...
     set dc_normal(where=(datamartid=%upcase("&dmid") and dc_name='XTBL_L3_DATES' and statistic="P5" and 
         ((table in ("ENCOUNTER" "DIAGNOSIS" "PROCEDURES") and variable="ADMIT_DATE") or 
          ...
          (table in ("MED_ADMIN") and variable="MEDADMIN_START_DATE") or 
          (table in ("VITAL") and variable="MEASURE_DATE") or 
          (table in ("EXTERNAL_MEDS") and variable="EXT_RECORD_DATE") or
          (table in ("PRO_CM") and variable="PRO_DATE"))));

     if not missing(resultc) then do;
...
data data;
     ...
     set dc_normal(where=(datamartid=%upcase("&dmid") and statistic="RECORD_N" and
         ((dc_name in ('ENC_L3_ENCTYPE_ADATE_YM' 'DIA_L3_ENCTYPE_ADATE_YM' 'PRO_L3_ENCTYPE_ADATE_YM') and 
          cross_category ne "NULL or missing" and category in ("AV", "ED", "IP", "EI", "TH")) or
         (dc_name in ('VIT_L3_MDATE_YM' 'PRES_L3_ODATE_YM' 'LAB_L3_RDATE_YM' 'MEDADM_L3_SDATE_YM'
          'EXTMED_L3_RDATE_YM' 'PROCM_L3_PDATE_YM') and category ne "NULL or missing"))))
     ;
     ...
     * create a month variable *;
     ...
     else if dc_name in ('VIT_L3_MDATE_YM' 'PRES_L3_ODATE_YM' 'LAB_L3_RDATE_YM' 'MEDADM_L3_SDATE_YM'
            'EXTMED_L3_RDATE_YM' 'PROCM_L3_PDATE_YM') then 
...
%exp(dsn=MED_ADMIN,cat=0);
%exp(dsn=EXTERNAL_MEDS,cat=0);
%exp(dsn=PRO_CM,cat=0);
...
data print_iiig;
     ...
     merge expand_encounter(in=c1) 
           ...
           expand_med_admin(in=nc4)
           expand_external_meds(in=nc5)
           expand_pro_cm(in=nc6)
           p5values
     ;
     by table;
     ...
     * keep only months where no obs or beyond the limit of std deviations *;
     ...
     array o c1 c2 c3 nc1 nc2 nc3 nc4 nc5 nc6;
        do oi = 1 to dim(o);
...
data _null_;
     ...
     if not missing(max) and max ne 0 then do;
          d=int(log10(max));
          f=ceil(max/(10**d));
          r=f*(10**d);
...
data _null_;
     set vertaxis;
     if max>=1000 then do;
        vertaxis=max-mod(max,100)+100;
        tick=100;
     end;
     else if max>=500 then do;
        vertaxis=max-mod(max,50)+50;
        tick=50;
     end;
     else if max>=250 then do;
        vertaxis=max-mod(max,25)+25;
        tick=25;
     end;
     else if max>=100 then do;
...
proc format cntlout=r_field;
     value $r_field
        ...
        "DRG_TYPE"="29"
        "ADMITTING_SOURCE"="30"
        "PAYER_TYPE_PRIMARY"="31"
        "PAYER_TYPE_SECONDARY"="32"
        "FACILITY_TYPE"="33"
        "FACILITY_LOCATION"="34"
        "DX_DATE"="35"
        "DX_TYPE"="36"
        "DX_SOURCE"="37"
        "DX_ORIGIN"="38"
     ...
     value $r_source
        ...
        "09"="DEM_L3_BIOBANK"
        "10"="DEM_L3_RACEETHMISS"
        "11"="DEM_L3_RACEETHAIAN"
        "12"="DEM_L3_RACEETHASIAN"
        "13"="DEM_L3_RACEETHBLACK"
        "14"="DEM_L3_RACEETHHISPANIC"
        "15"="DEM_L3_RACEETHMENA"
        "16"="DEM_L3_RACEETHNHPI"
        "17"="DEM_L3_RACEETHWHITE"
        "18"="XTBL_L3_DATES"
        "19"="ENR_L3_CHART"
        "20"="XTBL_L3_TIMES"
        "21"="ENC_L3_ENCTYPE_DDATE_YM"
        "22"="XTBL_L3_TIMES"
        "23"="ENC_L3_ENCTYPE"
        "24"="ENC_L3_N"
        "25"="ENC_L3_N"
        "26"="ENC_L3_ENCTYPE_DISDISP"
        "27"="ENC_L3_ENCTYPE_DISSTAT"
        "28"="ENC_L3_ENCTYPE_DRG"
        "29"="ENC_L3_DRG_TYPE"
        "30"="ENC_L3_ENCTYPE_ADMSRC"
        "31"="ENC_L3_PAYERTYPE1"
        "32"="ENC_L3_PAYERTYPE2"
        "33"="ENC_L3_FACILITYTYPE"
        "34"="ENC_L3_FACILITYLOC"
        "35"="XTBL_L3_DATES"
        "36"="DIA_L3_DXTYPE"
        "37"="DIA_L3_DXSOURCE"
        "38"="DIA_L3_ORIGIN"
        "39"="DIA_L3_PDX_ENCTYPE"
        "40"="DIA_L3_N"
        "41"="XTBL_L3_DATES"
        "42"="DIA_L3_ENCTYPE"
        "43"="DIA_L3_DXPOA"
...
data dum_field;
     ...
     ord=strip(label);
     ordn=input(ord,2.);
     if 1<=ordn<=17 then table="DEMOGRAPHIC";
     else if 18<=ordn<=19 then table="ENROLLMENT";
     else if 20<=ordn<=34 then table="ENCOUNTER";
     else if 35<=ordn<=44 then table="DIAGNOSIS";
     else if 45<=ordn<=52 then table="PROCEDURES";
...
data numerators;
     ...
     if (dc_name in ("DEM_L3_SEXDIST" "DEM_L3_HISPDIST" "DEM_L3_RACEDIST" "DEM_L3_GENDERDIST"
                     "DEM_L3_ORIENTDIST" "DEM_L3_PATPREFLANG" "DEM_L3_BIOBANK" "DEM_L3_RACEETHMISS" "DEM_L3_RACEETHAIAN" 
                     "DEM_L3_RACEETHASIAN" "DEM_L3_RACEETHBLACK" "DEM_L3_RACEETHHISPANIC" "DEM_L3_RACEETHMENA" 
                     "DEM_L3_RACEETHNHPI" "DEM_L3_RACEETHWHITE" "ENC_L3_DRG_TYPE" "ENC_L3_ENCTYPE" "ENR_L3_CHART" 
...
data print_ivc(keep=ord col: perc thresholdflag);
     ...
     if var in ("BIRTH_DATE" "SEX" "DX_ORIGIN" "DX_SOURCE" "DIA_ENCOUNTERID" "DISCHARGE_DATE" "DISCHARGE_DISPOSITION" 
                "PRO_ENCOUNTERID" "PX_DATE" "PX_SOURCE") 
...
proc format cntlout=o_field;
     value $o_field
        ...
        "MEDADMIN_SOURCE"="87"
        "MEDADMIN_CODE"="88"
        "MEDADMIN_START_TIME"="88.1"
        "MEDADMIN_STOP_DATE"="88.2"
        "MEDADMIN_STOP_TIME"="88.3"
        "MEDADMIN_TYPE"="89"
        ...
        "ADDRESS_ZIP5"="119"
        "ADDRESS_ZIP9"="120"
        "CURRENT_ADDRESS_FLAG" = "120.1"
        "RUCA_ZIP" = "120.2"
        "STATE_FIPS" = "120.3"
        "ADDRESS_COUNTY"="120.4"
        ...
        "PERIOD_END"="150"
        "EXT_DOSE_FORM"="152"
        "EXT_DOSE_ORDERED_UNIT"="153"
        "EXT_ROUTE"="154"
        "EXTMED_RXNORM_CUI"="155"
        "EXTMED_SOURCE"="156"
        "EXT_BASIS"="157"
        "EXT_RECORD_DATE"="159"
        "EXT_PAT_START_DATE"="160"
        "EXT_END_DATE"="160.1"
        "EXT_PAT_END_DATE"="160.2"
        "RELATIONSHIP_START"="161"
        "RELATIONSHIP_END"="162"
        "PRO_CAT"="163"
        "PRO_CODE"="164"
        "PRO_FULLNAME"="165"
        "PRO_METHOD"="166"
        ...
        "PRO_SOURCE"="169"
        "PRO_TYPE"="170"
        "PROCM_ENCOUNTERID"="171"
        "PRO_TIME"="172"
        "DEATH_CAUSE_CODE"="173"
        "DEATH_CAUSE_CONFIDENCE"="174"
        "DEATH_CAUSE_SOURCE"="175"
        "DEATH_CAUSE_TYPE"="176"
        ;
     ...
     value $o_source
       ...
       "116"="LDSADRS_L3_COUNTYFIPS"
       "117"="LDSADRS_L3_ADRSCITY"
       "118"="LDSADRS_L3_ADRSSTATE"
       "119"="LDSADRS_L3_ADRSZIP5"
       "120"="LDSADRS_L3_ADRSZIP9"
       "120.1" = "LDSADRS_L3_CURRENT"
       "120.2" = "LDSADRS_L3_RUCAZIP"
       "120.3" = "LDSADRS_L3_STATEFIPS"
       "120.4" = "LDSADRS_L3_N"
       "121"="XTBL_L3_DATES"
       "122"="XTBL_L3_DATES"
       "123"="IMMUNE_L3_N"
       ...
       "126"="XTBL_L3_DATES"
       "127"="XTBL_L3_DATES"
       "129"="IMMUNE_L3_CODETYPE"
       "130"="IMMUNE_L3_STATUS"
       "131"="IMMUNE_L3_STATUSREASON"
       "132"="IMMUNE_L3_SOURCE"
       "133"="IMMUNE_L3_DOSE_DIST"
       "134"="IMMUNE_L3_DOSEUNIT"
       "135"="IMMUNE_L3_ROUTE"
       "136"="IMMUNE_L3_BODYSITE"
       "137"="IMMUNE_L3_MANUFACTURER"
       "138"="IMMUNE_L3_LOTNUM"
       "139"="XTBL_L3_DATES"
       "140"="LABHIST_L3_N"
       "141"="LABHIST_L3_SEXDIST"
       "142"="LABHIST_L3_RACEDIST"
       "143"="LABHIST_L3_MIN_WKS"
       "144"="LABHIST_L3_MAX_WKS"
       "145"="LABHIST_L3_UNIT"
       "146"="LABHIST_L3_LOW"
       "148"="LABHIST_L3_HIGH"
       "149"="LABHIST_L3_PDSTART_Y"
       "150"="LABHIST_L3_PDEND_Y"
       "152"="EXTMED_L3_RXDOSEFORM"
       "153"="EXTMED_L3_RXDOSE_UNIT"
       "154"="EXTMED_L3_ROUTE"
       "155"="EXTMED_L3_RXCUI"
       "156"="EXTMED_L3_SOURCE"
       "157"="EXTMED_L3_BASIS"
       "159"="EXTMED_L3_RDATE_Y"
       "160" = "XTBL_L3_DATES"
       "160.1" = "XTBL_L3_DATES"
       "160.2" = "XTBL_L3_DATES"
       "161"="XTBL_L3_DATES"
       "162"="XTBL_L3_DATES"
       "163"="PROCM_L3_CAT"
       "164"="PROCM_L3_CODE_TYPE"
       "165"="PROCM_L3_FULLNAME"
       "166"="PROCM_L3_METHOD"
       "167"="PROCM_L3_MODE"
       "168"="PROCM_L3_NAME"
       "169"="PROCM_L3_SOURCE"
...
*- Create a dummy dataset to ensure every variable appears in the table -*;
data dum_field;
     length ord $5 var table $50 dc_name $100;
     set o_field;
     if fmtname="O_FIELD";
     var=strip(start);
     ord=strip(label);
     ordn=input(ord,best8.);
     ...
     else if 123<=ordn<=139 then table="IMMUNIZATION";
     else if 140<=ordn<=150 then table="LAB_HISTORY";
     else if 151<=ordn<161 then table="EXTERNAL_MEDS";
     else if 161<=ordn<=162 then table="PAT_RELATIONSHIP";
     else if 163<=ordn<=172 then table="PRO_CM";
     else if 173<=ordn<=176 then table="DEATH_CAUSE";
...
data denominators;
     set dc_normal(where=(datamartid=%upcase("&dmid") and 
                ((dc_name in ("VIT_L3_N" "LAB_L3_N" "PRES_L3_N" "DISP_L3_N" 
                              ...
                              "IMMUNE_L3_N" "EXTMED_L3_N" "PROCM_L3_N" "DEATHC_L3_N") and
                    statistic in ("ALL_N" "NULL_N") and variable in ("PATID")) or
                (dc_name in ("PATREL_L3_N") and statistic in ("ALL_N" "NULL_N") and variable in ("PATRELID")) or
...
data numerators;
     ...
     if (dc_name in ("VIT_L3_VITAL_SOURCE" "DEATH_L3_IMPUTE" "DEATH_L3_MATCH" "DEATH_L3_SOURCE"
                     "LAB_L3_NAME" "LAB_L3_SOURCE" "LAB_L3_PRIORITY" 
                     ...
                     "PRES_L3_FREQ" "PRES_L3_BASIS" 
                     "PRES_L3_RXDOSEFORM"  
                     "PRES_L3_ROUTE" "PRES_L3_SOURCE" "PRES_L3_DISPASWRTN" "PRES_L3_RXDOSEODRUNIT"
                     "DISP_L3_DOSEUNIT" "DISP_L3_ROUTE" "DISP_L3_SOURCE"
                     "COND_L3_STATUS" "COND_L3_SOURCE" "COND_L3_TYPE"                      
                     "PROV_L3_SPECIALTY" "PROV_L3_SEX"
                     "MEDADM_L3_DOSEADMUNIT" "MEDADM_L3_ROUTE" "MEDADM_L3_SOURCE"  "MEDADM_L3_TYPE"
                     "OBSCLIN_L3_TYPE" "OBSCLIN_L3_QUAL" "OBSCLIN_L3_MOD" "OBSCLIN_L3_RUNIT" "OBSCLIN_L3_ABN"
                     "OBSGEN_L3_TYPE" "OBSGEN_L3_QUAL" "OBSGEN_L3_MOD" "OBSGEN_L3_RUNIT" "OBSGEN_L3_ABN"
                     "OBSCLIN_L3_SOURCE" "OBSGEN_L3_SOURCE" "LDSADRS_L3_ADRSUSE" "LDSADRS_L3_ADRSTYPE" 
                     ...
                     "LABHIST_L3_RACEDIST" "LABHIST_L3_UNIT" "LABHIST_L3_LOW"
                     "LABHIST_L3_HIGH" "EXTMED_L3_RXDOSEFORM" "EXTMED_L3_RXDOSE_UNIT" 
                     "EXTMED_L3_ROUTE" "EXTMED_L3_SOURCE" "EXTMED_L3_BASIS"
                     "PROCM_L3_CAT" "PROCM_L3_METHOD" "PROCM_L3_MODE" "PROCM_L3_SOURCE" "PROCM_L3_TYPE"
                     "DEATHC_L3_CODE" "DEATHC_L3_CONF" "DEATHC_L3_SOURCE" "DEATHC_L3_TYPE") and 
                     category in ("NULL or missing" "NI" "UN" "OT") and statistic="RECORD_N") or
        (dc_name in ("PRES_L3_RXCUI" "PRES_L3_SUPDIST2" "LAB_L3_LOINC" "PRES_L3_PRNFLAG" "MEDADM_L3_CODE_TYPE" "OBSCLIN_L3_CODE_TYPE" "OBSGEN_L3_CODE_TYPE" 
                    "EXTMED_L3_RXCUI" "DISP_L3_SUPDIST2" "PROV_L3_NPIFLAG" "LDSADRS_L3_ADRSCITY" "LDSADRS_L3_ADRSSTATE" "LDSADRS_L3_ADRSZIP5" 
                    "LDSADRS_L3_COUNTYFIPS" "LDSADRS_L3_CURRENT" "LDSADRS_L3_RUCAZIP" "LDSADRS_L3_STATEFIPS" "PROCM_L3_CODE_TYPE" "PROCM_L3_FULLNAME" "PROCM_L3_NAME"
                    "LDSADRS_L3_ADRSZIP9" "LABHIST_L3_PDSTART_Y" "LABHIST_L3_PDEND_Y" "IMMUNE_L3_LOTNUM" "LABHIST_L3_SEXDIST" /*"EXTMED_L3_RDATE_Y"*/) and 
                    category="NULL or missing" and statistic="RECORD_N") or                    
        ...
        (dc_name in ("LDSADRS_L3_N") and statistic="NULL_N" and variable in ("ADDRESS_COUNTY")) or
        (dc_name in ("XTBL_L3_DATES") and statistic="NMISS" and variable in ("LAB_ORDER_DATE" "SPECIMEN_DATE" "DEATH_DATE"
                     ...
                     "VX_ADMIN_DATE" "VX_EXP_DATE" "MEDADMIN_STOP_DATE" "RELATIONSHIP_START" "RELATIONSHIP_END"
                     "EXT_RECORD_DATE" "EXT_PAT_START_DATE" "EXT_END_DATE" "EXT_PAT_END_DATE")) or
        (dc_name in ("XTBL_L3_TIMES") and statistic="NMISS" and variable in ("MEASURE_TIME" "SPECIMEN_TIME" "RESULT_TIME" "PRO_TIME"
                     "RX_ORDER_TIME" "OBSCLIN_START_TIME" "OBSCLIN_STOP_TIME" "OBSGEN_START_TIME" "OBSGEN_STOP_TIME" "MEDADMIN_START_TIME" "MEDADMIN_STOP_TIME")) or
        (dc_name in ("PRES_L3_RXQTY_DIST" "PRES_L3_RXREFILL_DIST" "PRES_L3_RXDOSEODR_DIST" "MEDADM_L3_DOSEADM" "DISP_L3_DOSE_DIST" "DISP_L3_DISPAMT_DIST"
                     "IMMUNE_L3_DOSE_DIST" "LABHIST_L3_MIN_WKS" "LABHIST_L3_MAX_WKS") and 
     ...
     else if dc_name="EXTMED_L3_RXCUI" and variable="RXNORM_CUI" then var="EXTMED_RXNORM_CUI";
     else var=variable;
...
data print_ivd(keep=ord: col: perc thresholdflag);
     ...
     else if var="EXTMED_RXNORM_CUI" then col2="RXNORM_CUI";
     else col2=var;
     col3=" ";
     col4=strip(put(num,comma16.));
     col5=strip(put(denom,comma16.));
     ...
     * threshold values for this table *;
     if var in ("DEATH_SOURCE" "ENCOUNTERID" "LAB_RESULT_SOURCE" "MEDADMIN_ENCOUNTERID" "MEDADMIN_CODE" "MEDADMIN_SOURCE" "RX_ENCOUNTERID" "RX_ORDER_DATE" "RX_SOURCE"
                "PRO_CODE" "PRO_SOURCE" "VITAL_ENCOUNTERID" "VITAL_SOURCE")
                and perc>=10.00 then thresholdflag=1;
...
data print_ive(keep=pg col: perc:);
...
%macro print_ive;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ive split='|' style(header)=[backgroundcolor=CXCCCCCC];
         column pg perc col0 col1 col2 col3 col4 col5 col6 col7;
         where pg=1;
...
data dc_summary_ivf;
     ...
     if ord in (2) and ((compress(col2) in ("0" ) or .<col3p<75.0) or 
...
%macro print_ivf;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ivf split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         compute col2;
            if ord in (2) and compress(col2) in ("0") then call define(_col_, "style", "style=[color=blue fontweight=bold]");
         endcomp;
         compute col3;
            if ord in (2) and col3p<75.0 then do;
                col3=col3p;
                call define(_col_, "style", "style=[color=blue fontweight=bold]");
            end;
            else do;
         ...
         endcomp;
         compute col4;
            if ord in (2) and compress(col4) in ("0") then call define(_col_, "style", "style=[color=blue fontweight=bold]");
         endcomp;
         compute col5;
            if ord in (2) and col5p<75.0 then do;
                col5=col5p;
                call define(_col_, "style", "style=[color=blue fontweight=bold]");
            end;
            else do;
         ...
         endcomp;
         compute col6;
            if ord in (2) and compress(col6) in ("0") then call define(_col_, "style", "style=[color=blue fontweight=bold]");
         endcomp;
         compute col7;
            if ord in (2) and col7p<75.0 then do;
                col7=col7p;
                call define(_col_, "style", "style=[color=blue fontweight=bold]");
            end;
            else do;
...
data print_ivg;
     ...
     if ord<24 then do;
         ...
         col1="Benchmark average";
         col1a=" ";
         col2=strip(put(vit/12,comma16.));

         col4=strip(put(pres/12,comma16.));

         col6=strip(put(lab/12,comma16.));
...
proc format;
     value tier_desc
       1 = 'RXCUI encodes ingredient(s), strength and dose form'
     1.5 = 'RXCUI encodes ingredient(s), strength, dose form and brand'
       2 = 'RXCUI encodes ingredient(s) and potentially strength or dose form.  Can still represent medications with multiple ingredients with a single RXCUI.'
       3 = 'Requires more than one RXCUI to represent medications with multiple ingredients.'
       4 = 'RXCUI does not encode any ingredient information.'
       5 = 'RXCUI was not populated or could not be matched to the reference table'
...
data data;
     set dc_normal(where=(datamartid=%upcase("&dmid") and 
                  (dc_name in ("PRES_L3_RXCUI_TIER" "MEDADM_L3_RXCUI_TIER"))));
...
*- Bring everything together -*;
data print_ivh;
     length col1 col3 col4 col6 $65 col2 $200;
...
data dc_summary_ivh;
     ...
     if ord in (1,7) and (col5p<80.0 or compress(col4) in ("0"));
     if ord in (1) then row=3.08;
     else if ord in (7) then row=3.15;
     exception=1;
...
%macro print_ivh;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ivh split='|' style(header)=[backgroundcolor=CXCCCCCC];
         column pg col_1 col1 col2 col3 ord col4 col5p col5 col6;
         where pg=1;
         ...
         endcomp;
         compute col5;
            if ord in (1,7) and col5p<80.0 then do;
                col5=col5p;
                call define(_col_, "style", "style=[color=blue fontweight=bold]");
            end;
            else do;
         ...
         endcomp;
         compute col4;
            if ord in (1,7) and compress(col4) in ("0") then call define(_col_, "style", "style=[color=blue fontweight=bold]");
         endcomp;
    run;
...
%if &_anylabdata>0 %then %do;
    ...
    *- Bring everything together -*;
    data print_ivi_lab;
         length col000 $25 col0 $10 col1 $130 col3 col4 col6 $40;
...
%if &_anyobsclindata>0 %then %do;
    ...
    data r12;
         length col000 $25 col0 $10 col1 $130 col3 col6 $30;
         ord=12;
         col000="OBS_CLIN";
         col0=strip(put(ord,_data_check.));
         col1=strip(put(ord,known_desc.));
    ...
    *- Subset and re-categorize -*;
    data print_ivi_oc;
         ...
         length col0 $10 col1 $130 col3 col4 col6 $40;
         set dc_normal(where=(datamartid=%upcase("&dmid") and 
...
%if &_anylabdata>0 or &_anyobsclindata>0 %then %do;
    ...
    *- Keep records resulting in an exception -*;
    data dc_summary_ivi;
         ...
         if ord in (3,8,14,16) and (.<col5p<80.0 or (compress(col3) in ("0") and denom_flag^=1));
         if ord in (3) then row=3.09;
         else if ord in (8) then row=3.12;
    ...
    %macro print_ivi;
        ...
        *- Get titles and footnotes -;
        ...
        proc report data=print_ivi split='|' style(header)=[backgroundcolor=CXCCCCCC];
             column pg denom_flag col000 col0 col1 ord col3 col4 col5p col5 col6;
             where pg=1;
             ...
             define col0     /display flow "Data Check" style(header)=[just=left cellwidth=5.5%] style(column)=[just=left];
             ...
             define col5p    /display noprint;
             compute col5;
                if ord in (3,8,14,16) and .<col5p<80.0 then do;
                    col5=col5p;
                    call define(_col_, "style", "style=[color=blue fontweight=bold]");
                end;
                else do;
             ...
             endcomp;
             compute col3;
             if ord in (3,8,14,16) and compress(col3) in ("0") and denom_flag^=1
...
data print_ivj;
     ...
     if ord<24 then do;
         ...
         if med>0 and med_den>0 then col3=(med/round((med_den/12),1))*100;

         if disppresent=0 then col4="N/A";
         else col4=strip(put(disp,comma16.));
         if disp>0 and disp_den>0 then col5=(disp/round((disp_den/12),1))*100;

         if obscpresent=0 then col6="N/A";
         else col6=strip(put(obsc,comma16.));
         if obsc>0 and obsc_den>0 then col7=(obsc/round((obsc_den/12),1))*100;
...
%macro print_ivj;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ivj split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define col1a    /display flow "Calendar|Month" style(header)=[just=center cellwidth=8%] style(column)=[just=center];
         define col2     /display flow "Records" style(header)=[just=center cellwidth=13%] style(column)=[just=center];
         define col3     /display flow format=8.1 "Percent of benchmark average" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col4     /display flow "Records" style(header)=[just=center cellwidth=13%] style(column)=[just=center];
         define col5     /display flow format=8.1 "Percent of benchmark average" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col6     /display flow "Records" style(header)=[just=center cellwidth=13%] style(column)=[just=center];
         define col7     /display flow format=8.1 "Percent of benchmark average" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
    run;
...
data current(keep=ord table dc_name statistic resultn);
     set dc_normal(where=(datamartid=%upcase("&dmid") and scan(dc_name,3,"_")="N" and 
                          (variable in ("PATID" "PATRELID") and statistic in ("ALL_N" "DISTINCT_N")) or
...
proc sql noprint;
     select unique(memname) into :_prevdata separated by '|'  from _prevdata
         where memname in ("DEM_L3_N" "ENR_L3_N" "ENC_L3_N" "DIA_L3_N" "PRO_L3_N" 
             ...
             "OBSGEN_L3_N" "HASH_L3_N" "IMMUNE_L3_N" "LDSADRS_L3_N" "LABHIST_L3_N" "EXTMED_L3_N" "PATREL_L3_N");
...
data previous(keep=ord dataset dc_name col: rename=(dataset=table));
     set previous;
     if tag in ("PATID" "PATRELID") or (dataset="PROVIDER" and tag="PROVIDERID");
...
%macro print_va;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_va split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define col4     /computed format=comma16.1 "Percent|Change" style(header)=[just=center cellwidth=12%] style(column)=[just=center];
         ...
         define col7     /computed format=comma16.1 "Percent|Change" style(header)=[just=center cellwidth=12%] style(column)=[just=center];
...
%macro print_vb;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_vb split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define col4     /computed format=comma16.1 "Percent|Change" style(header)=[just=center cellwidth=9%] style(column)=[just=center];
         ...
         define col7     /computed format=comma16.1 "Percent|Change" style(header)=[just=center cellwidth=9%] style(column)=[just=center];
...
proc format;
     invalue type_vc
        ...
        "PRES_L3_RXCUI"=6
        "OBSCLIN_L3_CODE_TYPE"=7
        "PROCM_L3_CODE_TYPE"=8
        "EXTMED_L3_RXCUI"=9
        ;
     ...
     value vc_type
        ...
        2="PRO_L3_PX_PXTYPE"
        3="DISP_L3_NDC"
        4="IMMUNE_L3_CODE_CODETYPE"
        5="MEDADM_L3_CODE_TYPE"
        6="PRES_L3_RXCUI"
        7="OBSCLIN_L3_CODE_TYPE"
        8="PROCM_L3_CODE_TYPE"
        9="EXTMED_L3_RXCUI"
        ;
...
proc means data=dc_normal nway noprint;
     ...
     where datamartid=%upcase("&dmid") and statistic in ("RECORD_N") and 
           ...
           (dc_name in ("MEDADM_L3_CODE_TYPE") and cross_category in ("ND" "RX")) or
           (dc_name in ("OBSCLIN_L3_CODE_TYPE") and cross_category in ("LC")) or
           (dc_name in ("PROCM_L3_CODE_TYPE") and cross_category in ("LC"))) 
     ;
run;
...
proc means data=dc_normal nway noprint;
     ...
     where datamartid=%upcase("&dmid") and statistic in ("RECORD_N") and resultn>0 and 
           ...
           (dc_name in ("PRES_L3_RXCUI") and category not in ("NULL or missing")) or 
           (dc_name in ("EXTMED_L3_RXCUI") and category not in ("NULL or missing")))
     ;
run;
...
data current_ncat;
     ...
     else if dc_name="EXTMED_L3_RXCUI" then category="RX";
     keep dc_name category col3n _freq_;
run;
...
%prevdataloop(pdata=OBSCLIN_L3_CODE_TYPE,type=obsclin_type,
              whr=where obsclin_type in ("LC") and obsclin_code^="NULL or missing")

%prevdataloop(pdata=PROCM_L3_CODE_TYPE,type=pro_type,
              whr=where pro_type in ("LC") and pro_code^="NULL or missing")

%prevdataloop(pdata=EXTMED_L3_RXCUI,type="RX",
              whr=where rxnorm_cui^="NULL or missing")
...
data previous(keep=dc_name ord category col:);
     set prev_pro_l3_px_pxtype
         ...
         prev_pres_l3_rxcui
         prev_obsclin_l3_code_type
         prev_procm_l3_code_type
         prev_extmed_l3_rxcui
     ;
...
data print_vc;
     ...
     if category="00" then col1=strip(put(ord,row_vc.));
          ...
          col1=category;
          if missing(col2) then do;
               col2='0';
               col2n=0;
          end;
          if missing(col3n) then col3n=0;
          if missing(col5) then do;
               col5='0';
               col5n=0;
          end;
          if missing(col6n) then col6n=0;
     end;

     if category ne '00' then col8=strip(put(ord,vc_type.));
...
%macro print_vc;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_vc split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define pg       /order noprint;
         define category /display noprint;
         define col1     /display flow " " style(header)=[just=left cellwidth=13%] style(column)=[just=left];
         define col2     /display flow "Previous|Refresh" style(header)=[just=center cellwidth=10%] style(column)=[just=center];
         define col3     /computed format=comma16. "Current|Refresh" style(header)=[just=center cellwidth=10%] style(column)=[just=center];
         define col4     /computed format=comma16.1 "Percent|Change" style(header)=[just=center cellwidth=10%] style(column)=[just=center];
         define col5     /display flow "Previous|Refresh" style(header)=[just=center cellwidth=10%] style(column)=[just=center];
         define col6     /computed format=comma16. "Current|Refresh" style(header)=[just=center cellwidth=10%] style(column)=[just=center];
         define col7     /computed format=comma16.1 "Percent|Change" style(header)=[just=center cellwidth=10%] style(column)=[just=center];
         define col8     /display flow "Source table" style(header)=[just=left cellwidth=25%] style(column)=[just=left];
         define col2n    /display noprint;
         define col3n    /display noprint;
         define col4n    /display noprint;
         define col5n    /display noprint;
...
data dc_summary;
     ...
     * create color code variable *;
     if exception=1 then do;
        exceptions="Yes";
        if row in (1.01, 1.02, 1.03, 1.04, 1.05, 1.06, 1.07, 1.08, 1.09, 1.10, 1.11, 1.12, 1.13, 1.15, 1.17, 1.18, 1.21, 1.22, 3.04, 3.05) then color = 1; /*Red exceptions*/
        else color = 2; /*All other exceptions blue*/
     end;
     else exceptions="No";

     * set exceptions to n/a if no data in the cdm table *;
     %if &_anylabdata=0 and &_anyobsgendata=0 and &_anyobsclindata=0 %then %do;
        if row in (1.16) then do;
           exception=.; exceptions="n/a"; color=.;
        end;
     %end;
     %if &_anyhashdata=0 %then %do;
        if row in (1.19) then do;
           exception=.; exceptions="n/a"; color=.;
        end;
     %end;
     %if &_anylabdata=0 and &_anyprocmdata=0 and &_anyobsclindata=0 %then %do;
        if row in (1.20) then do;
           exception=.; exceptions="n/a"; color=.;
        end;
     %end;
     %if &_anyprescribingdata=0 %then %do;
        if row in (3.08) then do;
           exception=.; exceptions="n/a"; color=.;
        end;
     %end;
     %if &_anylabdata=0 %then %do;
        if row in (3.09, 3.12) then do;
           exception=.; exceptions="n/a"; color=.;
        end;
     %end;
     %if &_anylabdata=0 and &_anyprescribingdata=0 and &_anyvitaldata=0 %then %do;
        if row in (3.11) then do;
           exception=.; exceptions="n/a"; color=.;
        end;
     %end;
     %if &_anymedadmindata=0 %then %do;
        if row in (3.15) then do;
           exception=.; exceptions="n/a"; color=.;
        end;
     %end;
     %if &_anyobsclindata=0 %then %do;
        if row in (3.16, 3.17) then do;
           exception=.; exceptions="n/a"; color=.;
        end;
     %end;

     if _n_<=14 then pg=1;
     else if _n_<=24 then pg=2;
     else if _n_<=30 then pg=3;
     else if _n_<=37 then pg=4;
     else if _n_<=46 then pg=5;
     else pg=6;
run;
        ...
        * macro to prevent open code *;
        %macro etsprnt;
            %if %upcase(&ets_inst)=YES %then %do;
                ...
                %end;
                %if %upcase(&exp_med_admin)=YES %then %do;
                    %chart_iii_noncat(chart=G,tbl=med_admin,xlbl=MEDADMIN_START_DATE);
                %end;
            %end;
        %mend etsprnt;
        %etsprnt;

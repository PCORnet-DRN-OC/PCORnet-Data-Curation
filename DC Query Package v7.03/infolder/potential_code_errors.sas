...
*  Outputs: 
*    1) Up to 16 SAS datasets stored in /dmlocal containing all records which violate one or more of the heuristics
...
*	 4) [DataMartID]_[Date]_potential_code_errors.cpt stored in /drnoc
...
options validvarname=upcase errors=0 ls=max nomprint;
...
data _null_;
     ...
     call symput("_pstart",left(put(datetime(),datetime20.)));
run;
...
proc sql noprint;
     ...
     select count(*) into :_yexternal_meds from dpath
            where memname="EXTERNAL_MEDS";
     select count(*) into :_ypro_cm from dpath
...
*ELAPSED: Macro to track processing time;
%macro elapsed(b_or_e);
    ...
    %else %if &b_or_e=end %then %do;
        data _qe(drop=_elapsed_time _total_run_time);
            length query $100;
            format elapsedtime totalruntime time8.;
            set _qe;
                _qend=datetime();
                elapsedtime=_qend-_qstart;
                _elapsed_time=put(elapsedtime,time8.);
                totalruntime=_qend-input("&_pstart",datetime20.);
                _total_run_time=put(totalruntime,time8.);
                 put "******************************************************************************************************";
                 put "End query:  %upcase(&qname)  on " _qstart "elapsed time (hh:mm:ss): " _elapsed_time " total run time (hh:mm:ss): " _total_run_time;
                 put "******************************************************************************************************";
        run;
...
data elapsed;
     ...
     query="PCE PROGRAM";
     _qstart=input("&_pstart",datetime20.);
     output;
run;

*****************************************************************************;
* Begin clock for BAD_DX
********************************************************************************;
%let qname=bad_dx;
...
data diagnosis (compress=yes);
	...
	where not missing(dx) and dx_type in ('09','10','SM');
	table='DIAGNOSIS';
	code_type=dx_type;
	code=dx;
...
data dmlocal.bad_dx (drop=dx dx_type);
	set diagnosis;
	*ICD09:length is not between 3-5 OR has alpha characters other than E or V OR has no numeric characters OR first 3 digits (min length) are 0;
	...
	*ICD10: length is not between 3 and 7 OR 1st character is not alpha OR first 3 digits (min length) are 0 or 9 OR has no numeric characters;
	...
	*SNOMED:length is not between 6-18 OR has alpha characters characters OR is all zeros or all nines;
	if code_type='SM' then do;
		if anyalpha(code_clean)>0 then unexp_alpha=1; else unexp_alpha=0;
		if .<code_length<6 or 18<code_length then unexp_length=1; else unexp_length=0; 
		if code_length=countc(code_clean,'0') or code_length=countc(code_clean,'9') then unexp_string=1; else unexp_string=0;
		unexp_numeric=.;
		end;
...
* end elapsed time count *;
%elapsed(end);

*****************************************************************************;
* Begin clock for BAD_CONDITION
********************************************************************************;
%let qname=bad_condition;
...
data condition (compress=yes);
	...
	where not missing(condition) and condition_type in ('09','10','SM');
	table='CONDITION';
...
data procedures(compress=yes);
	...
	code=px;
    code_clean=strip(upcase(compress((code),"., ")));
	if code_type='CH' then code_clean=substr(code_clean,1,5);
	code_length=length(code_clean);
	anyalpha=anyalpha(code_clean);
	anydigit=anydigit(code_clean);
	run;
...
*****************************************************************************;
* Begin clock for BAD_IMMUNIZATION
********************************************************************************;
%let qname=bad_immunization;
...
*****************************************************************************;
* Begin clock for BAD_MEDADMIN
********************************************************************************;
%let qname=bad_medadmin;
...
data lab_result_cm (compress=yes);
	...
	length code_type $4 table $15;
	set 
		dpath.lab_result_cm (in=in1 keep=lab_result_cm_id lab_loinc result_snomed where=(not missing(lab_loinc)))
		dpath.lab_result_cm (in=in2 keep=lab_result_cm_id result_snomed where=(not missing(result_snomed)))
		;
...
data dmlocal.bad_lab (drop=lab_loinc code_clean_r);
	...
	*LOINC: flag for any alphabetical characters OR a length less than 3 or greater than 8 OR the absence of a dash after the next to last position;
	if code_type='LC' then do; 		
		if anyalpha(code_clean)>0 then unexp_alpha=1; else unexp_alpha=0;
		if code_length<3 or code_length>8 then unexp_length=1; else unexp_length=0;
		if substr(code_clean_r,2,1)='-' then unexp_string=0; else unexp_string=1;
		unexp_numeric=.;
		end;
	*SNOMED:length is not between 6-18 OR has alpha characters characters OR is all zeros or all nines;
	else if code_type='SM' then do;
		if anyalpha(code_clean)>0 then unexp_alpha=1; else unexp_alpha=0;
		if .<code_length<6 or 18<code_length then unexp_length=1; else unexp_length=0; 
		if code_length=countc(code_clean,'0') or code_length=countc(code_clean,'9') then unexp_string=1; else unexp_string=0;
		unexp_numeric=.;
...
*****************************************************************************;
* Begin clock for BAD_LAB_HIST
********************************************************************************;
%let qname=bad_lab_hist;
...
data lab_history (compress=yes);
	...
	length code_type $4 table $15;
	set dpath.lab_history (keep=labhistoryid lab_loinc);
...
*****************************************************************************;
* Begin clock for BAD_OBSCLIN
********************************************************************************;
%let qname=bad_obsclin;
...
data obs_clin (compress=yes);
	...
	set 
		dpath.obs_clin (in=in1 keep=obsclinid obsclin_code obsclin_type where=(not missing(obsclin_code) and obsclin_type in ('LC' 'SM')))
		dpath.obs_clin (in=in2 keep=obsclinid obsclin_result_snomed obsclin_type where=(not missing(obsclin_result_snomed) and obsclin_type in ('LC' 'SM')))
		;
...
*****************************************************************************;
* Begin clock for BAD_OBSGEN
********************************************************************************;
%let qname=bad_obsgen;
...
data obs_gen (compress=yes);
	...
	set dpath.obs_gen (keep=obsgenid obsgen_code obsgen_type);
	where not missing(obsgen_code) and obsgen_type in ('LC','ND','RX','09DX','09PX','10DX','10PX','CH','SM');
	table='OBS_GEN';
...
*****************************************************************************;
* Begin clock for BAD_EXT_MEDS
********************************************************************************;
%let qname=bad_ext_meds;

* Begin elapsed time count;
%elapsed(begin);
	
*********************************************************************************************;
* EXTERNAL_MEDS table (RXCUI)
*********************************************************************************************;
...
data external_meds (compress=yes);
	format table extmedid code_type code code_clean code_length anyalpha anydigit;
	length code_type $4 table $15;
	set dpath.external_meds (keep=extmedid rxnorm_cui);
	where not missing(rxnorm_cui);
	table='EXTERNAL_MEDS';
	code_type='RX';
	code=rxnorm_cui;
...
*summarize records by type;
%tblsum(tbl=external_meds);
	
*apply heuristics to identify bad records;	
data dmlocal.bad_ext_meds (drop=rxnorm_cui);
	set external_meds;
	*flag codes with any alphabetical characters OR a length <2 or >7; 
	if anyalpha(code_clean)>0 then unexp_alpha=1; else unexp_alpha=0;
	if code_length<2 or code_length>7 then unexp_length=1; else unexp_length=0;
...
*summarize # of bad records by code type and code and calculate % of total;	
%badsum(ds=bad_ext_meds,tbl=external_meds);
...
data pro_cm (compress=yes);
	format table pro_cm_id code_type code code_clean code_length anyalpha anydigit;
	length code_type $4 table $15;
	set dpath.pro_cm (keep=pro_cm_id pro_code pro_type);
	where not missing(pro_code) and upcase(strip(pro_type))='LC';
	table='PRO_CM';
	code_type='LC';
	code=pro_code;
	code_clean=strip(upcase(compress((code),"., ")));
...
*apply heuristics to identify bad records;	
data dmlocal.bad_procm (drop=pro_code pro_type code_clean_r);
	set pro_cm;
	*flag for any alphabetical characters OR a length less than 3 or greater than 8 OR the absence of a dash after
	the next to last position;
	if anyalpha(code_clean)>0 then unexp_alpha=1; else unexp_alpha=0;
	if code_length<3 or code_length>8 then unexp_length=1; else unexp_length=0;
	if substr(code_clean_r,2,1)='-' then unexp_string=0; else unexp_string=1;
...
*summarize # of bad records by code type and code and calculate % of total;	
%badsum(ds=bad_procm,tbl=pro_cm);
...
*****************************************************************************;
* Begin clock for MISPLACED_LOINCS
********************************************************************************;
%let qname=misplaced_loincs;
...
data dmlocal.misplaced_loincs (keep=table recordid code code_desc classtype expected_classtype);
                ...
                length recordid $1000 code_long $100 expected_classtype $3;
                set 
                
                %if &_mislabcm.=1 %then %do; misplaced_lab_result_cm (in=a rename=(code=code_long)) %end;
                %if &_misobsgen.=1 %then %do; misplaced_obs_gen (in=b rename=(code=code_long)) %end; 
                %if &_misobsclin.=1 %then %do; misplaced_obs_clin (in=c rename=(code=code_long)) %end;
                %if &_mislabhist.=1 %then %do; misplaced_lab_history (in=d rename=(code=code_long)) %end;
                ;
...
*****************************************************************************;
* Begin clock for CODE_SUMMARY
********************************************************************************;
%let qname=code_summary;
...
data code_summary (drop=i);
	...
	set diagnosis_summary 
		...
		%if &_yexternal_meds=1 %then %do; external_meds_summary %end;
		%if &_ypro_cm=1 %then %do; pro_cm_summary %end;
		;
	...
	DATAMARTID="&dmid.";
	QUERY_DATE=left(put("&sysdate9"d,yymmddn8.));
	run;

	proc sort data=code_summary out=dmlocal.code_summary;
		by table code_type;
	run;
...
data elapsed;
	...
	source='ELAPSED_PCE';
    datamartid="&dmid";
    query_package="&pce_version";
    response_date="&sysdate9"d;

    label 
        obs='Obs'
        _qstart="Query start time"
...
data dmlocal.elapsed_pce;
	retain OBS QUERY _QSTART _QEND ELAPSEDTIME TOTALRUNTIME SOURCE DATAMARTID QUERY_PACKAGE RESPONSE_DATE;
    set elapsed;
run;	

*******************************************************************************;
* Create .cpt file containing CODE_SUMMARY and ELAPSED datasets
*******************************************************************************;

proc cport library=dmlocal file="&qpath.drnoc/&dmid._&tday._potential_code_errors.cpt" memtype=data;
     select code_summary elapsed_pce;
run;
...
*******************************************************************************;
* Re-direct to DRN log
*******************************************************************************;
proc printto log=qlog;
run;

title1 'Query run times';
ods proclabel 'Query run times';
proc print width=min data=dmlocal.elapsed_pce label noobs;
	var obs query _qstart _qend elapsedtime totalruntime;
	run;
...
title2 "EXTERNAL_MEDS";
title3 'Top 50 Highest Volume Potentially Erroneous Codes';	
ods proclabel "EXTERNAL_MEDS errors";		
proc print width=min data=bad_ext_meds_sum2(obs=50) label;
	var code_type code records unexp_alpha unexp_length unexp_numeric unexp_string;
	run;	
%end;
...
title2 "PRO_CM";
title3 'Top 50 Highest Volume Potentially Erroneous Codes';	
ods proclabel "PRO_CM errors";		
proc print width=min data=bad_procm_sum2(obs=50) label;
	var code_type code records unexp_alpha unexp_length unexp_numeric unexp_string;
	run;	
%end;

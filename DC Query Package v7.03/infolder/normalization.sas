...
*    Study: PCORnet
*
*  Purpose: To consolidate PCORnet Data Curation Query Package V7.03
...
* Restrict PCORDATA to only CDM datasets;
data pcordata;
    set pcordata;

    if not missing(put(upcase(memname),$ord.));
run;

* Find NOBS separately, as NOBS from PROC CONTENTS is unreliable for SAS views;
* Place all table names into a macro variable;
proc sql noprint;
	select unique memname into :workdata separated by '|'  from pcordata;
	select count(unique memname) into :workdata_count from pcordata;
quit;
...
proc sql noprint;
     select count(*) into :_ycondition from nobs_all
            where memname="CONDITION" and nobs>0;
     select count(*) into :_ydeath from nobs_all
            where memname="DEATH" and nobs>0;
     select count(*) into :_ydeathc from nobs_all
            where memname="DEATH_CAUSE" and nobs>0;
     select count(*) into :_ydemographic from nobs_all
            where memname="DEMOGRAPHIC" and nobs>0;
     select count(*) into :_ydiagnosis from nobs_all
            where memname="DIAGNOSIS" and nobs>0;
     select count(*) into :_ydispensing from nobs_all
            where memname="DISPENSING" and nobs>0;
     select count(*) into :_yencounter from nobs_all
            where memname="ENCOUNTER" and nobs>0;
     select count(*) into :_yenrollment from nobs_all
            where memname="ENROLLMENT" and nobs>0;
     select count(*) into :_yhash_token from nobs_all
            where memname="HASH_TOKEN" and nobs>0;
     select count(*) into :_yharvest from nobs_all
            where memname="HARVEST" and nobs>0;
     select count(*) into :_yimmunization from nobs_all
            where memname="IMMUNIZATION" and nobs>0;
     select count(*) into :_ylab_history from nobs_all
            where memname="LAB_HISTORY" and nobs>0;
     select count(*) into :_ylab_result_cm from nobs_all
            where memname="LAB_RESULT_CM" and nobs>0;
     select count(*) into :_ylab_raw_name from pcordata_all
            where memname="LAB_RESULT_CM" and name="RAW_LAB_NAME";
     select count(*) into :_yldsadrs from nobs_all
            where memname="LDS_ADDRESS_HISTORY" and nobs>0;
     select count(*) into :_ymed_admin from nobs_all
            where memname="MED_ADMIN" and nobs>0;
     select count(*) into :_yobs_clin from nobs_all
            where memname="OBS_CLIN" and nobs>0;
     select count(*) into :_yobs_gen from nobs_all
            where memname="OBS_GEN" and nobs>0;
     select count(*) into :_yprescribing from nobs_all
            where memname="PRESCRIBING" and nobs>0;
     select count(*) into :_yprocedures from nobs_all
            where memname="PROCEDURES" and nobs>0;
     select count(*) into :_ypro_cm from nobs_all
            where memname="PRO_CM" and nobs>0;
     select count(*) into :_yprovider from nobs_all
            where memname="PROVIDER" and nobs>0;
     select count(*) into :_yvital from nobs_all
            where memname="VITAL" and nobs>0;
     select count(*) into :_yptrial from nobs_all
            where memname="PCORNET_TRIAL" and nobs>0;
...
%if %upcase(&_dcpart)=_DCPART1 or %upcase(&_dcpart)=_ALL %then %do;
    ...
    %if &_ydemographic=1 %then %do;
        ...
        %_recordn(dsn=DEM_L3_BIOBANK,type=_recordn,var=BIOBANK_FLAG,addvar=record_n record_pct)
        %_recordn(dsn=DEM_L3_RACEETHMISS,type=_recordn,var=RACE_ETH_MISSING,addvar=record_n record_pct)
        %_recordn(dsn=DEM_L3_RACEETHAIAN,type=_recordn,var=RACE_ETH_AI_AN,addvar=record_n record_pct)
        %_recordn(dsn=DEM_L3_RACEETHASIAN,type=_recordn,var=RACE_ETH_ASIAN,addvar=record_n record_pct)
        %_recordn(dsn=DEM_L3_RACEETHBLACK,type=_recordn,var=RACE_ETH_BLACK,addvar=record_n record_pct)
        %_recordn(dsn=DEM_L3_RACEETHHISPANIC,type=_recordn,var=RACE_ETH_HISPANIC,addvar=record_n record_pct)
        %_recordn(dsn=DEM_L3_RACEETHMENA,type=_recordn,var=RACE_ETH_ME_NA,addvar=record_n record_pct)
        %_recordn(dsn=DEM_L3_RACEETHNHPI,type=_recordn,var=RACE_ETH_NH_PI,addvar=record_n record_pct)
        %_recordn(dsn=DEM_L3_RACEETHWHITE,type=_recordn,var=RACE_ETH_WHITE,addvar=record_n record_pct)
    ...
    %if &_yldsadrs=1 %then %do;
        ...
        %_recordn(dsn=LDSADRS_L3_CURRENT,type=_recordn,var=CURRENT_ADDRESS_FLAG,addvar=record_n record_pct distinct_patid_n)
        %_recordn(dsn=LDSADRS_L3_STATEFIPS,type=_recordn,var=STATE_FIPS,addvar=record_n record_pct)
        %_recordn(dsn=LDSADRS_L3_COUNTYFIPS,type=_recordn,var=COUNTY_FIPS,addvar=record_n record_pct)
        %_recordn(dsn=LDSADRS_L3_RUCAZIP,type=_recordn,var=RUCA_ZIP,addvar=record_n record_pct)
    %end;
    ...
    %if &_ypro_cm=1 %then %do;
        ...
        %_recordn(dsn=PROCM_L3_CODE_TYPE,type=_cross,var=PRO_CODE,scat=PRO_TYPE,addvar=record_n distinct_patid_n)
    %end;
    ...
    %if &_yvital=1 %then %do;
        ...
        %_recordn(dsn=VIT_L3_SMOKING,type=_recordn,var=SMOKING,addvar=record_n record_pct distinct_patid_n)
        %_recordn(dsn=VIT_L3_TOBACCO,type=_recordn,var=TOBACCO,addvar=record_n record_pct distinct_patid_n)
    ...
    %if &_yexternal_meds=1 %then %do;
        %_recordn(dsn=EXTMED_L3_N,type=_n,var=tag,addvar=all_n distinct_n null_n)
        %_recordn(dsn=EXTMED_L3_RXDOSEFORM,type=_recordn,var=EXT_DOSE_FORM,addvar=record_n percent_n)
        %_recordn(dsn=EXTMED_L3_RXDOSE_UNIT,type=_recordn,var=EXT_DOSE_ORDERED_UNIT,addvar=record_n percent_n)
        %_recordn(dsn=EXTMED_L3_ROUTE,type=_recordn,var=EXT_ROUTE,addvar=record_n percent_n)
        %_recordn(dsn=EXTMED_L3_BASIS,type=_recordn,var=EXT_BASIS,addvar=record_n percent_n)
        %_recordn(dsn=EXTMED_L3_SOURCE,type=_recordn,var=EXTMED_SOURCE,addvar=record_n percent_n)
    %end;
    %if &_ypat_relationship=1 %then %do;
        %_recordn(dsn=PATREL_L3_N,type=_n,var=tag,addvar=all_n distinct_n null_n)
        %_recordn(dsn=PATREL_L3_TYPE,type=_recordn,var=RELATIONSHIP_TYPE,addvar=record_n record_pct distinct_patid_1_n distinct_patid_2_n elig_record_n)
    %end;
    ...
    %if &_yharvest=1 %then %do;
        ...
        %_recordn(dsn=XTBL_L3_DATES,type=_xtbl,var=tag,addvar=min p5 median p95 max n nmiss future_dt_n invalid_n)
    ...
    %if &_yhash_token=1 %then %do;
        ...
        %_recordn(dsn=HASH_L3_TOKEN_AVAILABILITY,type=_cross,var=TOKEN_ENCRYPTION_KEY,scat=TOKEN,addvar=distinct_patid_n)
    %end;
...
%if %upcase(&_dcpart)=_DCPART2 or %upcase(&_dcpart)=_ALL %then %do;
    ...
    %if &_yvital=1 %then %do;
        ...
        %_recordn(dsn=VIT_L3_DIASTOLIC,type=_recordn,var=DIASTOLIC_GROUP,addvar=record_n record_pct distinct_patid_n)
        %_recordn(dsn=VIT_L3_SYSTOLIC,type=_recordn,var=SYSTOLIC_GROUP,addvar=record_n record_pct distinct_patid_n)
        %_recordn(dsn=VIT_L3_BMI,type=_recordn,var=BMI_GROUP,addvar=record_n record_pct distinct_patid_n)
    ...
    %if &_ycondition=1 %then %do;
        ...
        %_recordn(dsn=COND_L3_COND_CONDTYPE,type=_cross,var=CONDITION,scat=CONDITION_TYPE,addvar=record_n record_pct distinct_patid_n)
    ...
    %end;
    %if &_ypro_cm=1 %then %do;
        %_recordn(dsn=PROCM_L3_FULLNAME,type=_recordn,var=PRO_FULLNAME,addvar=record_n record_pct)
        %_recordn(dsn=PROCM_L3_NAME,type=_recordn,var=PRO_NAME,addvar=record_n record_pct)
        %_recordn(dsn=PROCM_L3_LOINC,type=_cross,var=PRO_CODE,scat=PANEL_TYPE,addvar=record_n record_pct distinct_patid_n)
    ...
    %if &_yobs_clin=1 %then %do;
        ...
        %_recordn(dsn=OBSCLIN_L3_RESULT_SNOMED,type=_recordn,var=OBSCLIN_RESULT_SNOMED,addvar=record_n record_pct distinct_patid_n)
        ...
        %_recordn(dsn=OBSCLIN_L3_LOINC_RESULT_NUM,type=_recordn,var=OBSCLIN_CODE,addvar=min p1 p5 p25 median p75 p95 p99 max n nmiss)
    %end;
    %if &_ylab_result_cm=1 %then %do;
    ...
    %end;
    %if &_yexternal_meds=1 %then %do;
        %_recordn(dsn=EXTMED_L3_RDATE_Y,type=_recordn,var=EXT_RECORD_DATE,addvar=record_n record_pct distinct_patid_n)
        %_recordn(dsn=EXTMED_L3_RDATE_YM,type=_recordn,var=EXT_RECORD_DATE,addvar=record_n distinct_patid_n)
        %_recordn(dsn=EXTMED_L3_RXDOSE_DIST,type=_stat,var=EXT_DOSE,addvar=record_n)
        %_recordn(dsn=EXTMED_L3_RXCUI,type=_cross,var=RXNORM_CUI,scat=RXNORM_CUI_TTY,addvar=record_n record_pct distinct_patid_n)
        %_recordn(dsn=EXTMED_L3_RXCUI_TIER,type=_recordn,var=RXNORM_CUI_TIER,addvar=record_n percent_n)
    %end;
    %if &_ypat_relationship=1 %then %do;
        %_recordn(dsn=PATREL_L3_SDATE_Y,type=_recordn,var=RELATIONSHIP_START,addvar=record_n record_pct distinct_patid_1_n distinct_patid_2_n)
        %_recordn(dsn=PATREL_L3_SDATE_YM,type=_recordn,var=RELATIONSHIP_START,addvar=record_n distinct_patid_1_n distinct_patid_2_n)
    %end;
    ...
    %end;
    %if &_yencounter=1 and &_yvital=1 %then %do;
        %_recordn(dsn=XTBL_L3_VIT_ENCTYPE,type=_recordn,var=ENC_TYPE,addvar=record_n record_pct distinct_patid_n)
    %end;
    ...
    %if &_yldsadrs=1 and &_yencounter=1 %then %do;
        ...
        %_recordn(dsn=XTBL_L3_COUNTYFIPSC_5Y,type=_n,var=COUNTY_FIPS,addvar=record_n distinct_patid_n)
        %_recordn(dsn=XTBL_L3_STATEFIPSC_5Y,type=_n,var=STATE_FIPS,addvar=record_n distinct_patid_n)
        %_recordn(dsn=XTBL_L3_CFIPSZIP_5Y,type=_n,var=tag,addvar=distinct_patid_n)
        %_recordn(dsn=XTBL_L3_RUCAC_5Y,type=_n,var=RUCA_ZIP,addvar=record_n distinct_patid_n)
        %_recordn(dsn=XTBL_L3_ZIP5C_1Y,type=_n,var=ADDRESS_ZIP5,addvar=record_n distinct_patid_n)
        %_recordn(dsn=XTBL_L3_ZIP5C_5Y,type=_n,var=ADDRESS_ZIP5,addvar=record_n distinct_patid_n)
    %end;
%end;

...
*    Study: PCORnet
*
*  Purpose: Print PCORnet Data Curation Query Package V6.05
* 
...
proc contents data=dmlocal._all_ noprint 
     %if %upcase(&_grp)=MAIN %then %do;
          out=dmlocal(where=((scan(upcase(memname),1,'_') in ("DEATH" "DEATHC" 
              ...
              and scan(upcase(memname),2,'_')="L3") or 
              (scan(upcase(memname),1,'_') in ("DEM") and (scan(upcase(memname),2,'_') in ("OBS")))
              or (upcase(memname)="ELAPSED_MAIN")));
     %end;
     %else %if %upcase(&_grp)=OBSCLIN %then %do;
          out=dmlocal(where=(scan(upcase(memname),1,'_') in ("OBSCLIN") or 
              (upcase(memname)="ELAPSED_OBSCLIN")));
     %end;
     %else %if %upcase(&_grp)=LAB %then %do;
          out=dmlocal(where=(substr(upcase(memname),1,7) in ("LAB_L3_") or 
              substr(upcase(memname),1,11) in ("LABHIST_L3_") or 
     ...
     %else %if %upcase(&_grp)=ALL %then %do;
          out=dmlocal(where=(upcase(memname) not in ("ELAPSED_MAIN" "ELAPSED_LAB"
                                                     "ELAPSED_XTBL" "CODE_SUMMARY"
                                                     "MISPLACED_LOINCS") and 
...
%if &_yencounter=1 %then %do;
    ...
    %prnt(pdsn=enc_l3_adate_y,_obs=100,_svar=admit_date,_suppvar=record_n record_pct distinct_patid_n);
    ...
    %prnt(pdsn=enc_l3_ddate_y,_obs=100,_svar=discharge_date,_suppvar=record_n record_pct distinct_patid_n);
...
%if &_ydiagnosis=1 %then %do;    
    ...
    %prnt(pdsn=dia_l3_adate_y,_obs=100,_svar=admit_date,_suppvar=record_n record_pct distinct_encid_n distinct_patid_n);
    ...
    %prnt(pdsn=dia_l3_dxdate_y,_obs=100,_svar=dx_date,_suppvar=record_n record_pct distinct_encid_n distinct_patid_n);
...
%if &_yprocedures=1 %then %do;
    ...
    %prnt(pdsn=pro_l3_adate_y,_obs=100,_svar=admit_date,_suppvar=record_n record_pct distinct_encid_n distinct_patid_n);
    %prnt(pdsn=pro_l3_adate_ym,_obs=100,_svar=admit_date,_suppvar=record_n);
    %prnt(pdsn=pro_l3_pxdate_y,_obs=100,_svar=px_date,_suppvar=record_n record_pct distinct_encid_n distinct_patid_n);
...
%if &_yvital=1 %then %do;    
    %prnt(pdsn=vit_l3_n);
    %prnt(pdsn=vit_l3_mdate_y,_obs=100,_svar=measure_date,_suppvar=record_n record_pct distinct_patid_n);
...
%if &_ydeath=1 %then %do;
    ...
    %prnt(pdsn=death_l3_date_y,_obs=100,_svar=death_date,_suppvar=record_n record_pct distinct_patid_n);
...
%if &_ydispensing=1 %then %do;
    ...
    %prnt(pdsn=disp_l3_ddate_y,_obs=100,_svar=dispense_date,_suppvar=record_n record_pct distinct_patid_n);
...
%if &_yprescribing=1 %then %do;
    ...
    %prnt(pdsn=pres_l3_odate_y,_obs=100,_svar=rx_order_date,_suppvar=record_n record_pct record_n_rxcui distinct_patid_n);
...
%if &_ymed_admin=1 %then %do;
    ...
    %prnt(pdsn=medadm_l3_sdate_y,_obs=100,_svar=medadmin_start_date,_suppvar=record_n record_pct distinct_patid_n);
    ...
    %prnt(pdsn=medadm_l3_rxcui_tier_5y);
%end;
%if &_ycondition=1 %then %do;
    ...
    %prnt(pdsn=cond_l3_rdate_y,_obs=100,_svar=report_date,_suppvar=record_n record_pct distinct_patid_n);
...
%if &_ypro_cm=1 %then %do;
    ...
    %prnt(pdsn=procm_l3_pdate_y,_obs=100,_svar=pro_date,_suppvar=record_n record_pct distinct_patid_n);
    ...
    %prnt(pdsn=procm_l3_measure_loinc,_obs=100,_svar=pro_measure_loinc,_suppvar=record_n record_pct);
...
%if &_yobs_gen=1 %then %do;
    ...
    %prnt(pdsn=obsgen_l3_sdate_y,_obs=100,_svar=obsgen_start_date,_suppvar=record_n record_pct distinct_patid_n);
...
%if &_yimmunization=1 %then %do;
    ...
    %prnt(pdsn=immune_l3_rdate_y,_obs=100,_svar=vx_record_date,_suppvar=record_n record_pct distinct_patid_n);
    %prnt(pdsn=immune_l3_rdate_ym,_obs=100,_svar=vx_record_date,_suppvar=record_n distinct_patid_n);
    %prnt(pdsn=immune_l3_adate_y,_obs=100,_svar=vx_admin_date,_suppvar=record_n record_pct distinct_patid_n);
...
*- print if OBSCLIN or All -*;
%if %upcase(&_grp)=OBSCLIN or %upcase(&_grp)=ALL %then %do;
...
%if &_yobs_clin=1 %then %do;
    ...
    %prnt(pdsn=obsclin_l3_sdate_y,_obs=100,_svar=obsclin_start_date,_suppvar=record_n record_pct distinct_patid_n);
    %prnt(pdsn=obsclin_l3_sdate_ym,_obs=100,_svar=obsclin_start_date,_suppvar=record_n distinct_patid_n);
    %prnt(pdsn=obsclin_l3_ht);
    %prnt(pdsn=obsclin_l3_ht_dist);
    %prnt(pdsn=obsclin_l3_wt);
    %prnt(pdsn=obsclin_l3_wt_dist);
    %prnt(pdsn=obsclin_l3_diastolic);
    %prnt(pdsn=obsclin_l3_systolic);
    %prnt(pdsn=obsclin_l3_bmi);
...
%if &_ylab_result_cm=1 %then %do;
    ...
    %end;
    %prnt(pdsn=lab_l3_rdate_y,_obs=100,_svar=result_date,_suppvar=record_n record_pct distinct_patid_n);
...
%if &_ylab_history=1 %then %do;
    ...
    %prnt(pdsn=labhist_l3_rlow_dist);
    %prnt(pdsn=labhist_l3_rhigh_dist);
%end;

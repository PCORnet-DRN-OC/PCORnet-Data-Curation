...
*    Study: PCORnet
*
*  Purpose: To consolidate PCORnet Data Curation Query Package v6.03
...
%if &_ydiagnosis=1 %then %do;
    ...
    %_recordn(dsn=DIA_L3_DCGROUP,type=_cross,var=DC_DX_GROUP,scat=PEDI,addvar=distinct_patid_n)
%end;
...
%if &_ylab_result_cm=1 %then %do;
    ...
    %_recordn(dsn=LAB_L3_N_5Y,type=_n,var=tag,addvar=all_n distinct_n null_n)
    ...
    %_recordn(dsn=LAB_L3_LOINC_SOURCE_5Y,type=_cross2,var=LAB_LOINC,scat=SPECIMEN_SOURCE,scat2=EXP_SPECIMEN_SOURCE,addvar=record_n)
    %_recordn(dsn=LAB_L3_RECORDC,type=_n,var=tag,addvar=all_n)
    %_recordn(dsn=LAB_L3_RECORDC_5Y,type=_n,var=tag,addvar=all_n)
    ...
    %_recordn(dsn=LAB_L3_LOINC_RESULT_NUM_5Y,type=_recordn,var=LAB_LOINC,addvar=min p1 p5 p25 median p75 p95 p99 max n nmiss)
...
%if &_ymed_admin=1 %then %do;
    ...
    %_recordn(dsn=MEDADM_L3_RXCUI_TIER_5Y,type=_recordn,var=RXNORM_CUI_TIER,addvar=record_n record_pct)
%end;
...
/*    %_recordn(dsn=OBSCLIN_L3_SNOMED,type=_recordn,var=OBSCLIN_RESULT_SNOMED,addvar=record_n percent_n distinct_patid_n)*/
    ...
    %_recordn(dsn=OBSCLIN_L3_HT,type=_recordn,var=HT_GROUP,addvar=record_n record_pct distinct_patid_n)
    %_recordn(dsn=OBSCLIN_L3_HT_DIST,type=_stat,var=HEIGHT,addvar=record_n)
    %_recordn(dsn=OBSCLIN_L3_WT,type=_recordn,var=WT_GROUP,addvar=record_n record_pct distinct_patid_n)
    %_recordn(dsn=OBSCLIN_L3_WT_DIST,type=_stat,var=WEIGHT,addvar=record_n)
    %_recordn(dsn=OBSCLIN_L3_DIASTOLIC,type=_recordn,var=DIASTOLIC_GROUP,addvar=record_n record_pct)
    %_recordn(dsn=OBSCLIN_L3_SYSTOLIC,type=_recordn,var=SYSTOLIC_GROUP,addvar=record_n record_pct)
    %_recordn(dsn=OBSCLIN_L3_BMI,type=_recordn,var=BMI_GROUP,addvar=record_n record_pct)
...
%if &_yprescribing=1 %then %do;
    ...
    %_recordn(dsn=PRES_L3_RXCUI_5Y,type=_cross,var=RXNORM_CUI,scat=RXNORM_CUI_TTY,addvar=record_n record_pct distinct_patid_n)
    %_recordn(dsn=PRES_L3_RXCUI_TIER,type=_recordn,var=RXNORM_CUI_TIER,addvar=record_n record_pct)
    %_recordn(dsn=PRES_L3_RXCUI_TIER_5Y,type=_recordn,var=RXNORM_CUI_TIER,addvar=record_n record_pct)
...
%if &_yprocedures=1 %then %do;
    ...
    %_recordn(dsn=PRO_L3_DCGROUP,type=_cross,var=DC_PX_GROUP,scat=PEDI,addvar=distinct_patid_n)
%end;
...
%if &_yprovider=1 %then %do;
    ...
    %_recordn(dsn=PROV_L3_SPECIALTY_ENCTYPE,type=_cross2,var=PROVIDER_SPECIALTY_GROUP,scat=PROVIDER_SPECIALTY_PRIMARY,scat2=ENC_TYPE,addvar=record_n)
%end;
...
%if &_ylab_history=1 %then %do;
    ...
    %_recordn(dsn=LABHIST_L3_RHIGH_DIST,type=_stat,var=NORM_RANGE_HIGH,addvar=record_n)
    %_recordn(dsn=LABHIST_L3_RLOW_DIST,type=_stat,var=NORM_RANGE_LOW,addvar=record_n)
%end;
...
%if &_yharvest=1 %then %do;
    ...
    %_recordn(dsn=XTBL_L3_REFRESH_DATE,type=_xtbl,var=nobs,addvar=refresh_date)
%end;

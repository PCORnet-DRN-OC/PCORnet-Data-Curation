...
%clean(savedsn=&savedeath &savedemog &saveenc &savediag &saveproc &saveenr &savevit 
               ...
               obs_gen_obsgen_stop_date xlab_encounter);
...
%clean(savedsn=&savedeath &savedemog &saveenc &savediag &saveproc &saveenr &savevit 
               &savepres &savedisp &savelab &savemedadm &saveobsclin &saveobsgen
               &savecond &saveprocm &savehashtoken &saveldsadrs 
               &saveimmune);
...
*- Get meta-data from DataMart data structures -*;
proc contents data=pcordata._all_ noprint out=datamart;
run;

*- Need only one observation per dataset -*;
proc sort data=datamart(keep=memname nobs rename=(memname=dataset)) nodupkey;
     by dataset;
run;
...
data query;
     ...
     if dataset in ('HARVEST' 'LAB_HISTORY') then delete;

...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet EDC report V7.01
* 
...
%if %index(&_part_mstring,%upcase(&tname))>0 %then %do;
%global nback nsd exp_vital exp_prescribing exp_lab_result_cm exp_med_admin;
...
data dum_field;
     ...
     else if 151<=ordn<=159 then table="EXTERNAL_MEDS";
...
data numerators;
     ...
     if (dc_name in ("VIT_L3_VITAL_SOURCE" "DEATH_L3_IMPUTE" "DEATH_L3_MATCH" "DEATH_L3_SOURCE"
        ...
        (dc_name in ("XTBL_L3_DATES") and statistic="NMISS" and variable in ("LAB_ORDER_DATE" "SPECIMEN_DATE" "DEATH_DATE"
                     ...
                     "VX_ADMIN_DATE" "VX_EXP_DATE" "MEDADMIN_STOP_DATE" "RELATIONSHIP_START" "RELATIONSHIP_END")) or
...
data print_ivd(keep=ord: col: perc thresholdflag);
     ...
     * threshold values for this table *;
     if var in ("VITAL_SOURCE" "DEATH_SOURCE" "RX_ORDER_DATE" "RX_SOURCE" 
                ...
                "ENCOUNTERID" "VX_RECORD_DATE" "VX_CODE_TYPE" "VX_SOURCE" "DISPENSE_SOURCE" 
                "LAB_RESULT_SOURCE" "CONDITION_TYPE" "RX_ENCOUNTERID" "OBSCLIN_ENCOUNTERID"
                "EXT_BASIS" "EXT_RECORD_DATE" "EXT_SOURCE" "DEATH_CAUSE_CODE" "DEATH_CAUSE_SOURCE")
                and perc>=10.00 then thresholdflag=1;
...
%if &_anylabdata>0 or &_anyobsclindata>0 %then %do;
    ...
    *- Keep records resulting in an exception -*;
    data dc_summary_ivi;
         ...
         if ord in (3,7,8,14,16) and (.<col5p<80.0 or (compress(col3) in ("0") and denom_flag^=1));
         if ord in (3) then row=3.09;
         else if ord in (7) then row=3.10;
    ...
    %macro print_ivi;
        ...
        *- Get titles and footnotes -;
        ...
        proc report data=print_ivi split='|' style(header)=[backgroundcolor=CXCCCCCC];
             ...
             define col5p    /display noprint;
             compute col5;
                if ord in (3,7,8,9,14,16) and .<col5p<80.0 then do;
                    col5=col5p;
                    call define(_col_, "style", "style=[color=blue fontweight=bold]");
                end;
                else do;
             ...
             endcomp;
             compute col3;
             if ord in (3,7,8,9,14,16) and compress(col3) in ("0") and denom_flag^=1
...
data dc_summary;
     ...
     * create color code variable *;
     if exception=1 then do;
        exceptions="Yes";
        if row in (1.01, 1.02, 1.03, 1.04, 1.05, 1.06, 1.07, 1.08, 1.09, 1.10, 1.11, 1.12, 1.13, 1.15, 1.17, 1.18, 1.21, 3.04, 3.05) then color = 1;
        else color = 2;
     end;
     else exceptions="No";
     ...
     * set exceptions to n/a if no data in the cdm table *;
     ...
     %end;
     %if &_anylabdata=0 %then %do;
        if row in (3.09, 3.10, 3.12) then do;
           exception=.; exceptions="n/a"; color=.;
        end;
     %end;
     ...
     %end;
     %if &_anydispensingdata=0 and &_anymedadmindata=0 and &_anyobsclindata=0 %then %do;
        if row in (3.14) then do;
           exception=.; exceptions="n/a"; color=.;
        end;
     %end;

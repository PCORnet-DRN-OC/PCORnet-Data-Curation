...
*    Study: PCORnet
*
*  Purpose: To consolidate PCORnet Data Curation Query Package v3.1
...
%if &_ylab_result_cm=1 %then %do;
    ...
    %_recordn(dsn=LAB_L3_LOINC_RUNIT,type=_cross,var=LAB_LOINC,scat=RESULT_UNIT,addvar=record_n)
...
%if &_yprescribing=1 %then %do;
    ...
    %_recordn(dsn=PRES_L3_QTYUNIT,type=_recordn,var=RX_QUANTITY_UNIT,addvar=record_n record_pct)
%end;
...
%if &_ypro_cm=1 %then %do;
    ...
    %_recordn(dsn=PROCM_L3_ITEM,type=_recordn,var=PRO_ITEM,addvar=record_n record_pct distinct_patid_n)
    ...
    %_recordn(dsn=PROCM_L3_LOINC,type=_recordn,var=PRO_LOINC,addvar=record_n record_pct distinct_patid_n)
%end;

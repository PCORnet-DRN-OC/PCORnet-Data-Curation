...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet EDC report v3.11
* 
...
data tbl(keep=ord ord2 col1 resultc dc_name statistic);
     ...
     %cat(dcn=DEM_L3_GENDERDIST,cat='GQ',ord=6,ord2=1,col1=%str(GQ (Genderqueer)))
     %cat(dcn=DEM_L3_GENDERDIST,cat='M',ord=6,ord2=2,col1=%str(M (Man)))
     %cat(dcn=DEM_L3_GENDERDIST,cat='F',ord=6,ord2=3,col1=%str(W (Woman)))
     %cat(dcn=DEM_L3_GENDERDIST,cat=%str('MU, SE, TF, TM'),ord=6,ord2=4,col1=%str(MU (Multiple gender categories), SE (Something else), TF (Transgender female/Trans woman/Male-to-female), or TM (Transgender male/Trans man/Female-to-male)))
     %cat(dcn=DEM_L3_GENDERDIST,cat=%str('Missing, NI, UN or OT'),ord=6,ord2=5,col1=%str(Missing or Refused))
     %cat(dcn=DEM_L3_ORIENTDIST,cat='BI',ord=7,ord2=1,col1=Bisexual)
     %cat(dcn=DEM_L3_ORIENTDIST,cat='GA',ord=7,ord2=2,col1=Gay)
     %cat(dcn=DEM_L3_ORIENTDIST,cat='LE',ord=7,ord2=3,col1=Lesbian)
     %cat(dcn=DEM_L3_ORIENTDIST,cat='QU',ord=7,ord2=4,col1=Queer)
     %cat(dcn=DEM_L3_ORIENTDIST,cat='ST',ord=7,ord2=5,col1=Straight)
     %cat(dcn=DEM_L3_ORIENTDIST,cat=%str('AS, MU, SE, QS'),ord=7,ord2=6,col1=%str(AS (Asexual), MU (Multiple sexual orientations), SE (Something else), QS (Questioning)))
     %cat(dcn=DEM_L3_ORIENTDIST,cat=%str('Missing, NI, UN or OT'),ord=7,ord2=7,col1=%str(Missing or Refused))
run;
...
data data;
     ...
     if category in ("NI" "NULL or missing" "OT" "UN") then category="Missing, NI, UN or OT";
...
data print_ig;
     ...
     merge stats(where=(statistic in ("RECORD_N")) rename=(sum=record))
           ...
           stats(where=(statistic in ("DISTINCT_PATID_N")) rename=(sum=dist_patid)) end=eof
     ;
     by ord;
...
data print_iib(drop=col3);
     ...
     * do not want to highlight these records *;
     ...
     if col1^ in ("LAB_RESULT_CM" "HARVEST" "CONDITION" "DEATH_CAUSE" "PRO_CM") 
        then pg=1;
     else if col1^ in ("PRO_CM") then pg=2;
     else pg=3;
run;
...
*- Import transport file of Data Curation Query Package required structure -*;
...
*- Keep records resulting in an exception -*;
data dc_summary_iie;
     set print_iie(keep=c6 c7 col1);
     if col1^=" " and (c6^=. or c7^=.);
     row=input(col1,8.2);
     exception=1;
     keep row exception;
run;
...
proc format cntlout=r_field;
     ...
     value $r_source
        ...
        "06"="XTBL_L3_DATES"
        "07"="XTBL_L3_TIMES"
        "08"="ENC_L3_ENCTYPE_DDATE_YM"
        "09"="XTBL_L3_TIMES"
        "10"="ENC_L3_ENCTYPE"
        "11"="ENC_L3_N"
        "12"="ENC_L3_N"
        "13"="ENC_L3_ENCTYPE_DISDISP"
        "14"="ENC_L3_ENCTYPE_DISSTAT"
        "15"="ENC_L3_ENCTYPE_DRG"
        "16"="ENC_L3_ENCTYPE_ADMSRC"
        "17"="DIA_L3_DXTYPE_DXSOURCE"
        "18"="DIA_L3_DXSOURCE"
        "19"="DIA_L3_ORIGIN"
        "20"="DIA_L3_PDX_ENCTYPE"
        "21"="PRO_L3_PXDATE_Y"
        "22"="PRO_L3_PXTYPE_ENCTYPE"
        "23"="PRO_L3_PXSOURCE"
        ;
run;
...
data dum_field;
     ...
     ord=strip(label);
     ordn=input(ord,2.);
     if 1<=ordn<=6 then table="DEMOGRAPHIC";
     else if 7<=ordn<=7 then table="ENROLLMENT";
     else if 8<=ordn<=16 then table="ENCOUNTER";
     else if 17<=ordn<=20 then table="DIAGNOSIS";
     else if 21<=ordn<=23 then table="PROCEDURES";
...
data denominators;
     set dc_normal(where=(datamartid=%upcase("&dmid") and 
                ...
                (dc_name in ("ENC_L3_ENCTYPE_DDATE_YM" "ENC_L3_ENCTYPE_DISDISP" "ENC_L3_ENCTYPE_DISSTAT" 
                             "ENC_L3_ENCTYPE_DRG" "ENC_L3_ENCTYPE_ADMSRC" "DIA_L3_PDX_ENCTYPE") 
...
data print_ivc(keep=ord col: perc thresholdflag);
     ...
     col1=table;
     col2=var;
     if ord in ("08" "13" "14" "15" "16" "20") then col3="IP or EI";
     else col3=" ";
     ...
     if ord in ('01' '03' '08' '13' '21') and perc>=10.00 then thresholdflag=1;
     col7=dc_name;
run;
...
proc format cntlout=o_field;
     value $o_field
        ...
        "ONSET_DATE"="40"
        "CONDITION_STATUS"="41"
        "DEATH_CAUSE_CONFIDENCE"="42"
        "PROCM_ENCOUNTERID"="43"
        "PRO_LOINC"="44"
        "PRO_TIME"="45"
        "PRO_METHOD"="46"
        "PRO_MODE"="47"
...
data dum_field;
     ...
     else if 3<=ordn<=5 then table="DEATH";
     else if 6<=ordn<=24 then table="LAB_RESULT_CM";
     else if 25<=ordn<=34 then table="PRESCRIBING";
     else if 35<=ordn<=36 then table="DISPENSING";
     else if 37<=ordn<=41 then table="CONDITION";
     else if 42<=ordn<=42 then table="DEATH_CAUSE";
     else if 43<=ordn<=48 then table="PRO_CM";
...
data numerators;
     length var $50;
     set dc_normal(where=(datamartid=%upcase("&dmid")));
     if (dc_name in ("VIT_L3_VITAL_SOURCE" "DEATH_L3_IMPUTE" "DEATH_L3_MATCH" 
                     ...
                     "DISP_L3_SUPDIST2" "COND_L3_STATUS" "DEATHC_L3_CONF" 
        ...
        (dc_name in ("XTBL_L3_DATES") and statistic="NMISS" and variable in ("LAB_ORDER_DATE" "SPECIMEN_DATE" "RESULT_DATE" "DEATH_DATE"
...
data print_ivd(keep=ord col: perc thresholdflag);
     ...
     * threshold values for this table *;
     if ord in ('27' '36') and perc>=10.00 then thresholdflag=1;
...
data print_ive(keep=col: perc: thresholdflag: exceptionflag);
     ...
     if category in ("EI" "IP") and (compress(col3) in ("0" "BT")) and pat_p>0 then exceptionflag=1;
...
%macro print_ive;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ive split='~' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         compute col3;
            if compress(col3) in ("0" "BT") and compress(col3)^="0" then call define(_col_, "style", "style=[color=blue]");
         endcomp;
         compute col4;
            if thresholdflag=1 then do;
                col4=perc;
...
data months;
     ...
     do i = 0 to -23 by -1;
        ...
        do dc_name = "ENC_L3_ENCTYPE_ADATE_YM", "DIA_L3_ENCTYPE_ADATE_YM", 
                     "PRO_L3_ENCTYPE_ADATE_YM", "VIT_L3_MDATE_YM", 
                     "DEATH_L3_DATE_YM";
           output;
        end;
     end;
...
data data;
     set dc_normal(keep=dc_name statistic category resultn cross_category 
                 ...
                 (statistic="RECORD_N" and dc_name in ("ENC_L3_ENCTYPE_ADATE_YM" "DIA_L3_ENCTYPE_ADATE_YM" 
                                                       "PRO_L3_ENCTYPE_ADATE_YM")) or
                 (resultn>=0 and category not in ("NULL or missing" "Values outside of CDM specifications") 
                  and statistic="RECORD_N" and dc_name in ("VIT_L3_MDATE_YM")) or
                 (resultn>=0 and category not in ("NULL or missing" "Values outside of CDM specifications") 
                  and statistic="DISTINCT_PATID_N" and dc_name="DEATH_L3_DATE_YM")));

     if dc_name not in ("VIT_L3_MDATE_YM" "DEATH_L3_DATE_YM") then 
            date=mdy(input(scan(cross_category,2,'_'),2.),1,input(scan(cross_category,1,'_'),4.));
     else if dc_name in ("VIT_L3_MDATE_YM" "DEATH_L3_DATE_YM") then 
...
*- Bring everything together -*;
data print_ivf;
     length col1 col2 col4 col6 col8-col11 $35;
     merge stats(where=(dc_name="ENC_L3_ENCTYPE_ADATE_YM") rename=(sum=enc denom=enc_den bt_flag=bt_enc))
           ...
           stats(where=(dc_name="VIT_L3_MDATE_YM") rename=(sum=vit denom=vit_den bt_flag=bt_vit))
           stats(where=(dc_name="DEATH_L3_DATE_YM") rename=(sum=dth denom=dth_den bt_flag=bt_dth))
     ;
     by ord;
     ...
     if ord<24 then do;
         ...
         if vit=. and bt_vit="*" then col6=strip(put(.t,threshold.));
         else col8=strip(put(vit,comma16.))||strip(bt_vit);
         if vit>0 and vit_den>0 then col9=strip(put((vit/round((vit_den/12),1))*100,16.1));

         if dth=. and bt_dth="*" then col10=strip(put(.t,threshold.));
         else col10=strip(put(dth,comma16.))||strip(bt_dth);
         if dth>0 and dth_den>0 then col11=strip(put((dth/round((dth_den/12),1))*100,16.1));
         ...
         if vit=. and bt_vit="*" then col6=strip(put(.t,threshold.));
         else col8=strip(put(vit/12,comma16.))||strip(bt_vit);

         if dth=. and bt_dth="*" then col10=strip(put(.t,threshold.));
         else col10=strip(put(dth/12,comma16.))||strip(bt_dth);
         output;
         ord=11.5;
         ...
         col7=" ";
         col8="VIT_L3_\line MDATE_YM";
         col9=" ";
         col10="DEATH_L3_\line DATE_YM";
         col11=" ";
         output;
     end;
run;
...
data dc_summary_ivf;
     ...
     if ord in (3) and (compress(col2) in ("0" "BT") or .<col3p<75.0);
     row=3.07;
     exception=1;
     keep row exception;
run;
...
%macro print_ivf;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ivf split='~' style(header)=[backgroundcolor=CXCCCCCC];
         column pg ord col3p col1 
                ("Ambulatory, ED, Inpatient or ED-to-Inpatient encounters~________________________" col2 col3)
                ("Ambulatory, ED, Inpatient or ED-to-Inpatient diagnoses~________________________" col4 col5p)
                ("Ambulatory, ED, Inpatient or ED-to-Inpatient procedures~________________________" col6 col7p)
                ("Vitals~________________________" col8 col9) ("Deaths~________________________" col10 col11);
         where pg=1;

         define pg       /order noprint;
         define ord      /display noprint;
         define col3p    /display noprint;
         define col1     /display flow "Month" style(header)=[just=center cellwidth=10%] style(column)=[just=left];
         define col2     /display flow "Records" style(header)=[just=center cellwidth=9.5%] style(column)=[just=center];
         define col3     /computed flow format=8.1 "Percent of benchmark average" style(header)=[just=center cellwidth=8%] style(column)=[just=center];
         define col4     /display flow "Records" style(header)=[just=center cellwidth=9.5%] style(column)=[just=center];
         define col5p    /display flow format=8.1 "Percent of benchmark average" style(header)=[just=center cellwidth=8%] style(column)=[just=center];
         define col6     /display flow "Records" style(header)=[just=center cellwidth=9.5%] style(column)=[just=center];
         define col7p    /display flow format=8.1 "Percent of benchmark average" style(header)=[just=center cellwidth=8%] style(column)=[just=center];
         define col8     /display flow "Records" style(header)=[just=center cellwidth=9.5%] style(column)=[just=center];
         define col9     /display flow "Percent of benchmark average" style(header)=[just=center cellwidth=8%] style(column)=[just=center];
         define col10    /display flow "Patients" style(header)=[just=center cellwidth=9.5%] style(column)=[just=center];
         define col11    /display flow "Percent of benchmark average" style(header)=[just=center cellwidth=8%] style(column)=[just=center];
         compute col2;
    ...
    proc report data=print_ivf split='~' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define ord      /display noprint;
         define col3p    /display noprint;
         define col1     /display flow "Month" style(header)=[just=left cellwidth=10%] style(column)=[just=left];
         define col2     /display flow "Records" style(header)=[just=center cellwidth=9.5%] style(column)=[just=center];
         define col3     /computed flow format=8.1 "Percent of benchmark average" style(header)=[just=center cellwidth=8%] style(column)=[just=center];
         define col4     /display flow "Records" style(header)=[just=center cellwidth=9.5%] style(column)=[just=center];
         define col5p    /display flow format=8.1 "Percent of benchmark average" style(header)=[just=center cellwidth=8%] style(column)=[just=center];
...
*- Keep records resulting in an exception -*;
data dc_summary_ivg;
     set print_ivg(keep=ord col4 col5p);
...
proc format;
     value known_desc
       ...
       2 = 'Number of distinct LAB_NAMEs'
       3 = 'Percentage of results mapped to a known LAB_NAME or LAB_LOINC'        
       4 = 'Percentage of results mapped to a known LAB_NAME or LAB_LOINC with a known result'
       5 = 'Number of quantitative results for tests mapped to LAB_NAME or LAB_LOINC'
       6 = 'Percentage of quantitative results for tests mapped to LAB_NAME or LAB_LOINC which fully specify the normal range.'
       ;

     value known_crit
       1 = " "
       2 = "LAB_NAME is a common measure*"
       3 = "LAB_NAME is a common measure* or LAB_LOINC is not null"
       4 = 'LAB_NAME is a common measure* or LAB_LOINC is not null and RESULT_NUM is not null or RESULT_QUAL is in ("BORDERLINE", "POSITIVE", "NEGATIVE" or "UNDETERMINED")'
       5 = "LAB_NAME is a common measure* or LAB_LOINC is not null and RESULT_NUM is not null"
       6 = "LAB_NAME is a common measure* or LAB_LOINC is not null and RESULT_NUM is not null and NORM_MODIFIER_LOW, NORM_RANGE_LOW, NORM_MODIFIER_HIGH, and NORM_RANGE_HIGH are all populated per CDM specifications.** "
       ;
run;
...
proc means data=dc_normal nway noprint;
     class dc_name;
     var resultn;
     output out=name n=resultn;
     where datamartid=%upcase("&dmid") and dc_name="LAB_L3_NAME" and statistic="RECORD_N" and 
           category not in ("NI" "UN" "OT" "NULL or missing" "Values outside of CDM specifications");
run;
...
*- Keep records resulting in an exception -*;
data dc_summary_ivh;
     set print_ivh(keep=ord col5p col3);
...
proc report data=dc_summary split='~' style(header)=[backgroundcolor=CXCCCCCC];
     column pg edc_table_s_ data_check data_check_description category type color exceptions;
     where pg=1;
...
proc report data=dc_summary split='~' style(header)=[backgroundcolor=CXCCCCCC];
     ...
     define pg                /order order=data noprint;
     define edc_table_s_      /display flow "EDC Table(s)" style(header)=[just=left cellwidth=10%] style(column)=[just=left];
     define data_check        /display flow "Data Check" style(header)=[just=center cellwidth=8%] style(column)=[just=center];
     define data_check_description /display flow "Data Check Description" style(header)=[just=left cellwidth=47%] style(column)=[just=left];
     define category          /display flow "Category" style(header)=[just=left cellwidth=15%] style(column)=[just=left];
     define type              /display flow "Type" style(header)=[just=left cellwidth=8%] style(column)=[just=left];

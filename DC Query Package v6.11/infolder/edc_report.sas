...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet EDC report v6.11
* 
...
proc report data=toc split='|' style(header)=[backgroundcolor=CXCCCCCC] contents=' ';
     ...
     where pg=1
        %if %upcase(&_part1)=YES and %upcase(&_part2)^=YES %then %do;
            and upcase(dcpart_macro_value)="_PART1";
        %end;
        %else %if %upcase(&_part1)^=YES and %upcase(&_part2)=YES %then %do;
            and upcase(dcpart_macro_value)="_PART2";
        %end;
     ;
...
data tbl;
     ...
     if ord=0 then do; col4=" "; threshold=.; end;
         ...
         if resultn^=.t and denom_dem345>0 then col4=strip(put((resultn/denom_dem345)*100,comma16.1));
         else col4=" ";
     end;
run;

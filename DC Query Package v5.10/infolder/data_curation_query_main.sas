...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package V5.10 - all queries,
...
data procedures(compress=yes);
     ...
     * remove possible decimal and comma characters and restrict CH to $5 *;     
     if px_type^="CH" then px=strip(upcase(compress(px,',.')));
     else px=strip(substr(upcase(compress(px,'.,')),1,5));
...
proc sort data=procedures(keep=px px_type patid) out=data;
     by px px_type;
run;
...
*- Derive appropriate counts and variables -*;
data query;
     length record_n distinct_patid_n $20 obsgen_type $30;
     merge stats
           pid_unique(rename=(_freq_=distinct_patid));
     by col1 obsgentype;
...
proc contents data=pcordata.pro_cm out=_procm_length noprint;
run;

data _null_;
     set _procm_length;
    
     if name="PRO_ITEM_FULLNAME" then call symput("_item_full_length",strip(put(length,8.)));
     else if name="PRO_ITEM_NAME" then call symput("_item_name_length",strip(put(length,8.)));
     else if name="PRO_MEASURE_FULLNAME" then call symput("_meas_full_length",strip(put(length,8.)));
     else if name="PRO_MEASURE_NAME" then call symput("_meas_name_length",strip(put(length,8.)));
run;
...
data data;
     ...
     if address_state in ("AK" "AL" "AR" "AZ" "CA" "CO" "CT" "DC" "DE" "FL" "GA" 
                          "HI" "IA" "ID" "IL" "IN" "KS" "KY" "LA" "MA" "MD" "ME" 
                          "MI" "MN" "MO" "MS" "MT" "NC" "ND" "NE" "NH" "NJ" "NM"
                          "NV" "NY" "OH" "OK" "OR" "PA" "PR" "RI" "SC" "SD" "TN"
                          "TX" "UT" "VA" "VT" "WA" "WI" "WV" "WY")
                          then col1=address_state;

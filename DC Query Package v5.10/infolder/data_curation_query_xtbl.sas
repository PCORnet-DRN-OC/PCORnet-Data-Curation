...
proc sql noprint;
     ...
     select max(length) into :maxlength_prov from sashelp.vcolumn 
        where upcase(libname)="PCORDATA" and upcase(name)="PROVIDERID";
...
proc sort data=&idsn(keep=patid &mvar &var2) out=&idsn&var;
     by patid &mvar;
run;

proc sort data=&mdsn(keep=patid &mvar &var) out=&mdsn&var;
     by patid &mvar;
run;
...
data data;    
     length networkid network_name datamartid datamart_name query_package tag 
            ...
            medadmin_stop_date_mgmt obsclin_date_mgmt obsgen_date_mgmt 
...
proc transpose data=data out=query(rename=(_name_=name col1=value));
     var networkid network_name datamartid datamart_name datamart_platform
         ...
         refresh_hash_token_date refresh_lds_address_hx_date
         refresh_immunization_date refresh_provider_date refresh_max 
         low_cell_cnt operating_system 

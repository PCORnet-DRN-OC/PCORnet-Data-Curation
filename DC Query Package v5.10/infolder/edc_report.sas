...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet EDC report v5.10
* 
...
%macro clean(savedsn);
     proc datasets  noprint;
          save dc_normal toc headers footers chart_months dc_summary missingness
               required_structure q2_stat_dlg_loinc tbl_ivi_ref &savedsn / memtype=data;
...
data _null_;
     ...
     if dc_name="XTBL_L3_METADATA" and 
        variable in ("LOW_CELL_CNT" "QUERY_PACKAGE" "REFRESH_MAX" "RESPONSE_DATE" 
                     "LOOKBACK_DATE" "CDM_VERSION" "SAS_ETS");
     ...
     else if variable="CDM_VERSION" then call symput("cdm_ver",strip(resultc));
...
title9 justify=left h=2.5 "CDM Version:  &cdm_ver";
title10 justify=left h=2.5 "SAS_ETS:  &ets";
...
proc format;
     ...
     value met_desc
        1='Number of unique patients with at least 1 ED, EI, IP, OS, or AV encounter within the past 5 years'
        2='Number of unique patients with at least 1 ED, EI, IP, OS, or AV encounter within the past 1 year'
...
data data;
     ...
     if (table="DISPENSING" and variable="PRESCRIBINGID") or (table in ("DIAGNOSIS" "PROCEDURES") and variable="PROVIDERID") or 
         dc_name in ("LAB_L3_N_5Y") then delete;
...
proc sort data=dc_normal(keep=datamartid dc_name table statistic result: variable)
     ...
     where datamartid=%upcase("&dmid") and dc_name="XTBL_L3_DATES" and 
        variable not in ("DISCHARGE_DATE" "ENR_END_DATE" "DX_DATE" "PX_DATE" 
           ...
           "ADDRESS_PERIOD_END" "VX_ADMIN_DATE" "VX_EXP_DATE");
run;
...
data print_id;
     ...
     if table^ in ('DEATH_CAUSE' 'HASH_TOKEN' 'LDS_ADDRESS_HISTORY' 'PROVIDER' 'PCORNET_TRIAL') then do;
        col6=variable;
     ...
     else if table in ('DEATH_CAUSE' 'HASH_TOKEN' 'LDS_ADDRESS_HISTORY' 'PROVIDER' 'PCORNET_TRIAL') then do;
        col6="---";
        col7="---";
        col8="---";
...
data print_if(keep=col:);
     ...
     set dc_normal(where=(datamartid=%upcase("&dmid") and dc_name="XTBL_L3_METADATA"
         ...
         "OBSGEN_DATE_MGMT" "DX_DATE_MGMT" "ADDRESS_PERIOD_START_MGMT" 
...
%macro print_ig;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ig split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define col1     /display flow "DC_LAB_GROUP" style(header)=[just=left cellwidth=23%] style(column)=[just=left];
         define col2     /display flow "Records" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col3     /display flow "Percentage of|records in the|LAB_RESULT_CM table|with a LAB_LOINC code" style(header)=[just=center cellwidth=17%] 
                                    style(column)=[just=center];
         define col4     /display flow "Patients" style(header)=[just=center cellwidth=10%] style(column)=[just=center];
         define col5     /display flow "Percentage of|patients in the ENCOUNTER table" style(header)=[just=center cellwidth=17%] 
                                    style(column)=[just=center];
         define col6     /display flow "Source tables" style(header)=[just=left cellwidth=20%] style(column)=[just=left];
...
*- If non-zero results exists, produce chart -*;
%if &_c10data^= and &_c10data^=0 %then %do;
...
data tbl(keep=ord: col:);
     ...
     %harvest(dcn=XTBL_L3_METADATA,ord=11,ord2=26,var=OBSCLIN_DATE_MGMT)
     %harvest(dcn=XTBL_L3_METADATA,ord=11,ord2=27,var=OBSGEN_DATE_MGMT)
...
data print_iib(drop=col3);
     ...
     * do not want to highlight these records *;
     ...
     if col1 in ("DEMOGRAPHIC" "ENROLLMENT" "DEATH" "ENCOUNTER" "DIAGNOSIS"
             ...
             col1 in ("CONDITION" "DEATH_CAUSE" "PRO_CM" "PROVIDER" "MED_ADMIN" 
                      "LDS_ADDRESS_HISTORY") then pg=4;
     else pg=5;
run;
...
proc format;
    ...
    value except
         ...
         9.03='IMMUNIZATION, LAB_RESULT_CM,'
         9.04='MED_ADMIN, OBS_CLIN, OBS_GEN,'
...
data denom;
     merge dc_normal(where=(datamartid=%upcase("&dmid") and dc_name="DIA_L3_N" and variable="ENCOUNTERID" and statistic="DISTINCT_N") 
           ...
           dc_normal(where=(datamartid=%upcase("&dmid") and dc_name="MEDADM_L3_N" and variable="MEDADMIN_PROVIDERID" and statistic="DISTINCT_N") 
                        rename=(resultn=prov_medadm))
           dc_normal(where=(datamartid=%upcase("&dmid") and dc_name="OBSCLIN_L3_N" and variable="OBSCLIN_PROVIDERID" and statistic="DISTINCT_N") 
                        rename=(resultn=prov_obsclin))
           dc_normal(where=(datamartid=%upcase("&dmid") and dc_name="OBSGEN_L3_N" and variable="OBSGEN_PROVIDERID" and statistic="DISTINCT_N") 
                        rename=(resultn=prov_obsgen))
...
data final;
     ...
     if row=8 and resultn>=0 then do;
        ...
        else if col4="IMMUNIZATION" and pat_immune>0 then col7=strip(put((nonunique/pat_immune)*100,16.1));
     end;
...
data tbl(keep=ord: col: dc_name resultn);
     ...
     %stat(dcn=XTBL_L3_DATES,ord=18,ord2=32,tbl=OBS_CLIN,var=OBSCLIN_DATE,stat=FUTURE_DT_N)
     %stat(dcn=XTBL_L3_DATES,ord=19,ord2=33,tbl=OBS_GEN,var=OBSGEN_DATE,stat=FUTURE_DT_N)
     %stat(dcn=XTBL_L3_DATES,ord=21,ord2=34,tbl=LDS_ADDRESS_HISTORY,var=ADDRESS_PERIOD_START,stat=FUTURE_DT_N)
     %stat(dcn=XTBL_L3_DATES,ord=21,ord2=35,tbl=LDS_ADDRESS_HISTORY,var=ADDRESS_PERIOD_END,stat=FUTURE_DT_N)
     %stat(dcn=XTBL_L3_DATES,ord=22,ord2=36,tbl=IMMUNIZATION,var=VX_RECORD_DATE,stat=FUTURE_DT_N)
     %stat(dcn=XTBL_L3_DATES,ord=22,ord2=37,tbl=IMMUNIZATION,var=VX_ADMIN_DATE,stat=FUTURE_DT_N)
run;
...
data den(keep=ord: denom);
     ...
     %denom(dcn=XTBL_L3_DATES,ord=18,ord2=32,tbl=OBS_CLIN,var=OBSCLIN_DATE)
     %denom(dcn=XTBL_L3_DATES,ord=19,ord2=33,tbl=OBS_GEN,var=OBSGEN_DATE)
     %denom(dcn=XTBL_L3_DATES,ord=21,ord2=34,tbl=LDS_ADDRESS_HISTORY,var=ADDRESS_PERIOD_START)
     %denom(dcn=XTBL_L3_DATES,ord=21,ord2=35,tbl=LDS_ADDRESS_HISTORY,var=ADDRESS_PERIOD_END)
     %denom(dcn=XTBL_L3_DATES,ord=22,ord2=36,tbl=IMMUNIZATION,var=VX_RECORD_DATE)
     %denom(dcn=XTBL_L3_DATES,ord=22,ord2=37,tbl=IMMUNIZATION,var=VX_ADMIN_DATE)
run;
...
proc format;
     invalue rowfmt
        ...
        'VX_RECORD_DATE < BIRTH_DATE'=12
        'ADMIT_DATE > DEATH_DATE'=13
        'DISCHARGE_DATE > DEATH_DATE'=14
        'PX_DATE > DEATH_DATE'=15
        'MEASURE_DATE > DEATH_DATE'=16
        'DISPENSE_DATE > DEATH_DATE'=17
        'RX_START_DATE > DEATH_DATE'=18
        'RESULT_DATE > DEATH_DATE'=19
        'MEDADMIN_START_DATE > DEATH_DATE'=20
        'OBSCLIN_DATE > DEATH_DATE'=21
        'OBSGEN_DATE > DEATH_DATE'=22
        'VX_RECORD_DATE > DEATH_DATE'=23
        'ADMIT_DATE > DISCHARGE_DATE'=24
        'PX_DATE < ADMIT_DATE'=25
        'PX_DATE > DISCHARGE_DATE'=26
    ;
...
data print_iiic;
     ...
     * create column variables and output *;
     if ord<25 then col1=strip(category);
     else if ord=25 then col1='PX_DATE is More Than 5 Days Prior To The ADMIT_DATE';
     else if ord=26 then col1='PX_DATE is More Than 5 Days After The DISCHARGE_DATE';
     if bt_dist_patid="*" then col2=strip(put(.t,threshold.));
     else col2=strip(put(dist_patid,comma16.));
...
*- Bring everything together -*;
data print_iiif(keep=col: statthresholdflag thresholdflag) anyout(keep=col0);
     length col0-col13 $50;
     merge data(in=d)
           _quant_loinc(in=q)           
     ;
     by lab_loinc;
...
%macro print_iiif;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_iiif split='|' style(header)=[backgroundcolor=CXCCCCCC];
         column pg statthresholdflag thresholdflag col0 col1 col2 
                ...
                ("DataMart Results|_____________________________________________" col9 col10 col11 col12 col13);

         define pg       /order noprint;
         define statthresholdflag /display noprint;
         define thresholdflag /display noprint;
         define col0     /display flow "Data Curation|Lab Group" style(header)=[just=left cellwidth=12%] style(column)=[just=left];
         define col1     /display flow "LAB|LOINC" style(header)=[just=center cellwidth=6%] style(column)=[just=center];
         define col2     /display flow "EX_UCUM_UNITS" style(header)=[just=center cellwidth=6%] style(column)=[just=center];
         define col3     /display flow "Clinical|Lower|Bound" style(header)=[just=center cellwidth=6%] style(column)=[just=center];
         define col4     /display flow "Clinical|Upper|Bound" style(header)=[just=center cellwidth=8%] style(column)=[just=center];
         ...
         define col11    /display flow "Statistical|Outlier" style(header)=[just=center cellwidth=7%] style(column)=[just=center];
         define col12    /display flow "Outside|Normal|Range" style(header)=[just=center cellwidth=6%] style(column)=[just=center];
         define col13    /display flow "Clinical|Outlier" style(header)=[just=center cellwidth=6%] style(column)=[just=center];
         compute col11;
...
data print_iiig;
     ...
     * drop data outside limits of interest *;    
     if lag_avg<500 or lag_std<=0 or .<month<=p5_date or month>month11_date then delete;
     ...
     * keep only months where no obs or beyond the limit of std deviations *;
     ...
     col1=strip(table);
     col2=put(month(p5_date),mnth.)||", "||put(year(p5_date),4.)||" - "||
...
%else %do;
    data print_iiig;
        ...
        dsd=.;
        pg=1;
        col1="No Data Table contains a Data Check Exception";
        output;
    run;
%end;
%mend check;
...
%if &_ch_iii^= and &_ch_iii^=0 %then %do;
    ...
    *- Calculate deviations from the mean -*;
    ...
    data chart_iii&chart._anno;
         ...
         * drop data outside limits of interest *;    
         if lag_avg<500 or lag_std<=0 then delete;
...
proc format cntlout=r_field;
     ...
     value $r_source
        ...
        "24"="XTBL_L3_DATES"
        "25"="DIA_L3_DXTYPE_DXSOURCE"
        "26"="DIA_L3_DXSOURCE"
        "27"="DIA_L3_ORIGIN"
        "28"="DIA_L3_PDX_ENCTYPE"
        "29"="DIA_L3_N"
        "30"="DIA_L3_DXPOA"
        "31"="DIA_L3_N"
        "32"="XTBL_L3_DATES"
        "33"="PRO_L3_PXTYPE_ENCTYPE"
        "34"="PRO_L3_PXSOURCE"
        "35"="PRO_L3_N"
        "36"="PRO_L3_PPX"
        "37"="PRO_L3_N"
...
data dum_field;
     ...
     else if 24<=ordn<=31 then table="DIAGNOSIS";
     else if 32<=ordn<=37 then table="PROCEDURES";
...
proc format cntlout=o_field;
     value $o_field
        ...
        "CONDITION_STATUS"="56"
        "CONDITION_SOURCE"="57"        
        "DEATH_CAUSE_CONFIDENCE"="58"
        "PROCM_ENCOUNTERID"="59"
        "PRO_ITEM_LOINC"="60"
        "PRO_DATE"="61"
        "PRO_TIME"="62"
        "PRO_METHOD"="63"
        ...
        "PRO_TYPE"="66"
        "PRO_ITEM_NAME"="67"
        "PRO_ITEM_FULLNAME"="68"
        "PRO_MEASURE_NAME"="69"
        "PRO_MEASURE_FULLNAME"="70"
        "PRO_SOURCE"="71"        
        "PROVIDER_NPI"="72"
        "PROVIDER_NPI_FLAG"="73"
        "PROVIDER_SPECIALTY_PRIMARY"="74"
        "PROVIDER_SEX"="75"
        "MEDADMIN_ENCOUNTERID"="76"
        "MEDADMIN_PRESCRIBINGID"="77"
        "MEDADMIN_PROVIDERID"="78"
        "MEDADMIN_DOSE_ADMIN"="79"
        "MEDADMIN_DOSE_ADMIN_UNIT"="80"
        "MEDADMIN_ROUTE"="81"
        "MEDADMIN_SOURCE"="82"
        "MEDADMIN_CODE"="83"
        "MEDADMIN_TYPE"="84"
        "OBSCLIN_ENCOUNTERID"="85"
        "OBSCLIN_PROVIDERID"="86"
        "OBSCLIN_DATE"="87"
        "OBSCLIN_TIME"="88"
        "OBSCLIN_CODE"="89"
        "OBSCLIN_TYPE"="90"
        "OBSCLIN_RESULT_QUAL"="91"
        "OBSCLIN_RESULT_MODIFIER"="92"
        "OBSCLIN_RESULT_UNIT"="93"
        "OBSGEN_ENCOUNTERID"="94"
        "OBSGEN_PROVIDERID"="95"
        "OBSGEN_DATE"="96"
        "OBSGEN_TIME"="97"
        "OBSGEN_CODE"="98"
        "OBSGEN_TYPE"="99"
        "OBSGEN_RESULT_QUAL"="100"
        "OBSGEN_RESULT_MODIFIER"="101"
        "OBSGEN_RESULT_UNIT"="102"
        "ADDRESS_USE"="103"
        "ADDRESS_TYPE"="104"
        "ADDRESS_PREFERRED"="105"
        "ADDRESS_CITY"="106"
        "ADDRESS_STATE"="107"
        "ADDRESS_ZIP5"="108"
        "ADDRESS_ZIP9"="109"
        "ADDRESS_PERIOD_START"="110"
        "ADDRESS_PERIOD_END"="111"
        "IMMUNE_ENCOUNTERID"="112"
        "PROCEDURESID"="113"
        "VX_PROVIDERID"="114"
        "VX_RECORD_DATE"="115"
        "VX_ADMIN_DATE"="116"
     ...
     value $o_source
        ...
        "66"="PROCM_L3_TYPE"
        "67"="PROCM_L3_ITEMNM"
        "68"="PROCM_L3_ITEMFULLNAME"
        "69"="PROCM_L3_MEASURENM"
        "70"="PROCM_L3_MEASUREFULLNAME"
        "71"="PROCM_L3_SOURCE"
        "72"="PROV_L3_N"
        "73"="PROV_L3_NPIFLAG"
        "74"="PROV_L3_SPECIALTY"
        "75"="PROV_L3_SEX"
        "76"="MEDADM_L3_N"
        "77"="MEDADM_L3_N"
        "78"="MEDADM_L3_N"
        "79"="MEDADM_L3_DOSEADM"
        "80"="MEDADM_L3_DOSEADMUNIT"
        "81"="MEDADM_L3_ROUTE"
        "82"="MEDADM_L3_SOURCE"
        "83"="MEDADM_L3_CODE_TYPE"
        "84"="MEDADM_L3_TYPE"
        "85"="OBSCLIN_L3_N"
        "86"="OBSCLIN_L3_N"
        "87"="XTBL_L3_DATES"
        "88"="XTBL_L3_TIMES"
        "89"="OBSCLIN_L3_CODE_TYPE"
        "90"="OBSCLIN_L3_TYPE"
        "91"="OBSCLIN_L3_QUAL"
        "92"="OBSCLIN_L3_MOD"
        "93"="OBSCLIN_L3_UNIT"
        ...
        "96"="XTBL_L3_DATES"
        "97"="XTBL_L3_TIMES"
        "98"="OBSGEN_L3_CODE_TYPE"
        "99"="OBSGEN_L3_TYPE"
       "100"="OBSGEN_L3_QUAL"
       "101"="OBSGEN_L3_MOD"
       "102"="OBSGEN_L3_UNIT"
       "103"="LDSADRS_L3_ADRSUSE"
       "104"="LDSADRS_L3_ADRSTYPE"
       "105"="LDSADRS_L3_ADRSPREF"
       "106"="LDSADRS_L3_ADRSCITY"
       "107"="LDSADRS_L3_ADRSSTATE"
       "108"="LDSADRS_L3_ADRSZIP5"
       "109"="LDSADRS_L3_ADRSZIP9"
       "110"="XTBL_L3_DATES"
       "111"="XTBL_L3_DATES"
       "112"="IMMUNE_L3_N"
       "113"="IMMUNE_L3_N"
       ...
       "115"="XTBL_L3_DATES"
       "116"="XTBL_L3_DATES"
       "117"="IMMUNE_L3_CODETYPE"
       "118"="IMMUNE_L3_STATUS"
       "119"="IMMUNE_L3_STATUSREASON"
       "120"="IMMUNE_L3_SOURCE"
       "121"="IMMUNE_L3_DOSE_DIST"
       "122"="IMMUNE_L3_DOSE_UNIT"
       "123"="IMMUNE_L3_ROUTE"
       "124"="IMMUNE_L3_BODYSITE"
       "125"="IMMUNE_L3_MANUFACTURER"
       "126"="IMMUNE_L3_LOTNUM"
       "127"="XTBL_L3_DATES"
        ;
run;
...
data dum_field;
     ...
     else if 46<=ordn<=51 then table="DISPENSING";
     else if 52<=ordn<=57 then table="CONDITION";
     else if 58<=ordn<=58 then table="DEATH_CAUSE";
     else if 59<=ordn<=71 then table="PRO_CM";
     else if 72<=ordn<=75 then table="PROVIDER";
     else if 76<=ordn<=84 then table="MED_ADMIN";
     else if 85<=ordn<=93 then table="OBS_CLIN";
     else if 94<=ordn<=102 then table="OBS_GEN";
     else if 103<=ordn<=111 then table="LDS_ADDRESS";
     else if 112<=ordn<=127 then table="IMMUNIZATION";
...
data denominators;
     set dc_normal(where=(datamartid=%upcase("&dmid") and 
                ((dc_name in ("VIT_L3_N" "LAB_L3_N" "PRES_L3_N" "DISP_L3_N" 
                             ...
                             "MEDADM_L3_N" "OBSCLIN_L3_N" "OBSGEN_L3_N" "LDSADRS_L3_N" "IMMUNE_L3_N") and
                    statistic in ("ALL_N" "NULL_N") and variable in ("PATID")) or
                (dc_name in ("PROV_L3_N" "OBCLIN_L3_N" "OBSGEN_L3_N" "IMMUNE_L3_N") and 
                 statistic in ("ALL_N" "NULL_N") and variable in ("PROVIDERID")))));
...
data numerators;
     ...
     if (dc_name in ("VIT_L3_VITAL_SOURCE" "DEATH_L3_IMPUTE" "DEATH_L3_MATCH" "DEATH_L3_SOURCE"
                     ...
                     "PROCM_L3_METHOD" "PROCM_L3_MODE" "PROCM_L3_CAT" "PROCM_L3_LOINC" "PROCM_L3_TYPE" 
                     "PROCM_L3_ITEMNM" "PROCM_L3_ITEMFULLNAME" "PROCM_L3_MEASURENM" "PROCM_L3_MEASURE_FULLNAME" 
                     "PROCM_L3_SOURCE" "PROV_L3_NPIFLAG" "PROV_L3_SPECIALTY" "PROV_L3_SEX"
                     ...
                     "OBSGEN_L3_CODE_TYPE" "OBSGEN_L3_TYPE" "OBSGEN_L3_QUAL" "OBSGEN_L3_MOD" "OBSGEN_L3_RUNIT" 
                     ...
                     "IMMUNE_L3_DOSE_UNIT" "IMMUNE_L3_ROUTE" "IMMUNE_L3_BODYSITE" "IMMUNE_L3_MANUFACTURER"
                     "IMMUNE_L3_LOTNUM") and 
                     category in ("NULL or missing" "NI" "UN" "OT") and statistic="RECORD_N") or
        ...
        (dc_name in ("IMMUNE_L3_N") and statistic="NULL_N" and variable in ("ENCOUNTERID" "PROCEDURESID" "PROVIDERID")) or
        ...
        (dc_name in ("XTBL_L3_DATES") and statistic="NMISS" and variable in ("LAB_ORDER_DATE" "SPECIMEN_DATE" "DEATH_DATE"
                     "RX_ORDER_DATE" "RX_START_DATE" "RX_END_DATE" "REPORT_DATE" "RESOLVE_DATE" "ONSET_DATE" "PRO_DATE" "OBSCLIN_DATE" 
                     "OBSGEN_DATE" "ADDRESS_PERIOD_START" "ADDRESS_PERIOD_END" "VX_RECORD_DATE" "VX_ADMIN_DATE" "VX_EXP_DATE")) or
        ...
        (dc_name in ("PRES_L3_RXQTY_DIST" "PRES_L3_RXREFILL_DIST" "PRES_L3_RXDOSEODR_DIST" "MEDADM_L3_DOSEADM" "DISP_L3_DOSE_DIST"
                     "IMMUNE_L3_DOSE_DIST") and statistic="NULL or missing")
        ;
...
data print_ivd(keep=ord: col: perc thresholdflag);
     ...
     * threshold values for this table *;
     if var in ("VITAL_SOURCE" "DEATH_SOURCE" "RX_ORDER_DATE" "RX_SOURCE" 
                ...
                "ENCOUNTERID" "VX_RECORD_DATE" "VX_SOURCE" "PRO_SOURCE" 
                "DISPENSE_SOURCE" "LAB_RESULT_SOURCE") 
                and perc>=10.00 then thresholdflag=1;
...
%macro print_ive;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ive split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define perc     /display noprint;
         define col1     /display flow "Encounter Type" style(header)=[just=left cellwidth=20%] style(column)=[just=left];
         define col2     /display flow "Distinct encounter|IDs with a|principal diagnosis" style(header)=[just=center cellwidth=14%] 
                            style(column)=[just=center];
         define col3     /display flow "Distinct encounter|IDs without a|principal diagnosis" style(header)=[just=center cellwidth=14%] 
                            style(column)=[just=center];
         define col4     /computed format=8.1 "% of encounters|without a principal|diagnosis" style(header)=[just=center cellwidth=13%] 
                            style(column)=[just=center];
         define col5     /display flow "Principal|diagnoses" style(header)=[just=center cellwidth=9%] style(column)=[just=center];
         define col6     /display flow "Principal diagnoses|per encounter with any|principal diagnosis" style(header)=[just=center cellwidth=10.5%] 
                            style(column)=[just=center];
...
data data;
     set dc_normal(where=(datamartid=%upcase("&dmid") and (dc_name="PRES_L3_RXCUI_TIER" or (dc_name="PRES_L3_RXCUI_TIER_5Y" and category="Tier 1"))));
...
data print_ivh;
     ...
     if not c and ord<=5 then col6="PRES_L3_RXCUI_TIER";
     else if not c and ord=6 then col6="PRES_L3_RXCUI_TIER_5Y";
...
data dc_summary_ivh;
     ...
     if ord in (1,6) and (col5p<80.0 or compress(col4) in ("0" "BT"));
     row=3.08;
     exception=1;
...
%macro print_ivh;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ivh split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define col2     /display flow "Term Type Tier Description" style(header)=[just=left cellwidth=34%] style(column)=[just=left];
         ...
         endcomp;
         compute col5;
            if ord in (1,6) and col5p<80.0 then do;
                col5=col5p;
                call define(_col_, "style", "style=[color=blue]");
            end;
            else do;
         ...
         endcomp;
         compute col4;
            if ord in (1,6) and compress(col4) in ("0" "BT") then call define(_col_, "style", "style=[color=blue]");
         endcomp;
    run;
...
proc format;
     value known_desc
       ...
       5 = '~mQuantitative results which specify the specimen source'
       6 = '~mQuantitative results which specify the result unit'
       7 = '~mQuantitative results which fully specify the normal range'
       8 = '~mQuantitative results which specify the specimen source and ~-2nresult unit'
       9 = 'Results mapped to a known LAB_LOINC'
      10 = '~mQuantitative results which fully specify the normal range'
      11 = '~mQuantitative results which specify the specimen source and ~-2nresult unit'
       ;
...
data print_ivi;
     ...
     if l then do;
        ...
        if dc_name="LAB_L3_RECORDC" then ord=5;
        else if dc_name="LAB_L3_RECORDC_5Y" then delete;
        ...
        if dc_name="LAB_L3_RECORDC" then ord=6;
        else if dc_name="LAB_L3_RECORDC_5Y" then delete;
        col4=strip(put(denom_8,comma16.));
        if resultn>=0 and denom_8>0 then col5p=(resultn/denom_8)*100;
        else col5p=.;
...
data dc_summary_ivi;
     ...
     if ord in (2,7,8,10,11) and (col5p<80.0 or compress(col3) in ("0" "BT"));
     if ord in (2) then row=3.09;
...
%macro print_ivi;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_ivi split='|' style(header)=[backgroundcolor=CXCCCCCC];
         column pg col00 col0 col1 col3 col4 ord col5p col5 col6;
         where pg=1;
         ...
         endcomp;
         compute col5;
            if ord in (2,7,8,9,10,11) and col5p<80.0 then do;
                col5=col5p;
                call define(_col_, "style", "style=[color=blue]");
            end;
            else do;
         ...
         endcomp;
         compute col3;
            if ord in (2,7,8,9,10,11) and compress(col3) in ("0" "BT") then call define(_col_, "style", "style=[color=blue]");
         endcomp;
    run;
...
proc sql noprint;
     select unique(memname) into :_prevdata separated by '|'  from _prevdata
         where memname in ("DEM_L3_N" "ENR_L3_N" "ENC_L3_N" "DIA_L3_N" "PRO_L3_N" 
             ...
             "OBSGEN_L3_N" "HASH_L3_N" "IMMUNE_L3_N" "LDSADRS_L3_N");
...
data current(keep=ord dc_name statistic category resultn);
     set dc_normal(where=(datamartid=%upcase("&dmid") and dc_name IN ("PRO_L3_ENCTYPE"
                    "DIA_L3_ENCTYPE" "XTBL_L3_LAB_ENCTYPE" "XTBL_L3_PRES_ENCTYPE") and 
                     category in ("AV" "ED" "IP" "OA") and statistic in ("RECORD_N" "DISTINCT_PATID_N")));

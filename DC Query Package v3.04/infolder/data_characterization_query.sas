...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Characterization Query Package V3.04
* 
...
data data;
     ...
     * create UNIQUE_VISIT and determine the max length needed *;
     ...
     if cmiss(patid,encounterid,admit_date,enc_type,providerid)=0 then do;
...
data query;
     ...
     * apply threshold *;
     %threshold(nullval=col1^="ZZZA" and labname^="ZZZA")
        ...
        %if &_orph=DIAGNOSIS or &_orph=PROCEDURES or &_orph=VITAL or 
            ...
            * mismatch rows *;
            %if &_orph=DIAGNOSIS or &_orph=PROCEDURES %then %do;
                proc sql noprint;
                     create table mismatch_&_orph as select unique encounterid,
                         enc_type, admit_date, providerid
                        from pcordata.&_orph
                        where encounterid^=" "
                        order by encounterid;
                quit;
...
data query;
     ...
     set orph_encounterid(in=oe)
         ...
         mismatch_encounterid(in=mdp where=(memname="DIAGNOSIS") rename=(mis_provid_n=recordn))
         ...
         mismatch_encounterid(in=mpp where=(memname="PROCEDURES") rename=(mis_provid_n=recordn))
     ;

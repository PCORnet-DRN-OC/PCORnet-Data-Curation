...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package V4.13 - Lab queries only
* 
...
data lab_result_cm(compress=yes);
     ...
     * known test result *;
     if lab_loinc^=" " then do;
        ...
        if result_num^=. and result_modifier not in (' ' 'NI' 'UN' 'OT') and specimen_source not in (' ' 'NI' 'UN' 'OT') then known_test_result_num_source=1;
        ...
        if result_num^=. and result_modifier not in (' ' 'NI' 'UN' 'OT') and specimen_source not in (' ' 'NI' 'UN' 'OT') and 

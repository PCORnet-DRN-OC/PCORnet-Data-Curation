...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package V6.14 - all queries,
  ...
  %if %index(&_part1_mstring,%upcase(&tname))>0 %then %do;
    ...
    *- Derive categorical variable -*;
    data data;
        ...
        * keep col1 patid;
    run;
proc freq;table col1*payer_type_primary/list missing;
    *- Derive statistics - year -*;
    proc means data=data completetypes noprint missing;
         class col1/preloadfmt;
         output out=stats;
    ...
    *- Derive statistics - patid -*;
    ...
    data payer_type_primary_grp;
        ...
        keep start col1 payer_type_primary_grp;
    run;
    ...
    *- Derive statistics - patid -*;
    ...
    data payer_type_secondary_grp;
        ...
        keep start col1 payer_type_secondary_grp;
    run;
    ...
    *- Derive statistics - patid -*;
    ...
    data facility_type;
         length col1 facility_type_grp $60;
         set facility_type(rename=(facility_type_grp=facility_type_group));
         col1=start;

         if start="ZZZA" then facility_type_grp="NULL or missing";
         else if start="ZZZB" then
...
%if %index(&_part1_mstring,%upcase(&tname))>0 %then %do;
    ...
    *- Clear working directory -*;
    %clean(savedsn=&savedeath &saveenc &savedemog &savextbl xtbl_obs);
    ...
    *- Clear working directory -*;
    %clean(savedsn=&savedeath &saveenc &savedemog &savextbl);
    ...
    *- Clear working directory -*;
    %clean(savedsn=&savedeath &saveenc &savedemog);
...
%if %index(&_part1_mstring,%upcase(&tname))>0 %then %do;
  ...
  ******************************************************************************;
  ...
  %elapsed(begin);
    ...
    %end;
    %if &_yencounter=1 %then %do;
        %_dates(dsn=encounter,_vn=2,dtvlist=admit_date discharge_date,tmvlist1=,tmvlist=admit_time discharge_time)
    ...
    %end;
    %if &_ymed_admin=1 %then %do;
        %_dates(dsn=med_admin,_vn=2,dtvlist=medadmin_start_date medadmin_stop_date,tmvlist1=,tmvlist=medadmin_start_time medadmin_stop_time)

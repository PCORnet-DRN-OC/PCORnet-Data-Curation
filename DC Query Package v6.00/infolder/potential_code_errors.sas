...
data lab_history (compress=yes);
	...
	*get the max length of the loinc code to use in misplaced_loincs;
	retain max_length 0;
        ...
        if raw_length>max_length then max_length=raw_length;
        if eof then do;
                Call symputx("_len_hist",max_length);
              end;
    *create other variables needed for bad records;
	table='LAB_HISTORY';
	code_type='LC';
...
data obs_clin (compress=yes);
	...
	*get the max length of the loinc code to use in misplaced_loincs;	
	retain max_length 0;
        raw_length=length(obsclin_code);
        if raw_length>max_length then max_length=raw_length;
        if eof then do;
                Call symputx("_len_clin",max_length);
              end;
    *create other variables needed for bad records;
	table='OBS_CLIN';
...
data obs_gen (compress=yes);
	...
	*get the max length of the loinc code to use in misplaced_loincs;	
	retain max_length 0;
        raw_length=length(obsgen_code);
        if raw_length>max_length then max_length=raw_length;
        if eof then do;
                Call symputx("_len_gen",max_length);
              end;
    *create other variables needed for bad records;
	table='OBS_GEN';
...
data dmlocal.code_summary (drop=i);
	...
	set diagnosis_summary 
		...
		;
	label code_type='Code Type' table='Table' records='Total Records' codes='Total Codes' bad_records='Potential Bad Records' 
...
proc print width=min data=dmlocal.code_summary label;
	var table code_type records codes bad_record_pct misplaced_record_pct;
	run;

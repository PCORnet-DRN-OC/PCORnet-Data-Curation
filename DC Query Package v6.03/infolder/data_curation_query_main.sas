...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package V6.03 - all queries,
...
data &qname(compress=yes);
     ...
     * define diagnosisid_plus *;
     if (.<admit_date<=&mxrefreshn or .<dx_date<=&mxrefreshn) and encounterid^=' ' and 
        dx_type not in ('NI' 'UN' 'OT' ' ') then diagnosisid_plus=1;
...
%enc_oneway(encdsn=diagnosis,encvar=diagnosisid_plus,_nc=1,ord=5)
...
data &qname(compress=yes);
     ...
     * define proceduresid_plus *;
     if (.<admit_date<=&mxrefreshn or .<px_date<=&mxrefreshn) and encounterid^=' ' and 
        px_type not in ('NI' 'UN' 'OT' ' ') then proceduresid_plus=1;
...
%enc_oneway(encdsn=procedures,encvar=proceduresid_plus,_nc=1,ord=5)
...
data &qname(compress=yes);
     ...
     * define prescribingid_plus *;
     if (.<rx_start_date<=&mxrefreshn or .<rx_order_date<=&mxrefreshn) and rxnorm_cui^=' ' then 
        prescribingid_plus=1;
...
%enc_oneway(encdsn=prescribing,encvar=prescribingid_plus,_nc=1,ord=5)
...
data &qname(compress=yes);
     ...
     * define medadminid_plus *;
     if medadmin_code^=' ' and medadmin_type not in ('NI' 'UN' 'OT' ' ') and 
        .<medadmin_start_date<=&mxrefreshn then medadminid_plus=1;
...
%enc_oneway(encdsn=med_admin,encvar=medadminid_plus,_nc=1,ord=6)
...
%macro _5yrlook(sufx,subset);
    ...
    *- Derive categorical variable -*;
    ...
    proc sort data=data(rename=(medadmin_code=rxnorm_cui));
         by rxnorm_cui;     
    run;
...
data query;
     ...
     * counts *;
     ...
     keep datamartid response_date query_package provider_specialty_group
          provider_specialty_primary enc_type record_n;
run;

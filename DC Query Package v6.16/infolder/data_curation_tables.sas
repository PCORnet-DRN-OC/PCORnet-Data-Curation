...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package V6.16 - all queries
* 
...
%if &_yldsadrs=1 %then %do;
    ...
    %if %upcase(&_part2)=YES %then %do;
        ...
        * Create format;
        proc format library=work cntlin=address_rank;
        run;
    ...
    data
        ...
        %end;
        ;
		set pcordata.lds_address_history (keep=patid address_zip5 address_zip9 address_use address_type address_preferred address_period_end
            %if %upcase(&_part1)=YES %then %do;
		...
		* Flag valid zip codes;
        ...
        %if %upcase(&_part2)=YES %then %do;
            ...
            * Output records to be used in zip code queries for patients with F2F visits within a certain timeframe;
            if valid_address_zip5='Y' and address_preferred='Y' and address_use not in ('WO' 'TP' 'OL') and address_period_end='' then output lds_address_history_f2f;
        %end;
    run;
...
%if &_ypro_cm=1 %then %do;
	...
	data pro_cm (compress=yes);
        set pcordata.pro_cm (keep=patid pro_date 
            %if %upcase(&_part1)=YES %then %do;
                encounterid pro_time pro_cat pro_method pro_mode pro_cm_id pro_source pro_type
            %end;
            %if %upcase(&_part2)=YES %then %do;
                pro_item_fullname pro_item_name pro_item_loinc pro_measure_fullname pro_measure_loinc pro_measure_name
            %end;
            ) end=eof;
...
%if &_ydemographic=1 and %upcase(&_part1)=YES %then %do;
	...
	data demographic (compress=yes);
        set pcordata.demographic (keep=patid birth_date birth_time gender_identity hispanic sexual_orientation pat_pref_language_spoken race sex) end=eof;
	run;
...
%if %upcase(&_part1)=YES %then %do;
    ...
    *- Macro to compare dates within a data table -*;
    %macro within(idsn,var,var2,adjp,adjm,ord);
        ...
        * Delete intermediary datasets;
        proc delete data=&idsn &idsn&var;
        quit;
	...
	* Delete intermediary datasets;
	proc delete data=data sumproceduresdischarge_date sumprocedures_dlpx_date sumencounter_dldischarge_date sumdiagnosis_dldx_date sumdiagnosisdischarge_date
            sumobs_clin_dlobsclin_stop_date sumobs_gen_dlobsgen_stop_date query;
	quit;
...
%if %upcase(&_part1)=YES %then %do;
    ...
    %macro xminmax(idsn);
        ...
        * Delete them if they are not needed for the _TIMES version of this query;
        %if &idsn=diagnosis or &idsn=procedures or &idsn=enrollment or &idsn=death or &idsn=dispensing or &idsn=condition or &idsn=lds_address_history or &idsn=immunization %then %do;
		...
		*******************************************************************************;
		%if %upcase(&_part2)=YES %then %do;
			...
			*Start timer outside of macro in order to capture derivation processing time;
            ...
            data dmlocal.obsclin_l3_loinc;
                length obsclin_code $15;
                merge
                    dmlocal.obsclin_l3_loinc (in=inp)
...
%if &_ypro_cm=1 %then %do;
		...
		*******************************************************************************;
		%if %upcase(&_part2)=YES %then %do;
			...
			%n_pct_noref(dset=PRO_CM, var=PRO_ITEM_FULLNAME,   qnam=procm_l3_itemfullname);
            %n_pct_noref(dset=PRO_CM, var=PRO_ITEM_NAME,       qnam=procm_l3_itemnm);
			...
			*Start timer outside of macro in order to capture derivation processing time;
            ...
            %n_pct_noref(dset=PRO_CM, var=PRO_ITEM_LOINC, qnam=procm_l3_loinc, distinct=y, timer=n);

            data dmlocal.procm_l3_loinc;
                length pro_item_loinc $15;
                merge
                    dmlocal.procm_l3_loinc (in=inp)
                    loinc (keep=loinc_num panel_type rename=(loinc_num=pro_item_loinc))
                    ;
                by pro_item_loinc;
                if inp;

                format pro_item_loinc;
                informat pro_item_loinc;
                label pro_item_loinc=' ';
            run;
			...
			%n_pct_noref(dset=PRO_CM, var=PRO_MEASURE_FULLNAME, qnam=procm_l3_measure_fullname);
			%n_pct_noref(dset=PRO_CM, var=PRO_MEASURE_LOINC,   qnam=procm_l3_measure_loinc);
			%n_pct_noref(dset=PRO_CM, var=PRO_MEASURE_NAME,    qnam=procm_l3_measurenm);
...
%if &_yldsadrs=1 %then %do;
		...
		*******************************************************************************;
		%if %upcase(&_part1)=YES %then %do;
			...
			%tag(dset=LDS_ADDRESS_HISTORY,  varlist=PATID ADDRESSID ADDRESS_ZIP5 ADDRESS_ZIP9 ADDRESS_COUNTY, qnam=ldsadrs_l3_n, valid1=3, valid2=4);     
		...
		%if %upcase(&_part2)=YES %then %do;
			...
			*******************************************************************************; 
            %if &_yldsadrs=1 %then %do;
                ...
                * Delete intermediary datasets;
                proc delete data=lds_address_history_f2f encounter_period zip_5y;
                quit;
		...
		*******************************************************************************;
		%if %upcase(&_part2)=YES %then %do;
            ...
            * Merge flag back onto DIAGNOSIS by ENCOUNTERID;
            ...
            %n_pct_multilev(dset=DIAGNOSIS, var1=PDXGRP, var2=ENC_TYPE, var3=DX_ORIGIN, ref2=valuesets, ref3=valuesets, qnam=dia_l3_pdxgrp_enctype, distinctenc=y, timer=n);
            ...
            * Drop RECORD_N and PDXGRP=NULL or missing rows that come out of macro for this special case;
            data dmlocal.dia_l3_pdxgrp_enctype;
                ...
                if pdxgrp='NULL or missing' then delete;

                drop record_n;
            run;                
            
            * Delete intermediary and DIAGNOSIS datasets;
            proc delete data=pdxgrp diagnosis;
            quit;
		...
		*******************************************************************************;
		%if %upcase(&_part1)=YES %then %do;
            ...
            /***LAB_L3_QUAL***/
				...
				* Delete intermediary datasets;
				proc delete data=lab_result_cm_titer;
                quit;
		...
		*******************************************************************************;
		%if %upcase(&_part2)=YES %then %do;
			...
			*Start timer outside of macro in order to capture derivation processing time;
            ...
            data dmlocal.lab_l3_loinc;
                length lab_loinc $15;
                merge
                    dmlocal.lab_l3_loinc (in=inp)
		...
		%if %upcase(&_part2)=YES %then %do;
            ...
            * Create format for RXNORM_CUI_TTY from RXNORM_CUI_REF reference file;
            data rxnorm_cui_tty;
                ...
                * Find longest length of RXNORM_CUI_TTY in reference file (length must be at least $15 to accont for NULL or missing);
            ...
            * Merge PRES_L3_CODE_TYPE records onto reference file for categorizing into tiers;
            data pres_l3_code_type_tiers;
                ...
                merge
                    dmlocal.pres_l3_rxcui (in=inm keep=rxnorm_cui record_n)
            ...
            * Delete intermediary datasets;
            proc delete data=pres_l3_code_type_tiers pres_l3_rxcui_tier rxnorm_cui_ref;
            quit;
...
%if &_yvital=1 %then %do;
		...
		*******************************************************************************;
		%if %upcase(&_part1)=YES %then %do;
			...
			%n_pct(dset=VITAL, var=SMOKING,     qnam=vit_l3_smoking, dec=2);
			%n_pct(dset=VITAL, var=TOBACCO,     qnam=vit_l3_tobacco, dec=2);
		...
		*******************************************************************************;
		%if %upcase(&_part2)=YES %then %do;
			...
			* Create custom decode list for VIT_L3_BMI categories;
			...
			%n_pct(dset=VITAL_BMI, var=BMI_GROUP, qnam=vit_l3_BMI, ref=bmi_group_ref, cdmref=n, timer=n, alphasort=n, dec=2);
			...
			* Create custom decode list for VIT_L3_DIASTOLIC categories;
			...
			%n_pct(dset=VITAL_DBP, var=DIASTOLIC_GROUP, qnam=vit_l3_diastolic, ref=diastolic_group_ref, cdmref=n, timer=n, alphasort=n, dec=2);
			...
			* Create custom decode list for VIT_L3_SYSTOLIC categories;
			...
			%n_pct(dset=VITAL_SBP, var=SYSTOLIC_GROUP, qnam=vit_l3_systolic, ref=systolic_group_ref, cdmref=n, timer=n, alphasort=n, dec=2);

...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package V3.12
* 
...
********************************************************************************;
proc cimport library=cptdata infile=dcref;
run;

********************************************************************************;
*- Create a format of all of the lab group values
********************************************************************************;
data lab_dcgroup_ref;
     set cptdata.lab_dcgroup_ref;
...
proc freq data=lab_dcgroup_ref noprint;
...
data data;
     length col1 $15;
     set lab_result_cm(keep=raw_lab_name patid) end=eof;
...
*- Sort lab data and expected specimen source data for merging -*;
proc sort data=cptdata.lab_dcgroup_ref(keep=lab_loinc dc_lab_group include_edc) 
     out=lab_dcgroup_ref;
     by lab_loinc;
run;
...
*- Clear working directory -*;
%clean(savedsn=&savedeath &savedemog &saveenc &savediag &saveproc &saveenr &savevit 
               &savepres &savedisp &savelab &savecond &saveprocm enc_2010);

...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet EDC report V7.00
* 
...
data print_id;
     ...
     if col2='' then do;
          if table in ('EXTERNAL_MEDS' 'PAT_RELATIONSHIP' 'LAB_HISTORY') then col2='---';
          else col2='N/A';
     end;
...
data print_iiic;
     ...
     /*For v7.00, exclude the new PAT_RELATIONSHIP and EXTERNAL_MEDS data conparisons*/
     if category in ('RELATIONSHIP_START > RELATIONSHIP_END' 'EXT_PAT_START_DATE > EXT_PAT_END_DATE') then delete;
...
data data;
     ...
     set dc_normal(where=(datamartid=%upcase("&dmid") and statistic="RECORD_N" and
         ...
         (dc_name in ('VIT_L3_MDATE_YM' 'PRES_L3_ODATE_YM' 'LAB_L3_RDATE_YM' 'MEDADM_L3_SDATE_YM') and category ne "NULL or missing"))))
     ;
     ...
     * create a month variable *;
     ...
     else if dc_name in ('VIT_L3_MDATE_YM' 'PRES_L3_ODATE_YM' 'LAB_L3_RDATE_YM' 'MEDADM_L3_SDATE_YM') then 
...
data print_iiig;
     ...
     * keep only months where no obs or beyond the limit of std deviations *;
     ...
     array o c1 c2 c3 nc1 nc2 nc3 nc4;
        do oi = 1 to dim(o);
...
data dum_field;
     ...
     else if 163<=ordn<=170 then table="PRO_CM";
...
data denominators;
     set dc_normal(where=(datamartid=%upcase("&dmid") and 
                ((dc_name in ("VIT_L3_N" "LAB_L3_N" "PRES_L3_N" "DISP_L3_N" 
                              ...
                              "IMMUNE_L3_N" "EXTMED_L3_N" "PROCM_L3_N") and
                    statistic in ("ALL_N" "NULL_N") and variable in ("PATID")) or
...
data numerators;
     ...
     if (dc_name in ("VIT_L3_VITAL_SOURCE" "DEATH_L3_IMPUTE" "DEATH_L3_MATCH" "DEATH_L3_SOURCE"
                    ...
                    "DISP_L3_DOSEUNIT" "DISP_L3_ROUTE" "DISP_L3_SOURCE"
                     ...
                     "EXTMED_L3_ROUTE" "EXTMED_L3_SOURCE" "EXTMED_L3_BASIS" "EXTMED_L3_DOSE"
                     "PROCM_L3_CAT" "PROCM_L3_METHOD" "PROCM_L3_MODE" "PROCM_L3_SOURCE" "PROCM_L3_TYPE") and 
                     category in ("NULL or missing" "NI" "UN" "OT") and statistic="RECORD_N") or
...
data print_ivd(keep=ord: col: perc thresholdflag);
     ...
     * threshold values for this table *;
     if var in ("VITAL_SOURCE" "DEATH_SOURCE" "RX_ORDER_DATE" "RX_SOURCE" 
                ...
                "DEATH_CAUSE_SOURCE" "RX_ENCOUNTERID"
                "OBSCLIN_ENCOUNTERID")
                and perc>=10.00 then thresholdflag=1;
...
%macro print_vc;
    ...
    *- Get titles and footnotes -;
    ...
    proc report data=print_vc split='|' style(header)=[backgroundcolor=CXCCCCCC];
         ...
         define col2     /display flow "Previous|Refresh" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col3     /computed format=comma16. "Current|Refresh" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col4     /computed format=comma16.1 "Percent|Change" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col5     /display flow "Previous|Refresh" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col6     /computed format=comma16. "Current|Refresh" style(header)=[just=center cellwidth=11%] style(column)=[just=center];
         define col7     /computed format=comma16.1 "Percent|Change" style(header)=[just=center cellwidth=11%] style(column)=[just=center];

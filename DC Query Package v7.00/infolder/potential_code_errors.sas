...
*           elapsed.sas7bdat (elapsed time of query processing)            
...
%macro elapsed(b_or_e);
    ...
    %else %if &b_or_e=end %then %do;
        data _qe(drop=_elapsed_time _total_run_time);
            length query $100;
            format elapsed_time total_run_time time8.;
            set _qe;
                _qend=datetime();
                elapsed_time=_qend-_qstart;
                _elapsed_time=put(elapsed_time,time8.);
                total_run_time=_qend-input("&_pstart",datetime16.);
                _total_run_time=put(total_run_time,time8.);
                 put "******************************************************************************************************";
...
data elapsed;
     length query $100;
     format _qstart _qend datetime19. elapsed_time total_run_time time8.;
     array _char_ query ;
     array _num_  _qstart _qend elapsed_time total_run_time;
     query="QUERY PROGRAM";
...
data dmlocal.elapsed;
     ...
     label 
        ...
        elapsed_time="Query run time (hh:mm:ss)"
        total_run_time="Cumulative run time (hh:mm:ss)"
        ;
        
     if query="QUERY PROGRAM" then do;
        _qend=datetime();
        elapsed_time=_qend-_qstart;
        total_run_time=_qend-input("&_pstart",datetime16.);
     end;
...
proc print width=min data=dmlocal.elapsed label noobs;
	var obs query _qstart _qend elapsed_time total_run_time;
	run;

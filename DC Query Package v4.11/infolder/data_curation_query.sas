...
*    Study: PCORnet
*
*  Purpose: Produce PCORnet Data Curation Query Package V4.11
* 
...
%macro clean(savedsn);
     proc datasets  noprint;
          save xtbl_mdata_idsn elapsed short_x_unit short_result_unit short_source 
               facility_type _route prov_spec lab_dcgroup_ref lab_loinc_ref 
               rxnorm_cui_ref rx_dose_form &savedsn / memtype=data;
...
%macro fmt(fmt=,sheet=);
    ...
    * need a second format for grouping *;
    ...
    %end;
%mend fmt;
%fmt(fmt=preflang,sheet=PATIENT_PREF_LANGUAGE_SPOKEN)
%fmt(fmt=payer_type,sheet=PAYER_TYPE_)
%fmt(fmt=facility_type,sheet=FACILITY_TYPE)
%fmt(fmt=short_result_unit,sheet=Short_RESULT_UNIT)
%fmt(fmt=short_x_unit,sheet=Short_DOSE_X_UNIT)
%fmt(fmt=_unit,sheet=_UNIT)
%fmt(fmt=_route,sheet=_ROUTE)
%fmt(fmt=short_source,sheet=Short_SPECIMEN_SOURCE)
%fmt(fmt=_source,sheet=SPECIMEN_SOURCE)
%fmt(fmt=rx_dose_form,sheet=RX_DOSE_FORM)
%fmt(fmt=prov_spec,sheet=PROVIDER_PRIMARY_SPECIALTY)
...
data med_admin(compress=yes);
     ...
     * restrict data to within lookback period *;
     if medadmin_start_date>=&lookback_dt;
...
%let savemedadm=med_admin med_admin_medadmin_start_date med_admin_medadmin_stop_date;
...
*- Clear working directory -*;
%clean(savedsn=&savedeath &savedemog &saveenc &savediag &saveproc &saveenr &savevit 
               &savepres &savedisp &savelab &savemedadm);
...
%let saveobsclin=obs_clin obs_clin_obsclin_date;
...
*- Clear working directory -*;
%clean(savedsn=&savedeath &savedemog &saveenc &savediag &saveproc &saveenr &savevit 
               &savepres &savedisp &savelab &savemedadm &saveobsclin);
...
data data;
     ...
     if obsclin_type in ("ND" "RX") then obsclintype=obsclin_type;
...
*- Clear working directory -*;
%clean(savedsn=&savedeath &savedemog &saveenc &savediag &saveproc &saveenr &savevit 
               &savepres &savedisp &savelab &savemedadm &saveobsclin &savecond);
...
*- Clear working directory -*;
%clean(savedsn=&savedeath &savedemog &saveenc &savediag &saveproc &saveenr &savevit 
               &savepres &savedisp &savelab &savemedadm &saveobsclin &savecond &saveprocm);
...
data pcornet_trial(compress=yes);
     ...
     * restrict data to within lookback period *;
     if trial_enroll_date>=&lookback_dt;
        ...
        * mismatch rows - provider *;
        %if &_yprovider=1 %then %do;
            %if &_orph=DIAGNOSIS or &_orph=ENCOUNTER or &_orph=MED_ADMIN or &_orph=OBS_CLIN or 
                &_orph=PRESCRIBING or &_orph=PROCEDURES %then %do;                    
                ...
                data mismatch_providerid_&_orph(keep=memname ord mis:);
                     length providerid $ &maxlength memname $32;
...
%macro orph_pat;
    %do op = 1 %to &workdata_count;
        ...
        %if &_orphp=ENROLLMENT or &_orphp=ENCOUNTER or &_orphp=DIAGNOSIS or 
            ...
            &_orphp=PCORNET_TRIAL or &_orphp=MED_ADMIN or &_orphp=OBS_CLIN %then %do;
